#!/bin/bash
for line in `grep -e "Cl.*Include=\"\." win32/DSGame-server/*proj | tr -d " \"\" \"\/\>\r" | cut -c25- | sed 's:\\\:\/:g' `; do echo -n "${line} \\"; echo; done > game.sources
...
for line in `grep -e "Cl.*Include=\"\." win32/DSConnect-server/*proj | tr -d " \"\/\>\r" | cut -c25- | sed 's:\\\:\/:g' `; do echo -n "${line} \\"; echo; done > connect.sources

echo SRC_LOGIN = \\ >> sources.am
cat connect.sources | grep src/login/ | sed '$s/.$//' >> sources.am
echo "" >> sources.am

for line in `grep -e "Cl.*Include=\"\." win32/DSSearch-server/*proj | tr -d " \"\/\>\r" | cut -c25- | sed 's:\\\:\/:g' `; do echo -n "${line} \\"; echo; done > search.sources
