...
function onAbilityCheck(player,target,ability)
    ...
    if (player:hasStatusEffect(effectID) or player:hasBustEffect(effectID)) then
        ...
        player:setLocalVar("BST_roll_bonus", 0);
        return 0,0;
    end
end;
...
function onUseAbilityRoll(caster,target,ability,total)
    ...
    local jobBonus = caster:getLocalVar("BST_roll_bonus");

    if (total < 12) then -- see chaos_roll.lua for comments
        if (jobBonus == 0) then
            if (caster:hasPartyJob(JOB_BST) or math.random(0, 99) < caster:getMod(MOD_JOB_BONUS_CHANCE)) then
                jobBonus = 1;
            else
                jobBonus = 2;
            end
        ...
        if (target:getID() == caster:getID()) then
            caster:setLocalVar("BST_roll_bonus", jobBonus);
        end
    end
