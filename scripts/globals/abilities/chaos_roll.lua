...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.CHAOS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.CHAOS_ROLL, JOBS.DRK);
    end
...
-- Check if COR Main or Sub    
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.CHAOS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_ATTP) == false) then
