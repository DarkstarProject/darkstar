...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.CHORAL_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.CHORAL_ROLL, JOBS.BRD);
    end
...
-- Check if COR Main or Sub   
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.CHORAL_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_SPELLINTERRUPT) == false) then
