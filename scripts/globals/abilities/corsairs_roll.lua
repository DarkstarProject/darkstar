...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.CORSAIRS_ROLL;
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.CORSAIRS_ROLL, JOBS.COR);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.CORSAIRS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_EXP_BONUS) == false) then
