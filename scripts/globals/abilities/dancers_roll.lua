...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.DANCERS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.DANCERS_ROLL, JOBS.DNC);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.DANCERS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_REGEN) == false) then
