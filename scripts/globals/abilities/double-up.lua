...
function onAbilityCheck(player,target,ability)
    ...
    if (not player:hasStatusEffect(EFFECT.DOUBLE_UP_CHANCE)) then
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        local du_effect = caster:getStatusEffect(EFFECT.DOUBLE_UP_CHANCE);
        ...
        local snake_eye = caster:getStatusEffect(EFFECT.SNAKE_EYE);
        if (snake_eye) then
            ...
            caster:delStatusEffect(EFFECT.SNAKE_EYE)
        else
