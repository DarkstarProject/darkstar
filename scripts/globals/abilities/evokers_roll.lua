...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.EVOKERS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.EVOKERS_ROLL, JOBS.SMN);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.EVOKERS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_REFRESH) == false) then
