...
function onUseAbility(player,target,ability)
    ...
    if target:hasStatusEffect(EFFECT.TERROR) == true or target:hasStatusEffect(EFFECT.STUN) == true then -- effect already on, or target stunned, do nothing
    ...
    if resist <= 90 then -- still experimental. not exactly sure how to calculate hit %
        target:addStatusEffect(EFFECT.TERROR,potency,0,duration);
    else
