...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.FIGHTERS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.FIGHTERS_ROLL, JOBS.WAR);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.FIGHTERS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_DOUBLE_ATTACK) == false) then
