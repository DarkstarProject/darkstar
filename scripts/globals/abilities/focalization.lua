...
function onAbilityCheck(player,target,ability)
    if player:hasStatusEffect(dsp.effects.FOCALIZATION) then
...
function onUseAbility(player,target,ability)
    player:addStatusEffect(dsp.effects.FOCALIZATION,player:getMerit(MERIT_FOCALIZATION),0,60);

    return dsp.effects.FOCALIZATION;
end;