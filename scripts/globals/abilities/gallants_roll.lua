...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.GALLANTS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.GALLANTS_ROLL, JOBS.PLD);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.GALLANTS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_DMG) == false) then
