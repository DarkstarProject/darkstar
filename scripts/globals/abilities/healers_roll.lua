...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.HEALERS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.HEALERS_ROLL, JOBS.WHM);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.HEALERS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_CURE_POTENCY_RCVD) == false) then
