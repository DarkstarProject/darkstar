...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.HUNTERS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.HUNTERS_ROLL, JOBS.RNG);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.HUNTERS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_ACC) == false) then
