...
function onUseAbility(player,target,ability,action)
    if (not target:hasStatusEffect(EFFECT.CHAINBOUND, 0) and not target:hasStatusEffect(EFFECT.SKILLCHAIN, 0)) then
        target:addStatusEffectEx(EFFECT.CHAINBOUND, 0, 2, 0, 5, 0, 1);
    else
