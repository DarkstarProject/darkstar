...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.MAGUSS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.MAGUSS_ROLL, JOBS.BLU);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.MAGUSS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_MDEF) == false) then
