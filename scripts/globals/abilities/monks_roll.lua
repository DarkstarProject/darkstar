...
function onAbilityCheck(player,target,ability)
    local effectID = dsp.effects.MONKS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, dsp.effects.MONKS_ROLL, dsp.jobs.MNK);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), dsp.effects.MONKS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_SUBTLE_BLOW) == false) then
