...
function onAbilityCheck(player,target,ability)
    local effectID = dsp.effects.NINJA_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, dsp.effects.NINJA_ROLL, dsp.jobs.NIN);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), dsp.effects.NINJA_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_EVA) == false) then
