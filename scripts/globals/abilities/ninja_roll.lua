...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.NINJA_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.NINJA_ROLL, JOBS.NIN);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.NINJA_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_EVA) == false) then
