...
function onEquip(pet)
    pet:addListener("AUTOMATON_ATTACHMENT_CHECK", "ATTACHMENT_FLASHBULB", function(automaton, target)
        ...
        if master and master:countEffect(EFFECT.LIGHT_MANEUVER) > 0 and automaton:getLocalVar("flash") < VanadielTime() and (automaton:checkDistance(target) - target:getModelSize()) < 7 then
