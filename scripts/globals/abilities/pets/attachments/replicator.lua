...
function onEquip(pet)
    pet:addListener("AUTOMATON_ATTACHMENT_CHECK", "ATTACHMENT_REPLICATOR", function(automaton, target)
        ...
        if master and master:countEffect(dsp.effect.WIND_MANEUVER) > 0 and automaton:getHPP() <= hpthreshold and not automaton:hasStatusEffect(dsp.effect.BLINK) and automaton:getLocalVar("blink") < VanadielTime() then
