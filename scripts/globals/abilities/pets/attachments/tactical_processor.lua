...
function onEquip(pet)
    pet:addMod(dsp.mod.AUTO_DECISION_DELAY, 50) -- -0.5s
end

function onUnequip(pet)
    pet:delMod(dsp.mod.AUTO_DECISION_DELAY, 50)
end

function onManeuverGain(pet, maneuvers)
    if maneuvers == 1 then
        pet:addMod(dsp.mod.AUTO_DECISION_DELAY, 20) -- -0.70s ?
    elseif maneuvers == 2 then
        pet:addMod(dsp.mod.AUTO_DECISION_DELAY, 15) -- -0.85s ?
    elseif maneuvers == 3 then
        pet:addMod(dsp.mod.AUTO_DECISION_DELAY, 30) -- -1.15s
    end
end
