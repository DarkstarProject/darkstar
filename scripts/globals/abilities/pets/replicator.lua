...
function onPetAbility(target, automaton, skill, master, action)
    ...
    local maneuvers = master:countEffect(EFFECT.WIND_MANEUVER)
    local duration = 300
    local shadows = 1 + maneuvers -- math.floor(maneuvers * 3.5) currently on retail

    if target:addStatusEffect(EFFECT.BLINK, shadows, 0, duration) then
        skill:setMsg(msgBasic.SKILL_GAIN_EFFECT)
        for i = 1, maneuvers do
            master:delStatusEffectSilent(EFFECT.WIND_MANEUVER)
        end
    else
