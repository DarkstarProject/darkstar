...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.ROGUES_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.ROGUES_ROLL, JOBS.THF);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.ROGUES_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_CRITHITRATE) == false) then
