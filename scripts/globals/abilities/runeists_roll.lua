...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.RUNEISTS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.RUNEISTS_ROLL, JOBS.RUN);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.RUNEISTS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_MEVA) == false) then
