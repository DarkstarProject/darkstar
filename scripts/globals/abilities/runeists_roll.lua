...
function onAbilityCheck(player,target,ability)
    local effectID = dsp.effects.RUNEISTS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, dsp.effects.RUNEISTS_ROLL, dsp.jobs.RUN);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), dsp.effects.RUNEISTS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_MEVA) == false) then
