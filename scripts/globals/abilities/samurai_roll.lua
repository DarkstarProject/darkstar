...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.SAMURAI_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.SAMURAI_ROLL, JOBS.SAM);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.SAMURAI_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_STORETP) == false) then
