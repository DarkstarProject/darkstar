...
function onAbilityCheck(player,target,ability)
    local effectID = EFFECT.SCHOLARS_ROLL
...
function onUseAbility(caster,target,ability,action)
    if (caster:getID() == target:getID()) then
        corsairSetup(caster, ability, action, EFFECT.SCHOLARS_ROLL, JOBS.SCH);
    end
...
-- Check if COR Main or Sub
    ...
    if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT.SCHOLARS_ROLL, effectpower, 0, duration, caster:getID(), total, MOD_CONSERVE_MP) == false) then
