...
function onUseAbility(player,target,ability,action)
    ...
    if (player:hasStatusEffect(EFFECT.UNLIMITED_SHOT)) then
        player:delStatusEffect(EFFECT.UNLIMITED_SHOT);
        recycleChance = 100;
    end

     -- TODO: Acc penalty for /RNG, acc vs. mob level?
    if (math.random(0, 99) >= target:getMod(MOD_BINDRES) and target:hasStatusEffect(EFFECT.BIND) == false) then
        target:addStatusEffect(EFFECT.BIND, 0, 0, duration);
