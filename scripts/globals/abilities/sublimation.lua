...
function onUseAbility(player,target,ability)
    ...
    local sublimationComplete = player:getStatusEffect(EFFECT.SUBLIMATION_COMPLETE);
    local sublimationCharging = player:getStatusEffect(EFFECT.SUBLIMATION_ACTIVATED);
    local mp = 0;
    ...
    if sublimationComplete ~= nil then
        ...
        player:addMP(mp);
        player:delStatusEffectSilent(EFFECT.SUBLIMATION_COMPLETE);
    ...
    elseif sublimationCharging ~= nil then
        ...
        player:addMP(mp);
        player:delStatusEffectSilent(EFFECT.SUBLIMATION_ACTIVATED);
        ability:setMsg(msgBasic.JA_RECOVERS_MP);
    else
        local refresh = player:getStatusEffect(EFFECT.REFRESH);
        if refresh == nil or refresh:getSubPower() < 3 then
            player:delStatusEffect(EFFECT.REFRESH);
            player:addStatusEffect(EFFECT.SUBLIMATION_ACTIVATED,0,3,7200);
        else
