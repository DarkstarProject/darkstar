...
function onUseAbility(player,target,ability)
    ...
    local burden = 15
    if (target:getStat(dsp.mod.DEX) < target:getPet():getStat(dsp.mod.DEX)) then
        burden = 20
    end

    local overload = target:addBurden(dsp.magic.ele.THUNDER-1, burden)
    ...
    if (overload ~= 0) then
        ...
        target:addStatusEffect(dsp.effect.THUNDER_MANEUVER, bonus, 0, utils.clamp(dur,60,300))

    end

    return dsp.effect.THUNDER_MANEUVER
end