...
function onAbilityCheck(player,target,ability)
    if player:hasStatusEffect(EFFECT.TRANQUILITY) then
...
function onUseAbility(player,target,ability)
    player:addStatusEffect(EFFECT.TRANQUILITY,player:getMerit(MERIT_TRANQUILITY),0,60);

    return EFFECT.TRANQUILITY;
end;