...
function onAbilityCheck(trust, target, ability)
    ...
    if master:getFreeSlotsCount() == 0 then
        return 1, 0
    end
...
function onUseAbility(trust, target, ability, action)
    local master = trust:getMaster();
    local level = trust:getMainLvl() -- Can only reach THF77 as main job
    local despoilMod = trust:getMod(dsp.mod.DESPOIL)
