...
function checkNonTradeBCNM(player, npc, mode)
    ...
    local checks =
    {
        [6] =   {
                    ...
                    [643] = function() return (player:hasKeyItem(dsp.kis.ZEPHYR_FAN))  end, -- Brothers ENM
                },
        [8] =   {
                    [672] = function() return (player:getCurrentMission(COP) == THREE_PATHS  and  player:getVar("COP_Ulmia_s_Path") == 5)  end, -- head_wind
                    [673] = function() return (player:hasKeyItem(dsp.kis.MIASMA_FILTER)==true)  end,
                },
        [10] =  {
                    [704] = function() return (player:getCurrentMission(COP) == DARKNESS_NAMED  and  player:getVar("PromathiaStatus") == 2)  end,-- DARKNESS_NAMED
                    [706] = function() return (player:hasKeyItem(dsp.kis.VIAL_OF_DREAM_INCENSE)==true)  end, -- waking_dreams (diabolos avatar quest)
                },
        [13] =  {
        ...
        [17] = {
                    [768] = function() return (player:getCurrentMission(COP) == BELOW_THE_ARKS and player:getVar("PromathiaStatus") ==1 )  or
                                (player:getCurrentMission(COP) == THE_MOTHERCRYSTALS and player:hasKeyItem(dsp.kis.LIGHT_OF_HOLLA) == false) end, -- light of holla
               },
        [19] = {
                    [800] = function() return (player:getCurrentMission(COP) == BELOW_THE_ARKS and player:getVar("PromathiaStatus") ==1 )  or
                                (player:getCurrentMission(COP) == THE_MOTHERCRYSTALS and player:hasKeyItem(dsp.kis.LIGHT_OF_DEM) == false)  end, -- light of dem
               },
        [21] = {
                    [832] = function() return (player:getCurrentMission(COP) == BELOW_THE_ARKS and player:getVar("PromathiaStatus") ==1 )  or
                                (player:getCurrentMission(COP) == THE_MOTHERCRYSTALS and player:hasKeyItem(dsp.kis.LIGHT_OF_MEA) == false)  end, -- light of mea
               },
        [23] = {
        ...
        [140] = {
                    ...
                    [33] = function() return (player:hasKeyItem(dsp.kis.DRAGON_CURSE_REMEDY))  end, -- DRG Flag Quest
                },
        [144] = {
        ...
        [146] = {
                    [96] = function() return (player:hasKeyItem(dsp.kis.DARK_KEY))  end, -- Mission 2-3
        ...
        [170] = {
                    [224] = function() return (player:hasKeyItem(dsp.kis.MOON_BAUBLE))  end, -- The Moonlit Path
        ...
        [180] = {
                    [288] = function() return (player:getCurrentMission(ZILART) == ARK_ANGELS and player:getVar("ZilartStatus") == 1 and npc:getID() == 17514791 and player:hasKeyItem(dsp.kis.SHARD_OF_APATHY) == false)  end, -- Hume, Ark Angels 1
                    [289] = function() return (player:getCurrentMission(ZILART) == ARK_ANGELS and player:getVar("ZilartStatus") == 1 and npc:getID() == 17514792 and player:hasKeyItem(dsp.kis.SHARD_OF_COWARDICE) == false)  end, -- Tarutaru, Ark Angels 2
                    [290] = function() return (player:getCurrentMission(ZILART) == ARK_ANGELS and player:getVar("ZilartStatus") == 1 and npc:getID() == 17514793 and player:hasKeyItem(dsp.kis.SHARD_OF_ENVY) == false)  end, -- Mithra, Ark Angels 3
                    [291] = function() return (player:getCurrentMission(ZILART) == ARK_ANGELS and player:getVar("ZilartStatus") == 1 and npc:getID() == 17514794 and player:hasKeyItem(dsp.kis.SHARD_OF_ARROGANCE) == false)  end, -- Elvaan, Ark Angels 4
                    [292] = function() return (player:getCurrentMission(ZILART) == ARK_ANGELS and player:getVar("ZilartStatus") == 1 and npc:getID() == 17514795 and player:hasKeyItem(dsp.kis.SHARD_OF_RAGE) == false)  end, -- Galka, Ark Angels 5
                },
        [181] = {
                    [320] = function() return (player:getCurrentMission(ZILART) == THE_CELESTIAL_NEXUS)  end, -- Zilart Mission 16
                },
        [201] = {
                    [416] = function() return (player:hasKeyItem(dsp.kis.TUNING_FORK_OF_WIND))  end, -- Trial by Wind
                    [420] = function() return (player:getCurrentMission(ASA) == SUGAR_COATED_DIRECTIVE and player:hasKeyItem(dsp.kis.DOMINAS_EMERALD_SEAL))  end,
                },
        [202] = {
                    [448] = function() return (player:hasKeyItem(dsp.kis.TUNING_FORK_OF_LIGHTNING))  end, -- Trial by Lightning
                    [452] = function() return (player:getCurrentMission(ASA) == SUGAR_COATED_DIRECTIVE and player:hasKeyItem(dsp.kis.DOMINAS_VIOLET_SEAL))  end,
               },
        [203] = {
                    [480] = function() return (player:hasKeyItem(dsp.kis.TUNING_FORK_OF_ICE))  end, -- Trial by Ice
                    [484] = function() return (player:getCurrentMission(ASA) == SUGAR_COATED_DIRECTIVE and player:hasKeyItem(dsp.kis.DOMINAS_AZURE_SEAL))  end,
                },
        [206] = {
                ...
                --[[
                    ...
                    [532] = function() return (player:getCurrentMission(ACP) >= THOSE_WHO_LURK_IN_SHADOWS_III and player:hasKeyItem(dsp.kis.MARK_OF_SEED))  end, -- ACP Mission 7
                ]]
                },
        [207] = {
                    [544] = function() return (player:hasKeyItem(dsp.kis.TUNING_FORK_OF_FIRE))  end, -- Trial by Fire
                    [547] = function() return (player:getCurrentMission(ASA) == SUGAR_COATED_DIRECTIVE and player:hasKeyItem(dsp.kis.DOMINAS_SCARLET_SEAL))  end,
                },
        [209] = {
                    [576] = function() return (player:hasKeyItem(dsp.kis.TUNING_FORK_OF_EARTH))  end, -- Trial by Earth
                    [580] = function() return (player:getCurrentMission(ASA) == SUGAR_COATED_DIRECTIVE and player:hasKeyItem(dsp.kis.DOMINAS_AMBER_SEAL))  end,
               },
        [211] = {
                    [608] = function() return (player:hasKeyItem(dsp.kis.TUNING_FORK_OF_WATER))  end, -- Trial by Water
                    [611] = function() return (player:getCurrentMission(ASA) == SUGAR_COATED_DIRECTIVE and player:hasKeyItem(dsp.kis.DOMINAS_CERULEAN_SEAL))  end,
                },
    }
...
function CutsceneSkip(player, npc)
    ...
    elseif (Zone == 10) then -- The_Shrouded_Maw
        if ((player:hasCompletedMission(COP, DARKNESS_NAMED)) or (player:getCurrentMission(COP) == DARKNESS_NAMED and player:getVar("PromathiaStatus") > 2)) then -- DARKNESS_NAMED
            skip = 1
        elseif ((player:hasCompleteQuest(WINDURST, WAKING_DREAMS)) or (player:hasKeyItem(dsp.kis.WHISPER_OF_DREAMS))) then -- waking_dreams (diabolos avatar quest)
            skip = 1
        end
    ...
    elseif (Zone == 17) then -- Spire of Holla
        if ((player:hasCompletedMission(COP, THE_MOTHERCRYSTALS)) or (player:hasKeyItem(dsp.kis.LIGHT_OF_HOLLA))) then -- light of holla
            skip = 1
        end
    elseif (Zone == 19) then -- Spire of Dem
        if ((player:hasCompletedMission(COP, THE_MOTHERCRYSTALS)) or (player:hasKeyItem(dsp.kis.LIGHT_OF_DEM))) then -- light of dem
            skip = 1
        end
    elseif (Zone == 21) then -- Spire of Mea
        if ((player:hasCompletedMission(COP, THE_MOTHERCRYSTALS)) or (player:hasKeyItem(dsp.kis.LIGHT_OF_MEA))) then -- light of mea
            skip = 1
        end
    ...
    elseif (Zone == 170) then -- Full Moon Fountain
        if ((player:hasCompleteQuest(WINDURST, THE_MOONLIT_PATH)) or (player:hasKeyItem(dsp.kis.WHISPER_OF_THE_MOON))) then -- The Moonlit Path
            skip = 1
        end
    ...
    elseif (Zone == 201) then -- Cloister of Gales
        if ((player:hasCompleteQuest(OUTLANDS, TRIAL_BY_WIND)) or (player:hasKeyItem(dsp.kis.WHISPER_OF_GALES))) then -- Trial by Wind
            skip = 1
        end
    ...
    elseif (Zone == 203) then -- Cloister of Frost
        if ((player:hasCompleteQuest(SANDORIA, TRIAL_BY_ICE)) or (player:hasKeyItem(dsp.kis.WHISPER_OF_FROST))) then -- Trial by Ice
            skip = 1
        end
    ...
    elseif (Zone == 207) then -- Cloister of Flames
        if ((player:hasCompleteQuest(OUTLANDS, TRIAL_BY_FIRE)) or (player:hasKeyItem(dsp.kis.WHISPER_OF_FLAMES))) then -- Trial by Fire
            skip = 1
        end
    elseif (Zone == 209) then -- Cloister of Tremors
        if ((player:hasCompleteQuest(BASTOK, TRIAL_BY_EARTH)) or (player:hasKeyItem(dsp.kis.WHISPER_OF_TREMORS))) then -- Trial by Earth
            skip = 1
        end
    elseif (Zone == 211) then -- Cloister of Tides
        if ((player:hasCompleteQuest(OUTLANDS, TRIAL_BY_WATER)) or (player:hasKeyItem(dsp.kis.WHISPER_OF_TIDES))) then -- Trial by Water
            skip = 1
        end
    end
    return skip
