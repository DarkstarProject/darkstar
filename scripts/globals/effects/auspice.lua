...
function onEffectGain(target,effect)
    ...
    if (target:hasStatusEffect(EFFECT.AFFLATUS_MISERY)) then
        target:getStatusEffect(EFFECT.AFFLATUS_MISERY):setSubPower(0);
...
function onEffectLose(target,effect)
    ...
    if (target:hasStatusEffect(EFFECT.AFFLATUS_MISERY)) then
        local accuracyBonus = target:getStatusEffect(EFFECT.AFFLATUS_MISERY):getSubPower();
        --printf("AUSPICE: Removing Accuracy Bonus +%d!", accuracyBonus);
        target:delMod(MOD_ACC, accuracyBonus);
        local accuracyBonus = target:getStatusEffect(EFFECT.AFFLATUS_MISERY):setSubPower(0);
