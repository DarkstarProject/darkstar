...
function onEffectTick(target,effect)
    ...
    if not (target:getHPP() < 51 ) then
        if (target:hasStatusEffect(EFFECT.STONESKIN)) then
    ...
    if (complete) then
        target:delStatusEffectSilent(EFFECT.SUBLIMATION_ACTIVATED);
        target:addStatusEffect(EFFECT.SUBLIMATION_COMPLETE,store,0,7200);
    else
