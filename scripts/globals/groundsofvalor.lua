...
function finishGov(player,csid,option,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,msg_offset)
    ...
    local tabs = player:getCurrency("valor_point");
    local HAS_FOOD = player:hasStatusEffect(EFFECT.FOOD);
    local HAS_SUPPORT_FOOD = player:hasStatusEffect(EFFECT.FIELD_SUPPORT_FOOD);
...
-- ================= FIELD SUPPORT ============================================
    ...
    elseif (option == GOV_MENU_CIRCUMSPECTION) then -- Sneak + Invis
        if (tabs >= 5) then
            ...
            player:addStatusEffect(EFFECT.SNEAK,0,10,900);
            player:delStatusEffect(EFFECT.INVISIBLE);
            player:addStatusEffect(EFFECT.INVISIBLE,0,10,900);
        end
    elseif (option == GOV_MENU_HOMING_INSTINCT) then -- Send to Homepoint
        if (tabs >= 50) then
            player:delCurrency("valor_point", 50);
            player:addStatusEffectEx(EFFECT.TELEPORT,0,TELEPORT_WARP,0,1);
        end
    elseif (option == GOV_MENU_RERAISE) then -- Reraise 1
        if (tabs >= 10) then
            player:delCurrency("valor_point", 10);
            player:delStatusEffect(EFFECT.RERAISE);
            player:addStatusEffect(EFFECT.RERAISE,1,0,7200);
        end
    elseif (option == GOV_MENU_RERAISE_II) then -- Reraise 2
        if (tabs >= 20) then
            player:delCurrency("valor_point", 20);
            player:delStatusEffect(EFFECT.RERAISE);
            player:addStatusEffect(EFFECT.RERAISE,2,0,7200);
        end
    elseif (option == GOV_MENU_RERAISE_III) then -- ReRaise 3
        if (tabs >= 30) then
            player:delCurrency("valor_point", 30);
            player:delStatusEffect(EFFECT.RERAISE);
            player:addStatusEffect(EFFECT.RERAISE,3,0,7200);
        end
    elseif (option == GOV_MENU_REGEN) then -- Overwrites all other forms of regen.
        if (tabs >= 20) then
            ...
            -- Adds regen
            player:addStatusEffect(EFFECT.REGEN,1,3,3600);
        end
    elseif (option == GOV_MENU_REFRESH) then -- Overwrites all other forms of refresh.
        if (tabs >= 20) then
            ...
            player:delStatusEffect(EFFECT.SUBLIMATION_COMPLETE);
            player:delStatusEffect(EFFECT.SUBLIMATION_ACTIVATED);
            -- Add refresh
            player:addStatusEffect(EFFECT.REFRESH,1,3,3600, 0, 3);
        end
    elseif (option == GOV_MENU_PROTECT) then -- Overwrites all other forms of protect.
        if (tabs >= 15) then
            ...
            player:delStatusEffect(EFFECT.PROTECT);
    ...
    elseif (option == GOV_MENU_SHELL) then -- Overwrites all other forms of shell.
        if (tabs >= 15) then
            ...
            -- Add shell
            player:addStatusEffect(EFFECT.SHELL,def,0,1800);
        end
    elseif (option == GOV_MENU_HASTE) then -- Wiki info on haste amount is iffy at best. People guessed at 15 and, I went with 10.
        if (tabs >= 20) then
            player:delCurrency("valor_point", 20);
            player:delStatusEffect(EFFECT.HASTE);
            player:addStatusEffect(EFFECT.HASTE,10,0,600);
        end
    elseif (option == GOV_MENU_DRIED_MEAT) then -- Dried Meat: STR+4, Attack +22% (caps at 63)
        if (tabs >= 50) then
            if (HAS_FOOD == true or HAS_SUPPORT_FOOD == true) then
                ...
                player:delCurrency("valor_point", 50);
                player:addStatusEffectEx(EFFECT.FIELD_SUPPORT_FOOD, 251, 1, 0, 1800);
            end
        end
    elseif (option == GOV_MENU_SALTED_FISH) then -- Salted Fish: VIT+2 DEF+30% (Caps at 86)
        if (tabs >= 50) then
            if (HAS_FOOD == true or HAS_SUPPORT_FOOD == true) then
                ...
                player:delCurrency("valor_point", 50);
                player:addStatusEffectEx(EFFECT.FIELD_SUPPORT_FOOD, 251, 2, 0, 1800);
            end
        end
    elseif (option == GOV_MENU_HARD_COOKIE) then -- - Hard Cookie: INT+4, MaxMP+30
        if (tabs >= 50) then
            if (HAS_FOOD == true or HAS_SUPPORT_FOOD == true) then
                ...
                player:delCurrency("valor_point", 50);
                player:addStatusEffectEx(EFFECT.FIELD_SUPPORT_FOOD, 251, 3, 0, 1800);
            end
        end
    elseif (option == GOV_MENU_INSTANT_NOODLES) then -- Instant Noodles: VIT+1, Max HP+27% (caps at 75), StoreTP+5
        if (tabs >= 50) then
            if (HAS_FOOD == true or HAS_SUPPORT_FOOD == true) then
                ...
                player:delCurrency("valor_point", 50);
                player:addStatusEffectEx(EFFECT.FIELD_SUPPORT_FOOD, 251, 4, 0, 1800);
            end
        end
    elseif (option == GOV_MENU_DRIED_AGARICUS) then -- Dried Agaricus: MND+4
        if (tabs >= 50) then
            if (HAS_FOOD == true or HAS_SUPPORT_FOOD == true) then
                ...
                player:delCurrency("valor_point", 50);
                player:addStatusEffectEx(EFFECT.FIELD_SUPPORT_FOOD, 251, 5, 0, 1800);
            end
        end
    elseif (option == GOV_MENU_INSTANT_RICE) then -- Instant Rice: CHR+6
        if (tabs >= 50) then
            if (HAS_FOOD == true or HAS_SUPPORT_FOOD == true) then
                ...
                player:delCurrency("valor_point", 50);
                player:addStatusEffectEx(EFFECT.FIELD_SUPPORT_FOOD, 251, 6, 0, 1800);
            end
        end
            ...
            if (killed < needed) then -- Increment killed number and save.
                ...
                if (killed == needed) then
                    ...
                    if (k1 == fov_info[1] and k2 == fov_info[2] and k3 == fov_info[3] and k4 == fov_info[4]) then
                        ...
                        if (player:hasStatusEffect(EFFECT.PROWESS) == true) then
                            GoV_clears = player:getStatusEffect(EFFECT.PROWESS):getPower();
                            reward = reward * ((100+(GoV_clears*4))/100);
                            player:delStatusEffect(EFFECT.PROWESS);
                            player:delStatusEffect(RandomProwess);
                            player:addStatusEffect(EFFECT.PROWESS,(GoV_clears + 1),0,0);
                            player:addStatusEffectEx(RandomProwess,0,power,0,0);
                            ProwessMessage = (RandomProwess - 152);
                        else
                            player:addStatusEffect(EFFECT.PROWESS,1,0,0);
                            player:addStatusEffectEx(RandomProwess,0,power,0,0);
