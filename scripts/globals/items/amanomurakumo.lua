...
function onAdditionalEffect(player,target,damage)
    ...
    if (math.random(0,99) >= chance or applyResistanceAddEffect(player,target,ELE_WATER,0) <= 0.5) then
        ...
        target:delStatusEffect(EFFECT.ATTACK_BOOST);
        target:addStatusEffect(EFFECT.ATTACK_DOWN, 10, 0, 60); -- Power needs verification/correction
        return SUBEFFECT_ATTACK_DOWN, msgBasic.ADD_EFFECT_STATUS, EFFECT.ATTACK_DOWN;
    end
end;