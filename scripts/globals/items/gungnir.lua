...
function onAdditionalEffect(player,target,damage)
    ...
    if (math.random(0,99) >= chance or applyResistanceAddEffect(player,target,ELE_WIND,0) <= 0.5) then
        ...
        target:delStatusEffect(EFFECT.DEFENSE_BOOST)
        target:addStatusEffect(EFFECT.DEFENSE_DOWN, 17, 0, 60); -- Power and duration needs verification
        return SUBEFFECT_DEFENSE_DOWN, msgBasic.ADD_EFFECT_STATUS, EFFECT.DEFENSE_DOWN;
    end
end;