...
function onAdditionalEffect(player,target,damage)
	...
	if (player:getEquipID(SLOT_AMMO) == 18232) and (player:getEquipID(SLOT_MAIN) == 18201) then
		params.bonusmab = 0;
	...
	if (player:getEquipID(SLOT_AMMO) == 18234) and (player:getEquipID(SLOT_MAIN) == 18201) then -- mega fan
		dmg = (dmg + 6)		
		params.bonusmab = 0;
		params.includemab = false;
		dmg = addBonusesAbility(player, ELE_WATER, target, dmg, params);
	...
	if (player:getEquipID(SLOT_AMMO) == 18233) and (player:getEquipID(SLOT_MAIN) == 18201) then	-- kilo fan
		dmg = (dmg + 12)		
		params.bonusmab = 0;
		params.includemab = false;
		dmg = addBonusesAbility(player, ELE_WATER, target, dmg, params);
