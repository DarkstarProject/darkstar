...
function onAdditionalEffect(player,target,damage)
	...
	if (player:getEquipID(SLOT_AMMO) == 18228) and (player:getEquipID(SLOT_MAIN) == 18077) then -- BATTERY LVL 10
		params.bonusmab = 0;
	...
	if (player:getEquipID(SLOT_AMMO) == 18229) and (player:getEquipID(SLOT_MAIN) == 18077) then -- KILO BATTERY LVL 30
		dmg = (dmg + 6)		
		params.bonusmab = 0;
		params.includemab = false;
		dmg = addBonusesAbility(player, ELE_LIGHTNING, target, dmg, params);
	...
	if (player:getEquipID(SLOT_AMMO) == 18230) and (player:getEquipID(SLOT_MAIN) == 18077) then	-- MEGA BATTERY LVL 50
		dmg = (dmg + 12)		
		params.bonusmab = 0;
		params.includemab = false;
		dmg = addBonusesAbility(player, ELE_LIGHTNING, target, dmg, params);
