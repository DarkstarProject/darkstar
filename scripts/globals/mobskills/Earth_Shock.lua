...
function OnMobWeaponSkill(target, mob, skill)
	...
	local dmg = MobFinalAdjustments(info.dmg,mob,skill,target,MOBSKILL_PHYSICAL,MOBPARAM_NONE,info.hitslanded*math.random(2,3));
	...
	if(target:hasStatusEffect(typeEffect) == false and MobPhysicalHit(skill, dmg, target, info.hitslanded)) then
		...
		if(resist > 0.2) then
			target:addStatusEffect(typeEffect,1,0,6*resist);--power=1;tic=0;duration=7;
		end
	end
