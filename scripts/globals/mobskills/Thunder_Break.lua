...
function OnMobWeaponSkill(target, mob, skill)
	...
	if(target:hasStatusEffect(typeEffect) == false) then
		...
		if(resist > 0.2) then
			target:addStatusEffect(typeEffect,1,0,math.random(4,8)*resist);--power=1;tic=0;duration=5;
		end
	end

	local dmgmod = 2+math.random();
	local info = MobMagicalMove(mob,target,skill,mob:getWeaponDmg()*2,ELE_THUNDER,dmgmod,TP_MAB_BONUS,1);
