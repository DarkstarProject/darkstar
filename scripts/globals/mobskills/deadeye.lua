...
function onMobWeaponSkill(target, mob, skill)
    ...
    defDown = MobStatusEffectMove(mob, target, EFFECT.DEFENSE_DOWN, 50, 0, 120);
    mDefDown = MobStatusEffectMove(mob, target, EFFECT.MAGIC_DEF_DOWN, 50, 0, 120);
    ...
    if (defDown == msgBasic.SKILL_ENFEEB_IS) then
        typeEffect = EFFECT.DEFENSE_DOWN;
    elseif (mDefDown == msgBasic.NFEEB_IS) then
        typeEffect = EFFECT.MAGIC_DEF_DOWN;
    else
