    ...
    if wyvernType == WYVERN_DEFENSIVE then
        master:addListener("WEAPONSKILL_USE", "PET_WYVERN_WS", function(player, target, skillid)
            ...
            for _,member in ipairs(party) do
                if member:hasStatusEffect(EFFECT.POISON) then
                    player:getPet():useJobAbility(627, member)
                    break
                elseif member:hasStatusEffect(EFFECT.BLINDNESS) and player:getPet():getMainLvl() > 20 then
                    player:getPet():useJobAbility(628, member)
                    break
                elseif member:hasStatusEffect(EFFECT.PARALYSIS) and player:getPet():getMainLvl() > 40 then
                    player:getPet():useJobAbility(629, member)
                    break
                elseif (member:hasStatusEffect(EFFECT.CURSE_I) or member:hasStatusEffect(EFFECT.DOOM)) and player:getPet():getMainLvl() > 60 then
                    player:getPet():useJobAbility(637, member)
                    break
                elseif (member:hasStatusEffect(EFFECT.DISEASE) or member:hasStatusEffect(EFFECT.PLAGUE)) and player:getPet():getMainLvl() > 80 then
