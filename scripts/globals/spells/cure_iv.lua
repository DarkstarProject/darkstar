...
function onSpellCast(caster,target,spell)
    ...
    if (target:getAllegiance() == caster:getAllegiance() and (target:getObjType() == TYPE_PC or target:getObjType() == TYPE_MOB)) then
        ...
        if (caster:hasStatusEffect(EFFECT.AFFLATUS_SOLACE) and target:hasStatusEffect(EFFECT.STONESKIN) == false) then
            ...
            target:addStatusEffect(EFFECT.STONESKIN,solaceStoneskin,0,25);
        end;
