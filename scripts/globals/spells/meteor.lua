...
function onSpellCast(caster,target,spell)
    ...
    local dmg = 0;
    if (caster:isPC()) then
        dmg = ((100+caster:getMod(dsp.mod.MATT))/(100+target:getMod(dsp.mod.MDEF))) * (caster:getStat(dsp.mod.INT) + caster:getSkillLevel(ELEMENTAL_MAGIC_SKILL)/6) * 3.5;
    else
        dmg = ((100+caster:getMod(dsp.mod.MATT))/(100+target:getMod(dsp.mod.MDEF))) * (caster:getStat(dsp.mod.INT) + (caster:getMaxSkillLevel(caster:getMainLvl(), dsp.job.BLM, ELEMENTAL_MAGIC_SKILL))/6) * 9.4;
    end
