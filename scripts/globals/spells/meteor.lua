...
function onSpellCast(caster,target,spell)
    ...
    if (caster:isPC()) then
        ...
        dmg = ((100+caster:getMod(MOD_MATT))/(100+target:getMod(MOD_MDEF))) * (caster:getStat(MOD_INT) + (caster:getMaxSkillLevel(caster:getMainLvl(), dsp.job.BLM, ELEMENTAL_MAGIC_SKILL))/6) * 9.4;
    end
