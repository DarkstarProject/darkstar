...
function onSpellCast(caster,target,spell)
    ...
    if (target:getStatusEffect(EFFECT.CHOKE) ~= nil) then
        ...
        if (resist <= 0.125) then
            ...
            if (target:getStatusEffect(EFFECT.SHOCK) ~= nil) then
                target:delStatusEffect(EFFECT.SHOCK);
            end;
            local sINT = caster:getStat(MOD_INT);
            local DOT = getElementalDebuffDOT(sINT);
            local effect = target:getStatusEffect(EFFECT.RASP);
            ...
            if (noeffect) then
                ...
                target:addStatusEffect(EFFECT.RASP,DOT, 3, ELEMENTAL_DEBUFF_DURATION,FLAG_ERASABLE);
            end;
        end;
    end;
    return EFFECT.RASP;
