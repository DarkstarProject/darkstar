...
function onMagicCastingCheck(caster,target,spell)
    ...
    elseif (not caster:hasTrust(dsp.trust.CURILLA)) then
		local msg = caster:checkTrust(dsp.trust.CURILLA);
		if(msg > 0) then
			return msg
		end
    else
...
function onSpellCast(caster,target,spell)
    ...
    caster:spawnTrust(dsp.trust.CURILLA) -- see pet_list.sql, this should be 896 but the pet list won't let me do that
    return 0
end