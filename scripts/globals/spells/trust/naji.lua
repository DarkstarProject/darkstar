...
function onMagicCastingCheck(caster,target,spell)
    ...
    elseif (not caster:hasTrust(dsp.trust.NAJI)) then
		local msg = caster:checkTrust(dsp.trust.NAJI);
		if(msg > 0) then
			return msg
		end
    else
...
function onSpellCast(caster,target,spell)
    ...
    caster:spawnTrust(dsp.trust.NAJI) -- see pet_list.sql, this should be 896 but the pet list won't let me do that
    return 0
end
