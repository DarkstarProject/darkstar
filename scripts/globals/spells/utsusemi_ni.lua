...
function onSpellCast(caster,target,spell)
    ...
    if (effect == nil) then
        if caster:getMainJob() == JOBS.NIN then
            target:addStatusEffectEx(EFFECT.COPY_IMAGE,EFFECT.COPY_IMAGE_4, 4 + bonusShadow,0,900);
            target:setMod(MOD_UTSUSEMI, 4 + bonusShadow);
            spell:setMsg(msgBasic.MAGIC_GAIN_EFFECT);
        else
            target:addStatusEffectEx(EFFECT.COPY_IMAGE,EFFECT.COPY_IMAGE_3, 3 + bonusShadow,0,900);
