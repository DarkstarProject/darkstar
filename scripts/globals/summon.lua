...
function AvatarFinalAdjustments(dmg,mob,skill,target,skilltype,skillparam,shadowbehav)
        ...
        -- Blink has a VERY high chance of blocking tp moves, so im assuming its 100% because its easier!
            if (targShadows >= shadowbehav) then --no damage, just suck the shadows
                ...
                if (shadowType == MOD_UTSUSEMI) then --update icon
                    effect = target:getStatusEffect(EFFECT.COPY_IMAGE);
                    if (effect ~= nil) then
    ...
    teye = target:getStatusEffect(EFFECT.THIRD_EYE);
    ...
    if (skilltype == MOBSKILL_PHYSICAL and target:hasStatusEffect(EFFECT.PHYSICAL_SHIELD)) then
        return 0;
    end

    if (skilltype == MOBSKILL_RANGED and target:hasStatusEffect(EFFECT.ARROW_SHIELD)) then
        return 0;
    end

    -- handle elemental resistence
    if (skilltype == MOBSKILL_MAGICAL and target:hasStatusEffect(EFFECT.MAGIC_SHIELD)) then
        return 0;
    end
    ...
    --handle invincible
    if (target:hasStatusEffect(EFFECT.INVINCIBLE) and skilltype==MOBSKILL_PHYSICAL) then
        return 0;
    end
    -- handle pd
...
function avatarMiniFightCheck(caster)
    local result = 0;
    local bcnmid;
    if (caster:hasStatusEffect(EFFECT.BATTLEFIELD) == true) then
        bcnmid = caster:getStatusEffect(EFFECT.BATTLEFIELD):getPower();
