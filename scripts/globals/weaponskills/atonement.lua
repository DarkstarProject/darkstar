...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
    ...
    if target:getObjType() ~= dsp.objType.MOB then -- this isn't correct but might as well use what was originally here if someone uses this on a non-mob
        ...
        damage = takeWeaponskillDamage(target, player, wsParams, primary, damage, dsp.attackType.BREATH, dsp.damageType.NONE, dsp.slot.MAIN, tpHits, extraHits, 0, 0, action, nil)
    end
