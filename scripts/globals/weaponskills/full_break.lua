...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
    ...
    if (damage > 0) then
        ...
        if (target:hasStatusEffect(dsp.effect.DEFENSE_DOWN) == false) then
            target:addStatusEffect(dsp.effect.DEFENSE_DOWN, 12.5, 0, duration * applyResistanceAddEffect(player,target,ELE_WIND,0));
        end
        if (target:hasStatusEffect(dsp.effect.ATTACK_DOWN) == false) then
            target:addStatusEffect(dsp.effect.ATTACK_DOWN, 12.5, 0, duration * applyResistanceAddEffect(player,target,ELE_WATER,0));
        end
        if (target:hasStatusEffect(dsp.effect.EVASION_DOWN) == false) then
            target:addStatusEffect(dsp.effect.EVASION_DOWN, 20, 0, duration * applyResistanceAddEffect(player,target,ELE_ICE,0));
        end
        if (target:hasStatusEffect(dsp.effect.ACCURACY_DOWN) == false) then
            target:addStatusEffect(dsp.effect.ACCURACY_DOWN, 20, 0, duration * applyResistanceAddEffect(player,target,ELE_EARTH,0));
        end
    end
