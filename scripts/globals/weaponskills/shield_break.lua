...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
    ...
    if (damage > 0 and target:hasStatusEffect(EFFECT.EVASION_DOWN) == false) then
        local duration = (120 + (tp/1000 * 60)) * applyResistanceAddEffect(player,target,ELE_ICE,0);
        target:addStatusEffect(EFFECT.EVASION_DOWN, 40, 0, duration);
    end
