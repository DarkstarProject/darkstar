...
-- Level: 5
-- Mods : STR:20%, VIT:20%
-- Desc : Lowers enemies Evasion. Duration of effect varies with TP. (max40sec)
-------------------------------
...
function OnUseWeaponSkill(attacker, target, wsID)
	...
	-- Give effect of evasion down (20sec-40sec based on TP) if doesn't already have effect.
	if(target:hasStatusEffect(EFFECT_EVASION_DOWN) == false) then
		...
		target:addStatusEffect(EFFECT_EVASION_DOWN,0,10,math.floor(duration), FLAG_DISPELABLE);
	end
