...
function OnUseWeaponSkill(player, target, wsID)	
	...
	str_wsc = 0.0; dex_wsc = 0.0; vit_wsc = 0.3; agi_wsc = 0.0; int_wsc = 0.0; mnd_wsc = 0.0; chr_wsc = 0.0;
	...
	if damage > 0 then
		tp = player:getTP();
		duration = (tp/100);
		if(target:hasStatusEffect(10) == false) then
			target:addStatusEffect(10, 1, 0, duration);
		end
	end	
