...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
    ...
    if (damage > 0 and target:hasStatusEffect(EFFECT.ATTACK_DOWN) == false) then
        local duration = (120 + (tp/1000 * 60)) * applyResistanceAddEffect(player,target,ELE_WATER,0);
        target:addStatusEffect(EFFECT.ATTACK_DOWN, 25, 0, duration);
    end
