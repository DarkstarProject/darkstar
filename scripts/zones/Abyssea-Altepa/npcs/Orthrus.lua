...
function onTrigger(player,npc)
    if (GetMobAction(17670552) == ACTION_NONE) then
        ...
        if (player:hasKeyItem(STEAMING_CERBERUS_TONGUE)) then
            ...
            player:startEvent(1020, STEAMING_CERBERUS_TONGUE);
        else
            -- Do not ask, because player is missing at least 1.
            player:startEvent(1021, STEAMING_CERBERUS_TONGUE);
        end
    end
end;
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 1020 and option == 1) then
        ...
        SpawnMob(17670552, 300):updateEnmity(player);
