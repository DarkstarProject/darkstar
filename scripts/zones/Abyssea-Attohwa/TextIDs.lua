-- Variable TextID   Description text

-- General Texts
ITEM_CANNOT_BE_OBTAINED = 6380; -- You cannot obtain the #. Try trading again after sorting your inventory.
          ITEM_OBTAINED = 6381; -- Obtained: <item>
           GIL_OBTAINED = 6382; -- Obtained <number> gil
       KEYITEM_OBTAINED = 6384; -- Obtained key item: <keyitem>
         CRUOR_OBTAINED = 7370; -- <Possible Special Code: 1F>y<Player Name> obtains <Numeric Parameter 0> cruor.
            CRUOR_TOTAL = 6978; -- Obtained <Numeric Parameter 0> cruor. (Total: <Numeric Parameter 1>)<Prompt>
