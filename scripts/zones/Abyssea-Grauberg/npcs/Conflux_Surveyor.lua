...
function onTrigger(player,npc)
    ...
    if (player:hasStatusEffect(dsp.effects.VISITANT)) then
        visitant = 60;
    end
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 2002) then
        if (VISITANT_SYSTEM == false and (option == 65538 or option == 131074 or option == 196610 or option == 262146)) then
            player:addStatusEffect(dsp.effects.VISITANT,0,0,0,0,0); -- using 0 should cause an infinate duration.
        elseif (option == 2) then -- Use no stones, use previous remaining time
            player:addStatusEffect(dsp.effects.VISITANT,0,3,duration,0,0);
