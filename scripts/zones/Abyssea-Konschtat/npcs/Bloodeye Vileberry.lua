...
function onTrigger(player,npc)
    ...
    if (GetMobAction(16839068) == ACTION_NONE) then
		...
		-- Ask if player wants to use KIs
            ...
            player:startEvent(1020, TWISTED_TONBERRY_CROWN);

        else

        -- Do not ask, because player is missing at least 1.

            player:startEvent(1021, TWISTED_TONBERRY_CROWN);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 1020 and option == 1) then
        ...
        SpawnMob(16839068, 300):updateEnmity(player);
