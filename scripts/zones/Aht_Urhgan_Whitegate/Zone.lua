...
function onRegionEnter(player,region)
    switch (region:GetRegionID()): caseof
    {
        ...
        [5] = function (x) -- AH mission
            ...
            elseif (player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.NAVIGATING_THE_UNFRIENDLY_SEAS) == QUEST_COMPLETED and player:getQuestStatus(AHT_URHGAN,AGAINST_ALL_ODDS) == QUEST_AVAILABLE and player:getMainJob() == dsp.job.COR and player:getMainLvl() >= AF3_QUEST_LEVEL) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 797) then
        ...
        player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.AGAINST_ALL_ODDS); -- Start of af 3 not completed yet
