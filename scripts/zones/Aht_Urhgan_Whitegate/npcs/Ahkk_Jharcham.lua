...
function onTrade(player,npc,trade)
    local keepingNotes = player:getQuestStatus(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.KEEPING_NOTES)
...
function onEventFinish(player,csid,option)
    if csid == 9 then
        player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.KEEPING_NOTES)
    elseif csid == 11 then
        player:confirmTrade()
        player:moghouseFlag(16)
        player:completeQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.KEEPING_NOTES)
    end
end