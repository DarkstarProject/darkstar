...
function onTrigger(player,npc)
    local olduumQuest = player:getQuestStatus(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.OLDUUM)
...
function onEventFinish(player,csid,option)
    ...
    if csid == 4 then
        ...
        player:addQuest(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.OLDUUM)
    elseif csid == 6 or csid == 8 then
        if player:getFreeSlotsCount() >= 1 then
            ...
            if csid == 6 then
                player:completeQuest(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.OLDUUM)
            end
        else
