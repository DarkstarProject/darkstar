...
function onEventFinish(player,csid,option)
    ...
    if (csid == 0x004) then
        ...
        player:messageSpecial(text.KEYITEM_OBTAINED, DKHAAYAS_RESEARCH_JOURNAL);
        player:addQuest(AHT_URHGAN, OLDUUM);
    elseif (csid == 0x006 or csid == 0x008) then
        if (player:getFreeSlotsCount() >= 1) then
            player:addItem(2217);
            player:messageSpecial(text.ITEM_OBTAINED, 2217);
            ...
            player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED, 2217);
        end
    end
end;
