...
function onTrade(player,npc,trade)
    ...
    if (guildMember == 1) then
        if (trade:hasItemQty(2184,1) and trade:getItemCount() == 1) then
            if (player:hasStatusEffect(EFFECT.FISHING_IMAGERY) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 642 and option == 1) then
        ...
        player:addStatusEffect(EFFECT.FISHING_IMAGERY,1,0,3600);
    elseif (csid == 643) then
        player:messageSpecial(IMAGE_SUPPORT,0,0,0);
        player:addStatusEffect(EFFECT.FISHING_IMAGERY,2,0,7200);
    end
end;