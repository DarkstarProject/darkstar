...
function onTrigger(player,npc)
    ...
    local threeMenQuest = player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.THREE_MEN_AND_A_CLOSET)
    if player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.GOT_IT_ALL) == QUEST_COMPLETED and threeMenQuest == QUEST_AVAILABLE then
...
function onEventFinish(player,csid,option)
    if csid == 836 then
        player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.THREE_MEN_AND_A_CLOSET)
        player:setVar("threemenandaclosetCS",2)
    elseif csid == 838 then
        player:setVar("threemenandaclosetCS",4)
    elseif csid == 845 then
        npcUtil.completeQuest(player, AHT_URHGAN, dsp.quests.ids.ahtUrhgan.THREE_MEN_AND_A_CLOSET, { item=2184, var="threemenandaclosetCS"})
    end
end