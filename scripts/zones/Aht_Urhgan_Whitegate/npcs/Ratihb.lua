...
function onTrigger(player,npc)
    ...
    local EquippedforAllOccasions = player:getQuestStatus(AHT_URHGAN,EQUIPPED_FOR_ALL_OCCASIONS)
    ...
    elseif player:getQuestStatus(AHT_URHGAN,LUCK_OF_THE_DRAW) == QUEST_COMPLETED and player:getQuestStatus(AHT_URHGAN,EQUIPPED_FOR_ALL_OCCASIONS == QUEST_COMPLETED) then
...
function onEventFinish(player,csid,option)
    ...
    if csid == 772 then
        npcUtil.completeQuest(player, AHT_URHGAN, EQUIPPED_FOR_ALL_OCCASIONS, { item=18702, var="EquippedforAllOccasions"})
