...
function onTrigger(player,npc)
    local gotitall = player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.GOT_IT_ALL)
...
function onEventFinish(player,csid,option)
    if csid == 520 then
        player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.GOT_IT_ALL)
    ...
    elseif csid == 528 then
        npcUtil.completeQuest(player, AHT_URHGAN, dsp.quests.ids.ahtUrhgan.GOT_IT_ALL, {item=18257, var={"Wait1DayForgotitallCS_date", "gotitallCS"}})
