...
function onEventFinish(player,csid,option,npc)
    ...
    if csid ==150 then
        if CoinType == 1 then -- Bronze Pieces
            ...
            elseif Quantity <= 99 then
                if player:getFreeSlotsCount() == 0 then
                    ...
                    player:delCurrency("imperial_standing", 20*Quantity)
                    npc:showText(npc, ID.text.UGRIHD_PURCHASE_DIALOGUE)
                    player:messageSpecial(ID.text.ITEM_OBTAINEDX,2184,Quantity)
        ...
        elseif CoinType == 2 then -- Silver Pieces
            ...
            elseif Quantity <= 99 then
                if player:getFreeSlotsCount() == 0 then
                    ...
                    player:delCurrency("imperial_standing", 100*Quantity)
                    npc:showText(npc, ID.text.UGRIHD_PURCHASE_DIALOGUE)
                    player:messageSpecial(ID.text.ITEM_OBTAINEDX,2185,Quantity)
        ...
        elseif CoinType == 3 then -- Mythril Pieces
            if Quantity == 1 then
                if player:getFreeSlotsCount() == 0 then
                    player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED,2186)
                else
                    player:delCurrency("imperial_standing", 200*Quantity)
                    npc:showText(npc, ID.text.UGRIHD_PURCHASE_DIALOGUE)
                    player:messageSpecial(ID.text.ITEM_OBTAINED,2186)
                    player:addItem(2186)
                end
            elseif Quantity <= 99 then
                if player:getFreeSlotsCount() == 0 then
                    ...
                    player:delCurrency("imperial_standing", 200*Quantity)
                    npc:showText(npc, ID.text.UGRIHD_PURCHASE_DIALOGUE)
                    player:messageSpecial(ID.text.ITEM_OBTAINEDX,2186,Quantity)
        ...
        elseif CoinType == 4 then -- Gold Pieces
            if Quantity == 1 then
                if player:getFreeSlotsCount() == 0 then
                    player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED,2187)
                else
            ...
            elseif Quantity <= 99 then
                if player:getFreeSlotsCount() == 0 then
                    ...
                    player:delCurrency("imperial_standing", 1000*Quantity)
                    npc:showText(npc, ID.text.UGRIHD_PURCHASE_DIALOGUE)
                    player:messageSpecial(ID.text.ITEM_OBTAINEDX,2187,Quantity)
