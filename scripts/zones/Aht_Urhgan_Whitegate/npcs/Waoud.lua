...
function onTrade(player,npc,trade)
    local anEmptyVessel = player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.AN_EMPTY_VESSEL)
...
function onEventFinish(player,csid,option)
    ...
    if csid == 60 then
        ...
        elseif option == 50 then
            ...
            player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.AN_EMPTY_VESSEL)
        else
            player:setVar("LastDivinationDay",vanaDay())
            player:setVar("AnEmptyVesselProgress",1)
            player:delGil(1000)
    ...
    elseif csid == 705 and option == 1 then
        player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.BEGINNINGS)
    elseif csid == 706 and option == 1 then
        player:delGil(1000)
        player:messageSpecial(ID.text.PAY_DIVINATION) -- You pay 1000 gil for the divination.
    elseif csid == 707 then
        npcUtil.completeQuest(player, AHT_URHGAN, dsp.quests.ids.ahtUrhgan.BEGINNINGS, {item=17717})
    end
end