...
-- function onUseAbility(mob,target,ability)
    ...
    -- else
        -- if (mob:getID() == ID.mob.JAILER_OF_PRUDENCE_1 and GetMobAction(ID.mob.JAILER_OF_PRUDENCE_2) > 0 and GetMobAction(ID.mob.JAILER_OF_PRUDENCE_2) ~= dsp.act.SLEEP and GetMobAction(ID.mob.JAILER_OF_PRUDENCE_2) ~= dsp.act.STUN) then
            -- if (GetMobByID(PrudenceTwo):checkDistance(mob) <= 10) then
                -- GetMobByID(PrudenceTwo):useMobAbility(ability:getID());
            -- end
        -- elseif (mob:getID() == ID.mob.JAILER_OF_PRUDENCE_2 and GetMobAction(ID.mob.JAILER_OF_PRUDENCE_1) > 0 and GetMobAction(ID.mob.JAILER_OF_PRUDENCE_1) ~= dsp.act.SLEEP and GetMobAction(PrudenJAILER_OF_PRUDENCE_1eOne) ~= dsp.act.STUN) then
