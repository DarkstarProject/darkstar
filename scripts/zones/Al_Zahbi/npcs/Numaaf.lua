...
function onTrade(player,npc,trade)
    ...
    if (guildMember == 1) then
        if (trade:hasItemQty(2184,1) and trade:getItemCount() == 1) then
            if (player:hasStatusEffect(EFFECT.COOKING_IMAGERY) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 222 and option == 1) then
        ...
        player:addStatusEffect(EFFECT.COOKING_IMAGERY,1,0,120);
    elseif (csid == 223) then
        player:messageSpecial(IMAGE_SUPPORT,0,8,0);
        player:addStatusEffect(EFFECT.COOKING_IMAGERY,3,0,480);
    end
end;