...
function onTrade(player,npc,trade)
    ...
    if (guildMember == 1) then
        if (trade:hasItemQty(2184,1) and trade:getItemCount() == 1) then
            if (player:hasStatusEffect(EFFECT.GOLDSMITHING_IMAGERY) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 230 and option == 1) then
        ...
        player:addStatusEffect(EFFECT.GOLDSMITHING_IMAGERY,1,0,120);
    elseif (csid == 231) then
        player:messageSpecial(IMAGE_SUPPORT,0,3,0);
        player:addStatusEffect(EFFECT.GOLDSMITHING_IMAGERY,3,0,480);
    end
end;