...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(TOAU) == PATH_OF_DARKNESS and player:hasKeyItem(dsp.kis.NYZUL_ISLE_ROUTE) and player:getVar("AhtUrganStatus") == 1) then
        player:setVar("PathOfDarkness",1);
        player:startEvent(405, 58, -6, 0, 99, 5, 0);
    elseif (player:getCurrentMission(TOAU) == NASHMEIRAS_PLEA and player:hasKeyItem(dsp.kis.MYTHRIL_MIRROR) and player:getVar("AhtUrganStatus") == 1) then
        player:setVar("NashmeirasPlea",1);
        player:startEvent(405, 59, -10, 0, 99, 5, 0);
    elseif (player:hasKeyItem(dsp.kis.NYZUL_ISLE_ASSAULT_ORDERS)) then
...
function onEventUpdate(player,csid,option,target)
        ...
        if (party ~= nil) then
            for i,v in ipairs(party) do
                if (v:getID() ~= player:getID()) then
                    if (v:hasKeyItem(dsp.kis.NYZUL_ISLE_ROUTE) == false and v:hasCompletedMission(TOAU, PATH_OF_DARKNESS) == false) then
    ...
    elseif(nashmeirasPlea == 1) then
        ...
        if (party ~= nil) then
            for i,v in ipairs(party) do
                if (v:getID() ~= player:getID()) then
                    if (v:hasKeyItem(dsp.kis.MYTHRIL_MIRROR) == false and v:hasCompletedMission(TOAU, NASHMEIRAS_PLEA) == false) then
                        player:messageText(target,MEMBER_NO_REQS, false);
                        player:instanceEntry(target,1);
                        return;
                    elseif (v:getZoneID() == player:getZoneID() and v:checkDistance(player) > 50) then
        ...
        if (party ~= nil) then
            for i,v in ipairs(party) do
                if (v:getID() ~= player:getID()) then
                    if (not (v:hasKeyItem(dsp.kis.NYZUL_ISLE_ASSAULT_ORDERS) and v:getCurrentAssault() == assaultid)) then
                        print("NO REQS");
                        player:messageText(target,MEMBER_NO_REQS, false);
                        player:instanceEntry(target,1);
                        return;
...
function onInstanceCreated(player,target,instance)
    ...
    if (instance) then
        if (pathOfDarkness == 1) then
            ...
            player:delKeyItem(dsp.kis.NYZUL_ISLE_ROUTE);
        elseif (nashmeirasPlea == 1) then
            player:setVar("NashmeirasPlea", 0);
            player:delKeyItem(dsp.kis.MYTHRIL_MIRROR);
        else
            instance:setLevelCap(player:getVar("AssaultCap"));
            player:setVar("AssaultCap", 0);
            player:delKeyItem(dsp.kis.NYZUL_ISLE_ASSAULT_ORDERS);
        ...
        if (party ~= nil) then
            for i,v in ipairs(party) do
                if v:getID() ~= player:getID() and v:getZoneID() == player:getZoneID() then
                    ...
                    if (pathOfDarkness == 1) then
                        v:delKeyItem(dsp.kis.NYZUL_ISLE_ROUTE);
                    elseif (nashmeirasPlea == 1) then
                        v:delKeyItem(dsp.kis.MYTHRIL_MIRROR);
                    else
                        v:delKeyItem(dsp.kis.NYZUL_ISLE_ASSAULT_ORDERS);
                    end
                end
            end
        end
