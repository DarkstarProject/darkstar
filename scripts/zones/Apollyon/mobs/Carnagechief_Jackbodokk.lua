...
function onMobEngaged(mob, target)
    ...
    SpawnMob(ID.mob.APOLLYON_CS_MOB[1]+1):setMobMod(dsp.mobMod.SUPERLINK, mob:getShortID())
    SpawnMob(ID.mob.APOLLYON_CS_MOB[1]+2):setMobMod(dsp.mobMod.SUPERLINK, mob:getShortID())
    SpawnMob(ID.mob.APOLLYON_CS_MOB[1]+3):setMobMod(dsp.mobMod.SUPERLINK, mob:getShortID())
...
function onMobFight(mob, target)
    ...
    if battlefield then
        ...
        then
            ...
            SpawnMob(ID.mob.APOLLYON_CS_MOB[1]+4):setMobMod(dsp.mobMod.SUPERLINK, mob:getShortID())
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1]+5):setSpawn(mobX, mobY, mobZ)
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1]+5):setPos(mobX, mobY, mobZ)
            SpawnMob(ID.mob.APOLLYON_CS_MOB[1]+5):setMobMod(dsp.mobMod.SUPERLINK, mob:getShortID())
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1]+6):setSpawn(mobX, mobY, mobZ)
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1]+6):setPos(mobX, mobY, mobZ)
            SpawnMob(ID.mob.APOLLYON_CS_MOB[1]+6):setMobMod(dsp.mobMod.SUPERLINK, mob:getShortID())
