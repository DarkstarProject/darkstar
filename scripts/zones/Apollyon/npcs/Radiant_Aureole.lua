...
-- Area: Apollyon
-- NPC:  Radiant_Aureole
-- !pos  -601 0 -598 (SE/NE); 599 0 -600 (SW/NW)
-----------------------------------
...
function onTrade(player,npc,trade)
    ...
    if (player:hasKeyItem(dsp.ki.COSMOCLEANSE)) then
        ...
        print("error: player doesn't have cosmocleanse");
    end
...
function onTrigger(player,npc)
    ...
    if (player:hasKeyItem(dsp.ki.COSMOCLEANSE)) then
        if (player:hasStatusEffect(dsp.effect.BATTLEFIELD) == false) then
            ...
            for nt = 1,#instancelist,2 do
                ...
                if (instancelist[nt] == playerbcnmid) then
                    ...
                    if (AllowLimbusToPlayer == true) then --player has the correct key item for the current battflield
        ...
        if (limbusbitmap~= 0 ) then
            ...
            player:messageSpecial(CONDITION_FOR_LIMBUS);
            print("player needs a card for basic limbus");
        end
