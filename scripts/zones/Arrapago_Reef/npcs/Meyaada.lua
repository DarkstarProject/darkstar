...
function onTrigger(player,npc)
    ...
    elseif (beginnings == QUEST_ACCEPTED) then
        if (not player:hasKeyItem(dsp.kis.BRAND_OF_THE_SPRINGSERPENT)) then
    ...
    elseif (toauMission >= PRESIDENT_SALAHEEM) then
        ...
        if (player:hasKeyItem(dsp.kis.ILRUSI_ASSAULT_ORDERS) and player:hasKeyItem(dsp.kis.ASSAULT_ARMBAND) == false) then
...
function onEventFinish(player,csid,option)
    ...
    -- BEGINNINGS
    elseif (csid == 10) then
        player:addKeyItem(dsp.kis.BRAND_OF_THE_SPRINGSERPENT);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.BRAND_OF_THE_SPRINGSERPENT);
