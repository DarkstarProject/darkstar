...
function onTrigger(player,npc)
    if (player:hasKeyItem(dsp.kis.ILRUSI_ASSAULT_ORDERS)) then
        ...
        player:startEvent(219, assaultid, -4, 0, recommendedLevel, 2, armband);
    else
...
function onEventUpdate(player,csid,option,target)
    ...
    if (party ~= nil) then
        for i,v in ipairs(party) do
            if (not (v:hasKeyItem(dsp.kis.ILRUSI_ASSAULT_ORDERS) and v:getCurrentAssault() == assaultid)) then
        ...
        if (party ~= nil) then
            for i,v in ipairs(party) do
                if v:getID() ~= player:getID() and v:getZoneID() == player:getZoneID() then
                    ...
                    v:startEvent(108, 2);
                    v:delKeyItem(dsp.kis.ILRUSI_ASSAULT_ORDERS);
                end
            end
        end
    else
