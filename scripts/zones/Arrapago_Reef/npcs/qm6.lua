...
function onTrigger(player,npc)
    ...
    local efao   = player:getQuestStatus(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.EQUIPPED_FOR_ALL_OCCASIONS)
    local efaoCS = player:getVar("EquippedforAllOccasions")
    local ntus   = player:getQuestStatus(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.NAVIGATING_THE_UNFRIENDLY_SEAS)
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 228 then
        player:addQuest(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.EQUIPPED_FOR_ALL_OCCASIONS)
    ...
    elseif csid == 232 then
        player:addQuest(AHT_URHGAN, dsp.quests.ids.ahtUrhgan.NAVIGATING_THE_UNFRIENDLY_SEAS)
        player:setVar("NavigatingtheUnfriendlySeas", 1)
    elseif csid == 233 then
        npcUtil.completeQuest(player, AHT_URHGAN, dsp.quests.ids.ahtUrhgan.NAVIGATING_THE_UNFRIENDLY_SEAS, {item=15601, var={"NavigatingtheUnfriendlySeas", "HydrogauageTimer"}})
    end
end