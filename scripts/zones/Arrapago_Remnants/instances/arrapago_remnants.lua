...
function onInstanceTimeUpdate(instance, elapsed)
    ...
    if (wipeTime == 0) then
        ...
        if (wipe) then
            for i,v in pairs(players) do
                v:messageSpecial(Arrapago.text.PARTY_FALLEN, 3);
            end
    ...
    if (message ~= 0) then
        for i,v in pairs(players) do
            if (remainingTimeLimit >= 60) then
                v:messageSpecial(Arrapago.text.TIME_REMAINING_MINUTES, remainingTimeLimit / 60);
            else
                v:messageSpecial(Arrapago.text.TIME_REMAINING_SECONDS, remainingTimeLimit);
            end
        end
