...
function onMobInitialize(mob)
    ...
    mob:addStatusEffect(EFFECT.SHOCK_SPIKES,50,0,0);
    mob:getStatusEffect(EFFECT.SHOCK_SPIKES):setFlag(32);
end;

function onAdditionalEffect(mob,target,damage)
    -- Guestimating 2 in 3 chance to stun on melee.
    if ((math.random(1,100) >= 66) or (target:hasStatusEffect(EFFECT.STUN) == true)) then
        return 0,0,0;
    else
        local duration = math.random(5,15);
        target:addStatusEffect(EFFECT.STUN,5,0,duration);
