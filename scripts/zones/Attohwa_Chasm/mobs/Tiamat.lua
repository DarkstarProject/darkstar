...
function onMobFight(mob,target)
    ...
    if (mob:getHP() < ((mob:getMaxHP() / 10) * 2.5)) then
        if (mob:hasStatusEffect(EFFECT.ATTACK_BOOST) == false) then
            mob:addStatusEffect(EFFECT.ATTACK_BOOST,75,0,0);
            mob:getStatusEffect(EFFECT.ATTACK_BOOST):setFlag(32);
        end;
    end;

    if (mob:hasStatusEffect(EFFECT.MIGHTY_STRIKES) == false and mob:actionQueueEmpty() == true) then
