...
function onRegionEnter(player,region)
    if region:GetRegionID() == 1 then
        ...
        if player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.AN_EMPTY_VESSEL) == QUEST_ACCEPTED and player:getVar("AnEmptyVesselProgress") == 4 and player:hasItem(StoneID) then
...
function onEventFinish(player,csid,option)
    if
        csid == 3 and
        option == 13 and
        npcUtil.completeQuest(player, AHT_URHGAN, dsp.quests.ids.ahtUrhgan.AN_EMPTY_VESSEL, {title=dsp.title.BEARER_OF_THE_MARK_OF_ZAHAK, ki=dsp.ki.MARK_OF_ZAHAK, var={"AnEmptyVesselProgress", "EmptyVesselStone"}})
    ...
    elseif csid == 3 and option ~= 13 then -- Make a mistake and get reset
        ...
        player:delQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.AN_EMPTY_VESSEL)
        player:setPos(148,-2,0,130,50)
    elseif csid == 10 then
