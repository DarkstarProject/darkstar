...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK) == QUEST_ACCEPTED and player:getMaskBit(WildcatBastok,11) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 225 and option == 0) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.STAMP_HUNT);
    ...
    elseif (csid == 226) then
        if (player:getFreeSlotsCount(0) >= 1) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.STAMP_HUNT);
        else
