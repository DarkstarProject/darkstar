...
function onTrade(player,npc,trade)
    local BrygidReturns = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST_RETURNS);
    local wantsSubligar = player:getVar("BrygidWantsSubligar");

    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.RIDING_ON_THE_CLOUDS) == QUEST_ACCEPTED and player:getVar("ridingOnTheClouds_2") == 3) then
...
function onTrigger(player,npc)
    local BrygidTheStylist = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST);
    local BrygidReturns = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST_RETURNS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 310 and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST) == QUEST_AVAILABLE) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST);
    elseif (csid == 311) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST);
        end
    elseif (csid == 380) then
        player:delQuest(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST_RETURNS);
        player:addQuest(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST_RETURNS);
    ...
    elseif (csid == 383) then
        ...
        player:addFame(BASTOK,30);
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.BRYGID_THE_STYLIST_RETURNS);
    end
end;
