...
function onTrade(player,npc,trade)
    local theElvaanGoldsmith = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_ELVAAN_GOLDSMITH);
...
function onEventFinish(player,csid,option)
    ...
    -- THE ELVAAN GOLDSMITH
    if (csid == 215) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_ELVAAN_GOLDSMITH);
    elseif (csid == 216) then
        local fame = player:hasCompletedQuest(BASTOK, dsp.quests.ids.bastok.THE_ELVAAN_GOLDSMITH) and 8 or 100;
        if (npcUtil.completeQuest(player, BASTOK, dsp.quests.ids.bastok.THE_ELVAAN_GOLDSMITH, {gil=180, fame=fame})) then
            player:confirmTrade();
        end
    ...
    elseif (csid == 240) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.FATHER_FIGURE);
    elseif (csid == 241) then
        if (npcUtil.completeQuest(player, BASTOK, dsp.quests.ids.bastok.FATHER_FIGURE, {gil=2200, fame=120})) then
            player:confirmTrade();
        end
    end
end;
