...
function onTrigger(player,npc)
    ...
    local momTheAdventurer = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.MOM_THE_ADVENTURER);
    ...
    elseif (pFame >= 2 and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_SIGNPOST_MARKS_THE_SPOT) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 230 and option == 0) then
        if (player:getFreeSlotsCount(0) > 0) then
            ...
            if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.MOM_THE_ADVENTURER) == QUEST_AVAILABLE) then
                player:addQuest(BASTOK,dsp.quests.ids.bastok.MOM_THE_ADVENTURER);
            end
        else
    ...
    elseif (csid == 233 or csid == 234) then
        ...
        if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.MOM_THE_ADVENTURER) == QUEST_ACCEPTED) then
            player:addFame(BASTOK,20);
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.MOM_THE_ADVENTURER);
        else
            player:addFame(BASTOK,8)
        end
    elseif (csid == 235 and option == 0) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_SIGNPOST_MARKS_THE_SPOT);
        player:setVar("MomTheAdventurer_Event",0);
    end
