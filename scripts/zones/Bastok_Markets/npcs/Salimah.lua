...
function onTrade(player,npc,trade)
    ...
    local Gourmet = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.GOURMET);
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.GOURMET) ~= QUEST_AVAILABLE and player:needToZone()) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 200) then
        if (Gourmet == QUEST_AVAILABLE) then
            player:addQuest(BASTOK,dsp.quests.ids.bastok.GOURMET);
        end
    elseif (csid ~= 121) then
        player:tradeComplete();
        if (Gourmet == QUEST_ACCEPTED) then
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.GOURMET);
        end
