...
function onTrigger(player,npc)
    local cCollector = player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.THE_CURSE_COLLECTOR)
...
function onEventFinish(player,csid,option)
    if csid == 251 then
        player:addQuest(BASTOK, dsp.quests.ids.bastok.THE_CURSE_COLLECTOR)
        npcUtil.giveKeyItem(player, dsp.ki.CURSEPAPER)
    elseif csid == 252 and npcUtil.completeQuest(player, BASTOK, dsp.quests.ids.bastok.THE_CURSE_COLLECTOR, {item = 16387, var = {"cCollectSilence", "cCollectCurse"}}) then
