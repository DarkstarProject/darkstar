...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BETTER_PART_OF_VALOR) == QUEST_ACCEPTED) then
    ...
    elseif (player:getQuestStatus(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BETTER_PART_OF_VALOR) == QUEST_COMPLETED and player:getQuestStatus(CRYSTAL_WAR,FIRES_OF_DISCONTENT) == QUEST_AVAILABLE) then
            player:startEvent(120);
    elseif (player:getQuestStatus(CRYSTAL_WAR,dsp.quests.ids.crystalWar.FIRES_OF_DISCONTENT) == QUEST_ACCEPTED) then
    ...
    elseif (player:getQuestStatus(CRYSTAL_WAR,dsp.quests.ids.crystalWar.FIRES_OF_DISCONTENT) == QUEST_COMPLETED) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 118) then
        ...
        player:completeQuest(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BETTER_PART_OF_VALOR);
    ...
    elseif (csid == 120) then
        player:addQuest(CRYSTAL_WAR,dsp.quests.ids.crystalWar.FIRES_OF_DISCONTENT);
    ...
    elseif (csid == 164) then
        player:completeQuest(CRYSTAL_WAR,dsp.quests.ids.crystalWar.FIRES_OF_DISCONTENT);
        player:addGil(GIL_RATE*10000);
        player:messageSpecial(ID.text.GIL_OBTAINED,GIL_RATE*10000);
