...
function onEventFinish(player,csid,option)
    ...
    if (csid == 116) then
        ...
        player:delKeyItem(dsp.kis.CLUMP_OF_ANIMAL_HAIR);
    elseif (csid == 118) then
        player:delKeyItem(dsp.kis.XHIFHUT);
        player:completeQuest(CRYSTAL_WAR,BETTER_PART_OF_VALOR);
        player:addKeyItem(dsp.kis.WARNING_LETTER);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.WARNING_LETTER);
    ...
    elseif (csid == 120) then
        ...
        player:delKeyItem(dsp.kis.WARNING_LETTER);
