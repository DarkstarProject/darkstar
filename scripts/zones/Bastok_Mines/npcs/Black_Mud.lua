...
Drachenfall = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.DRACHENFALL);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 101) then
        ...
        if (Drachenfall == QUEST_AVAILABLE) then
            ...
            if (FreeSlots >= 1) then
                player:addQuest(BASTOK,dsp.quests.ids.bastok.DRACHENFALL);
                player:addItem(493);
    ...
    elseif (csid == 103) then
        ...
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.DRACHENFALL);
