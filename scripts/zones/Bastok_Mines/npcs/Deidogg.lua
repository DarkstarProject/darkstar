...
function onTrigger(player,npc)
    ...
    local theDoorman = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_DOORMAN);
    local theTalekeeperTruth = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_TALEKEEPER_S_TRUTH);
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK) == QUEST_ACCEPTED and player:getMaskBit(WildcatBastok,18) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 161) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_TALEKEEPER_S_TRUTH);
    ...
    elseif (csid == 165) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.THE_TALEKEEPER_S_TRUTH);
        end
    elseif (csid == 172) then
        player:tradeComplete();
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_TALEKEEPER_S_GIFT);
