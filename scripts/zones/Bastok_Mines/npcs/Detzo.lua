...
function onTrade(player,npc,trade)
    ...
    Rivals = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.RIVALS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 93) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.RIVALS);
    elseif (csid == 94) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.RIVALS);
        end
    end
