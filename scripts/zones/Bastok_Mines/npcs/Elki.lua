...
Hearts = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.HEARTS_OF_MYTHRIL);
HeartsVar = player:getVar("HeartsOfMythril");
Elevenths = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_ELEVENTH_S_HOUR);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 41 and option == 0) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.HEARTS_OF_MYTHRIL);
    ...
    elseif (csid == 42) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.HEARTS_OF_MYTHRIL);
            player:addFame(BASTOK,80);
            player:setVar("HeartsOfMythril",0);
            player:needToZone(true);
        end
    elseif (csid == 43 and option == 1) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_ELEVENTH_S_HOUR);
    elseif (csid == 44) then
