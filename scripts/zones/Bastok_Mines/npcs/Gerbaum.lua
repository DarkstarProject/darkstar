...
ZeruhnSoot = trade:hasItemQty(560,3);
    ...
    if (ZeruhnSoot == true and count == 3) then
        MineSweep = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.MINESWEEPER);
        if (MineSweep >= 1) then
...
MineSweep = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.MINESWEEPER);
    ...
    if (csid == 108) then
        if (MineSweep == 0) then
            player:addQuest(BASTOK,dsp.quests.ids.bastok.MINESWEEPER);
        end
    elseif (csid == 109) then
        if (MineSweep == 1) then
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.MINESWEEPER);
