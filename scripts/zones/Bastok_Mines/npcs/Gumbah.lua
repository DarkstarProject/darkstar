...
function onTrigger(player,npc)
    ...
    local bladeDarkness = player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.BLADE_OF_DARKNESS)
    ...
    elseif (bladeDarkness == QUEST_COMPLETED and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BLADE_OF_DEATH) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    if (csid == 99) then
        player:addQuest(BASTOK, dsp.quests.ids.bastok.BLADE_OF_DARKNESS)
    elseif (csid == 130) then
        player:addQuest(BASTOK, dsp.quests.ids.bastok.BLADE_OF_DEATH)
