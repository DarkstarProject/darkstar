...
function onTrade(player,npc,trade)
    ...
    if (CurrentMission ~= 255) then
        if (CurrentMission == FETICHISM and player:hasCompletedMission(BASTOK,dsp.mission.id.bastok.FETICHISM) == false and trade:hasItemQty(606,1) and trade:hasItemQty(607,1) and trade:hasItemQty(608,1) and trade:hasItemQty(609,1) and Count == 4) then
        ...
        elseif (CurrentMission == TO_THE_FORSAKEN_MINES and player:hasCompletedMission(BASTOK,dsp.mission.id.bastok.TO_THE_FORSAKEN_MINES) == false and trade:hasItemQty(563,1) and Count == 1) then
