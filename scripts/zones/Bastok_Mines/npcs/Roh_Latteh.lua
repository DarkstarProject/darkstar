...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.MOM_THE_ADVENTURER) ~= QUEST_AVAILABLE and player:getVar("MomTheAdventurer_Event") == 1) then
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_SIGNPOST_MARKS_THE_SPOT) == QUEST_ACCEPTED and HasPainting == true) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 96) then
        ...
        if (freeInventory > 0) then
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.THE_SIGNPOST_MARKS_THE_SPOT);
