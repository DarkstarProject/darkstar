...
function onTrade(player,npc,trade)
    local groceries = player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.GROCERIES);
...
function onEventFinish(player,csid,option)
    -- GROCERIES
    if (csid == 110) then
        player:addQuest(BASTOK, dsp.quests.ids.bastok.GROCERIES);
    ...
    elseif (csid == 113) then
        if (player:getFreeSlotsCount() >= 1) then
            ...
            player:setVar("Groceries",0);
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.GROCERIES);
