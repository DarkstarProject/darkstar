...
function onTrigger(player,npc)
    ...
    elseif (ENABLE_WOTG == 1 and hasMawActivated(player,0)) then
        ...
        (player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.CLAWS_OF_THE_GRIFFON) == QUEST_COMPLETED or
         player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.THE_TIGRESS_STRIKES) == QUEST_COMPLETED or
         player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.FIRES_OF_DISCONTENT) == QUEST_COMPLETED)) then
