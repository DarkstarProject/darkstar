...
function onZoneIn(player,prevZone)
    ...
    if prevZone == dsp.zone.PASHHOW_MARSHLANDS then
        if player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.BLADE_OF_DARKNESS) == QUEST_ACCEPTED and player:getVar("ChaosbringerKills") >= 100 then
            cs = 121
        elseif player:getCurrentMission(BASTOK) == THE_FOUR_MUSKETEERS and player:getVar("MissionStatus") == 1 then
            cs = 120
        elseif player:getMainJob() == dsp.job.DRK and player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.DARK_PUPPET) == QUEST_COMPLETED and player:getQuestStatus(BASTOK, BLADE_OF_EVIL) == QUEST_AVAILABLE then
            cs = 122
        end
    end
...
function onRegionEnter(player,region)
    if region:GetRegionID() <= 6 then
        if not player:hasStatusEffect(dsp.effect.CURSE_I) and not player:hasStatusEffect(dsp.effect.SILENCE) then
            ...
            if player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.THE_CURSE_COLLECTOR) == QUEST_ACCEPTED and player:getVar("cCollectCurse") == 0 then
...
function onEventFinish(player,csid,option)
    if csid == 121 and npcUtil.completeQuest(player, BASTOK, dsp.quests.ids.bastok.BLADE_OF_DARKNESS, {title=dsp.title.DARK_SIDER, var="ZeruhnMines_Zeid_CS"}) then
    ...
    elseif csid == 122 then
        player:addQuest(BASTOK, dsp.quests.ids.bastok.BLADE_OF_EVIL)
    end
end