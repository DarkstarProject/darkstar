...
-- TODO: The ??? should only spawn during rainy weather, temporary fix in place to prevent players from getting the keyitem unless the proper weather is present.
    if (player:getQuestStatus(BASTOK,BEADEAUX_SMOG) == QUEST_ACCEPTED and player:hasKeyItem(dsp.kis.CORRUPTED_DIRT) == false and player:getWeather() == dsp.weather.RAIN) then
