...
function onTrigger(player,npc)
    if (player:getVar("DynaBeaucedine_Win") == 1) then
        player:startEvent(134,dsp.kis.HYDRA_CORPS_INSIGNIA); -- Win CS
    elseif (player:hasKeyItem(dsp.kis.VIAL_OF_SHROUDED_SAND) and player:hasKeyItem(dsp.kis.HYDRA_CORPS_COMMAND_SCEPTER) and
            player:hasKeyItem(dsp.kis.HYDRA_CORPS_EYEGLASS) and player:hasKeyItem(dsp.kis.HYDRA_CORPS_LANTERN) and player:hasKeyItem(dsp.kis.HYDRA_CORPS_TACTICAL_MAP)) then
        local firstDyna = 0;
        ...
        elseif ((dynaWaitxDay + (BETWEEN_2DYNA_WAIT_TIME * 60 * 60)) < realDay or (player:getVar("DynamisID") == dynaUniqueID and dynaUniqueID > 0)) then
            player:startEvent(119,5,firstDyna,0,BETWEEN_2DYNA_WAIT_TIME,64,dsp.kis.VIAL_OF_SHROUDED_SAND,4236,4237);
        else
