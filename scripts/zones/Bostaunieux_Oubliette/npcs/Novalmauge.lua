...
function onTrade(player, npc, trade)
    ...
    if player:getVar("troubleAtTheSluiceVar") == 2 and npcUtil.tradeHas(trade, 959) then -- Dahlia
        ...
        npc:wait()
    elseif player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.THE_RUMOR) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, 930) then -- Beastman Blood
...
function onTrigger(player, npc)
    ...
    local troubleAtTheSluice = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.TROUBLE_AT_THE_SLUICE)
    local troubleAtTheSluiceStat = player:getVar("troubleAtTheSluiceVar")
    local theHolyCrestStat = player:getVar("TheHolyCrest_Event")
    local theRumor = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.THE_RUMOR)
...
function onEventFinish(player, csid, option, npc)
    ...
    elseif csid == 12 and npcUtil.completeQuest(player, SANDORIA, dsp.quests.ids.sandoria.THE_RUMOR, {item = 4853}) then
        player:confirmTrade()
    elseif csid == 13 and option == 1 then
        player:addQuest(SANDORIA, dsp.quests.ids.sandoria.THE_RUMOR)
    elseif csid == 14 then
