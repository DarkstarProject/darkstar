-- Variable TextID   Description text

-- General Texts
ITEM_CANNOT_BE_OBTAINED = 6403; -- You cannot obtain the item <item>. Come back after sorting your inventory.
          ITEM_OBTAINED = 6406; -- Obtained: <item>.
           GIL_OBTAINED = 6407; -- Obtained <number> gil.
       KEYITEM_OBTAINED = 6409; -- Obtained key item: <keyitem>.
        BEASTMEN_BANNER = 7128; -- There is a beastmen's banner.
 FISHING_MESSAGE_OFFSET = 7212; -- You can't fish here.

-- Conquest
CONQUEST = 7369; -- You've earned conquest points!

-- Logging
LOGGING_IS_POSSIBLE_HERE = 7353; -- Logging is possible here if you have

-- Dialog Texts
FIVEOFSPADES_DIALOG = 7206; -- GiMmeIvE! FiVe is cArdIanF WiN-DuRst! FIvES OnA-tRol!

	  YOU_CANNOT_ENTER_DYNAMIS = 7845; -- You cannot enter Dynamis
		  MYSTERIOUS_VOICE = 7846; -- You hear a mysterious, floating voice: The guiding aura has not yet faded... Bring forth the
PLAYERS_HAVE_NOT_REACHED_LEVEL = 7847; -- Players who have not reached levelare prohibited from entering Dynamis.

-- Signs          
SIGN_1 = 7348; -- West: Tahrongi Canyon Southeast: Mhaura
SIGN_2 = 7349; -- West: Tahrongi Canyon South: Mhaura
SIGN_3 = 7350; -- West: Tahrongi Canyon Southwest: Mhaura
SIGN_4 = 7351; -- West: Mhaura and Tahrongi Canyon
SIGN_5 = 7352; -- West: Mhaura Northwest: Tahrongi Canyon
