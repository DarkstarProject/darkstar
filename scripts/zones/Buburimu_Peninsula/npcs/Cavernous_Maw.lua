...
function onTrigger(player,npc)
    if (ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30) then
        ...
        and player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.A_FLUTTERY_FIEND) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    if (csid == 62) then
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.A_FLUTTERY_FIEND);
    elseif (csid == 63) then
        -- Killed Itzpapalotl
