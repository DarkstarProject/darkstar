...
function onTrigger(player,npc)
    ...
    if ((player:hasCompletedMission(COP,DARKNESS_NAMED) or FREE_COP_DYNAMIS == 1) and player:hasKeyItem(dsp.kis.VIAL_OF_SHROUDED_SAND)) then
        ...
        elseif ((dynaWaitxDay + (BETWEEN_2DYNA_WAIT_TIME * 60 * 60)) < realDay or (player:getVar("DynamisID") == dynaUniqueID and dynaUniqueID > 0)) then
            player:startEvent(22,8,0,0,BETWEEN_2DYNA_WAIT_TIME,32,dsp.kis.VIAL_OF_SHROUDED_SAND,4236,4237);
        else
