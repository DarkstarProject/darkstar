...
function onTrigger(player,npc)
    if (player:hasCompletedMission(TOAU,LOST_KINGDOM)) then
        if (not player:hasKeyItem(dsp.kis.EPHRAMADIAN_GOLD_COIN)) then
            player:addKeyItem(dsp.kis.EPHRAMADIAN_GOLD_COIN);
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.EPHRAMADIAN_GOLD_COIN);
        end
    elseif (player:getCurrentMission(TOAU) == LOST_KINGDOM) then
        if (player:hasKeyItem(dsp.kis.VIAL_OF_SPECTRAL_SCENT) and player:getVar("AhtUrganStatus") == 0) then
            player:startEvent(8);
