...
function onTrigger(player,npc)
    ...
    -- SHADES OF VENGEANCE
    elseif (toauMission == SHADES_OF_VENGEANCE and player:hasKeyItem(dsp.kis.PERIQIA_ASSAULT_AREA_ENTRY_PERMIT) == false and vanaDay() > player:getVar("TOAUM31_PERMITDAY")) then
        player:startEvent(22);

    -- BEGINNINGS
    elseif (beginnings == QUEST_ACCEPTED) then
        if (not player:hasKeyItem(dsp.kis.BRAND_OF_THE_GALESERPENT)) then
    ...
    elseif (toauMission >= PRESIDENT_SALAHEEM) then
        ...
        if (player:hasKeyItem(dsp.kis.PERIQIA_ASSAULT_ORDERS) and player:hasKeyItem(dsp.kis.ASSAULT_ARMBAND) == false) then
...
function onEventFinish(player,csid,option)
    ...
    -- BEGINNINGS
    elseif (csid == 10) then
        player:addKeyItem(dsp.kis.BRAND_OF_THE_GALESERPENT);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.BRAND_OF_THE_GALESERPENT);
