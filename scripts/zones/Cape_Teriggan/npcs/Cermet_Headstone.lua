...
function onTrade(player,npc,trade)
    ...
    if (trade:hasItemQty(949,1) and trade:getItemCount() == 1) then
        if (not player:hasCompletedQuest(OUTLANDS,dsp.quests.ids.outlands.WANDERING_SOULS) and (player:hasCompletedMission(ZILART,HEADSTONE_PILGRIMAGE) or player:hasKeyItem(dsp.ki.WIND_FRAGMENT))) then
             player:addQuest(OUTLANDS,dsp.quests.ids.outlands.WANDERING_SOULS);
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 202) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.WANDERING_SOULS);
        end
    end
end;
