...
function onTrade(player,npc,trade)
	...
	if (player:hasKeyItem(VIAL_OF_SHROUDED_SAND) == true) then
		...
		elseif (gil == 0) then
			...
			-- Currency Exchanges
			elseif (count == CURRENCY_EXCHANGE_RATE and trade:hasItemQty(1449,CURRENCY_EXCHANGE_RATE)) then -- Whiteshells -> Jadeshell
				player:startEvent(55,CURRENCY_EXCHANGE_RATE);
			elseif (count == CURRENCY_EXCHANGE_RATE and trade:hasItemQty(1450,CURRENCY_EXCHANGE_RATE)) then -- Jadeshells -> Stripeshell
				player:startEvent(56,CURRENCY_EXCHANGE_RATE);
			elseif (count == 1 and trade:hasItemQty(1451,1)) then -- Stripeshell -> Jadeshells
...
function onEventUpdate(player,csid,option)
	...
	if (csid == 53) then
		...
		elseif (option == 2) then -- Jadeshell sales menu Page 1 (price1 item1 price2 item2 price3 item3 price4 item4)
			player:updateEvent(7,1312,23,1463,8,1518,28,658);
		elseif (option == 3) then -- Jadeshell sales menu Page 2 (price1 item1 price2 item2 price3 item3)
...
function onEventFinish(player,csid,option)
	...
	elseif (csid == 55) then -- Trading Whiteshells for a Jadeshell
		if (player:getFreeSlotsCount() == 0) then
	...
	elseif (csid == 56) then -- Trading Jadeshells for Stripeshell
		if (player:getFreeSlotsCount() == 0) then
			player:messageSpecial(ITEM_CANNOT_BE_OBTAINED,1450);
		else
			player:tradeComplete();
	...
	elseif (csid == 58) then -- Trading Stripeshell for Jadeshells
