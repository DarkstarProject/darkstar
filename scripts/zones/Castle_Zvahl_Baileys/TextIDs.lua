-- Variable TextID   Description text

-- General Texts
ITEM_CANNOT_BE_OBTAINED = 6537; -- You cannot obtain the item <item>. come back after sorting your inventory.
          ITEM_OBTAINED = 6540; -- Obtained: <item>.
           GIL_OBTAINED = 6541; -- Obtained <number> gil.
       KEYITEM_OBTAINED = 6543; -- Obtained key item: <keyitem>.

-- Treasure Coffer/Chest Dialog
CHEST_UNLOCKED = 7197; -- You unlock the chest!
    CHEST_FAIL = 7198; -- Fails to open the chest.
    CHEST_TRAP = 7199; -- The chest was trapped!
    CHEST_WEAK = 7200; -- You cannot open the chest when you are in a weakened state.
   CHEST_MIMIC = 7201; -- The chest was a mimic!
  CHEST_MOOGLE = 7202; -- You cannot open the chest while participating in the moogle event.
CHEST_ILLUSION = 7203; -- The chest was but an illusion...
  CHEST_LOCKED = 7204; -- The chest appears to be locked.

-- Quest dialog
    SENSE_OF_FOREBODING = 6555; -- You are suddenly overcome with a sense of foreboding...
NOTHING_OUT_OF_ORDINARY = 7527; -- There is nothing out of the ordinary here.
