...
function onTrade(player,npc,trade)
    ...
    if ((trade:hasItemQty(1048,1) or trade:hasItemQty(1115,1) or trade:hasItemQty(1023,1) or trade:hasItemQty(1022,1)) and trade:getItemCount() == 1) then
        ...
        if (AFHandsActivated == 10 and player:hasKeyItem(dsp.ki.OLD_GAUNTLETS) == false) then
            questItemNeeded = 1;
        else
        ...
        if (success ~= -2) then
            ...
            if (math.random() <= success) then  -- 0 or 1
                ...
                elseif (player:getVar("UnderOathCS") == 3) then   -- Under Oath - PLD AF3
                    player:addKeyItem(dsp.ki.MIQUES_PAINTBRUSH);
                    player:messageSpecial(ID.text.KEYITEM_OBTAINED,dsp.ki.MIQUES_PAINTBRUSH);
                else
...
function onTrigger(player,npc)
    player:messageSpecial(ID.text.CHEST_LOCKED,1048);
end;
