...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(ZILART) == THE_CHAMBER_OF_ORACLES) then
        if (player:hasKeyItem(dsp.kis.DARK_FRAGMENT)) then
            player:delKeyItem(dsp.kis.DARK_FRAGMENT);
            player:setVar("ZilartStatus",ZilartStatus + 2);
            player:messageSpecial(YOU_PLACE_THE,dsp.kis.DARK_FRAGMENT);
        ...
        elseif (ZilartStatus == 255) then -- Execute cutscene if the player is interrupted.
            ...
            player:messageSpecial(IS_SET_IN_THE_PEDESTAL,dsp.kis.DARK_FRAGMENT);
        end
    elseif (player:hasCompletedMission(ZILART,THE_CHAMBER_OF_ORACLES)) then
        player:messageSpecial(HAS_LOST_ITS_POWER,dsp.kis.DARK_FRAGMENT);
    else
