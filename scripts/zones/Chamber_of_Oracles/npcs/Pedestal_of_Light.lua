...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(ZILART) == THE_CHAMBER_OF_ORACLES) then
        if (player:hasKeyItem(dsp.ki.LIGHT_FRAGMENT)) then
            ...
            player:messageSpecial(YOU_PLACE_THE,dsp.ki.LIGHT_FRAGMENT);
        ...
        elseif (ZilartStatus == 255) then -- Execute cutscene if the player is interrupted.
            ...
            player:messageSpecial(IS_SET_IN_THE_PEDESTAL,dsp.ki.LIGHT_FRAGMENT);
        end
    elseif (player:hasCompletedMission(ZILART,THE_CHAMBER_OF_ORACLES)) then
        player:messageSpecial(HAS_LOST_ITS_POWER,dsp.ki.LIGHT_FRAGMENT);
    else
