...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(ZILART) == THE_CHAMBER_OF_ORACLES) then
        if (player:hasKeyItem(dsp.kis.LIGHTNING_FRAGMENT)) then
            player:delKeyItem(dsp.kis.LIGHTNING_FRAGMENT);
            player:setVar("ZilartStatus",ZilartStatus + 32);
            player:messageSpecial(YOU_PLACE_THE,dsp.kis.LIGHTNING_FRAGMENT);
        ...
        elseif (ZilartStatus == 255) then -- Execute cutscene if the player is interrupted.
            ...
            player:messageSpecial(IS_SET_IN_THE_PEDESTAL,dsp.kis.LIGHTNING_FRAGMENT);
        end
    elseif (player:hasCompletedMission(ZILART,THE_CHAMBER_OF_ORACLES)) then
        player:messageSpecial(HAS_LOST_ITS_POWER,dsp.kis.LIGHTNING_FRAGMENT);
    else
