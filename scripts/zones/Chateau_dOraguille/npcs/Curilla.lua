...
function onTrigger(player,npc)
    ...
    local theGeneralSecret = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_GENERAL_S_SECRET)
    local envelopedInDarkness = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.ENVELOPED_IN_DARKNESS)
    local peaceForTheSpirit = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.PEACE_FOR_THE_SPIRIT)
    local WildcatSandy = player:getVar("WildcatSandy")

    if wsQuestEvent ~= nil then
        player:startEvent(wsQuestEvent)
    elseif (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.LURE_OF_THE_WILDCAT_SAN_D_ORIA) == QUEST_ACCEPTED and player:getMaskBit(WildcatSandy,15) == false) then
    ...
    elseif (mJob == dsp.job.RDM and mLvL >= AF2_QUEST_LEVEL and player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_CRIMSON_TRIAL) == QUEST_COMPLETED and envelopedInDarkness == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    if (csid == 55 and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_GENERAL_S_SECRET)
    ...
    elseif (csid == 54) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_GENERAL_S_SECRET)
        end
    elseif (csid == 94 and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.ENVELOPED_IN_DARKNESS)
    ...
    elseif (csid == 109 and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.PEACE_FOR_THE_SPIRIT)
