...
function onEventFinish(player,csid,option)
    ...
    if (csid == 0x0037 and option == 1) then 
        ...
        player:messageSpecial(text.KEYITEM_OBTAINED,CURILLAS_BOTTLE_EMPTY);
    elseif (csid == 0x0036) then 
        if (player:getFreeSlotsCount() == 0) then 
            player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED,16409); -- Lynx Baghnakhs
        else
            player:delKeyItem(CURILLAS_BOTTLE_FULL);
            player:addItem(16409);
            player:messageSpecial(text.ITEM_OBTAINED,16409); -- Lynx Baghnakhs
    ...
    elseif (csid == 0x005E and option == 1) then
        ...
        player:messageSpecial(text.KEYITEM_OBTAINED,OLD_POCKET_WATCH);
