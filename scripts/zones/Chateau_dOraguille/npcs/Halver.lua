...
function onTrigger(player,npc)
    ...
    -- Blackmail quest
    elseif player:getQuestStatus(SANDORIA, BLACKMAIL) == QUEST_ACCEPTED and player:hasKeyItem(dsp.ki.SUSPICIOUS_ENVELOPE) then
    ...
    elseif player:getQuestStatus(SANDORIA, A_TIMELY_VISIT) == QUEST_ACCEPTED and aTimelyVisitStep >= 3 then
        if aTimelyVisitStep == 3 then
            player:startEvent(112)
        else
            player:showText(npc, 7047)    
        end
        -- Important: Do not move any of the above logic after the nation checks below.
...
function onEventFinish(player,csid,option)
    ...
    if csid == 112 then
        player:setVar("aTimelyVisitStep", 4)
