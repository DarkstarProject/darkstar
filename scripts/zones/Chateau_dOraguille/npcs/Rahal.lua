...
function onTrigger(player,npc)
    ...
    local Stalker_Quest = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.KNIGHT_STALKER);
    local StalkerProgress = player:getVar("KnightStalker_Progress");
    local WildcatSandy = player:getVar("WildcatSandy");

    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.LURE_OF_THE_WILDCAT_SAN_D_ORIA) == QUEST_ACCEPTED and player:getMaskBit(WildcatSandy,17) == false) then
    ...
    elseif (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.CHASING_QUOTAS) == QUEST_COMPLETED and Stalker_Quest == QUEST_AVAILABLE and player:getMainJob() == dsp.job.DRG) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 121) then
        if (option == 1) then
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.KNIGHT_STALKER);
        else
