...
function onTrigger(player,npc)
    ...
    elseif (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.A_BOY_S_DREAM) == QUEST_COMPLETED and player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.UNDER_OATH) == QUEST_AVAILABLE and player:getMainJob() == dsp.job.PLD) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 88) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            if (player:getMainJob() == dsp.job.PLD) then
                player:addQuest(SANDORIA,dsp.quests.ids.sandoria.UNDER_OATH);
            end
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.A_BOY_S_DREAM);
        end
    ...
    elseif (csid == 89) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.UNDER_OATH);
        end
    elseif (csid == 81) then
