...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(ZILART) == HEADSTONE_PILGRIMAGE) then
        if (player:hasKeyItem(dsp.kis.ICE_FRAGMENT) == false) then
            player:startEvent(200,dsp.kis.ICE_FRAGMENT);
        ...
        elseif (player:hasKeyItem(dsp.kis.ICE_FRAGMENT)) then
            player:messageSpecial(ALREADY_OBTAINED_FRAG,dsp.kis.ICE_FRAGMENT);
        end
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 200 and option == 1) then
        player:addKeyItem(dsp.kis.ICE_FRAGMENT);
        -- Check and see if all fragments have been found (no need to check ice and dark frag)
        if (player:hasKeyItem(dsp.kis.FIRE_FRAGMENT) and player:hasKeyItem(dsp.kis.EARTH_FRAGMENT) and player:hasKeyItem(dsp.kis.WATER_FRAGMENT) and
           player:hasKeyItem(dsp.kis.WIND_FRAGMENT) and player:hasKeyItem(dsp.kis.LIGHTNING_FRAGMENT) and player:hasKeyItem(dsp.kis.LIGHT_FRAGMENT)) then
            player:messageSpecial(FOUND_ALL_FRAGS,dsp.kis.ICE_FRAGMENT);
            ...
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.ICE_FRAGMENT);
        end
    end
