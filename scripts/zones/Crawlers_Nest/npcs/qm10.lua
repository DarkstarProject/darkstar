...
function onTrigger(player,npc)
    if (OldSchoolG1 == false) then
        if (player:hasItem(1089) == false and player:hasKeyItem(dsp.kis.EXORAY_MOLD_CRUMB1) == false
        and player:getQuestStatus(JEUNO,IN_DEFIANT_CHALLENGE) == QUEST_ACCEPTED) then
            player:addKeyItem(dsp.kis.EXORAY_MOLD_CRUMB1);
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.EXORAY_MOLD_CRUMB1);
        end
