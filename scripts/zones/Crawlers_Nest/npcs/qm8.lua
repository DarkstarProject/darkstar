...
function onTrigger(player,npc)
    ...
    elseif (cprog == 1 and cdate == realday) then
        player:messageSpecial(text.EQUIPMENT_COMPLETELY_PURIFIED);
    elseif (cprog == 1 and cdate ~= realday) then
        player:startEvent(5);
    else
        player:messageSpecial(text.SOMEONE_HAS_BEEN_DIGGING_HERE);
    end
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 4 and option == 1) then
        ...
        player:messageSpecial(text.YOU_BURY_THE,OLD_BOOTS,CRAWLER_BLOOD);
    elseif (csid == 5) then
        if (player:getFreeSlotsCount() == 0) then 
            player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED,14093); -- Warlock's Boots
        else
            player:addItem(14093);
            player:messageSpecial(text.ITEM_OBTAINED, 14093); -- Warlock's Boots
