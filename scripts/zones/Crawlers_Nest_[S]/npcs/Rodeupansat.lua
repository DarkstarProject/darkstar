...
function onTrigger(player, npc)
    local Eati = player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.EVIL_AT_THE_INLET)
...
function onEventFinish(player, csid, option)
    if csid == 107 then
        player:addQuest(CRYSTAL_WAR, dsp.quests.ids.crystalWar.EVIL_AT_THE_INLET)
        npcUtil.giveKeyItem(player, dsp.ki.EVIL_WARDING_SEAL)
    elseif csid == 109 then 
       npcUtil.completeQuest(player, CRYSTAL_WAR, dsp.quests.ids.crystalWar.EVIL_AT_THE_INLET, {item = 4687})
    end
end
