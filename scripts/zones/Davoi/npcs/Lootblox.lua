...
function onEventFinish(player,csid,option)
   ...
   elseif (csid == 135) then -- Trading Singles for a Hundred
      if (player:getFreeSlotsCount() == 0) then
         player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED,1453);
      else
         player:tradeComplete();
         player:addItem(1453);
         player:messageSpecial(text.ITEM_OBTAINED,1453);
      end
   elseif (csid == 136) then -- Trading 100 Hundreds for Ten thousand
      if (player:getFreeSlotsCount() == 0) then
         player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED,1454);
      else
         player:tradeComplete();
         player:addItem(1454);
         player:messageSpecial(text.ITEM_OBTAINED,1454);
      end
   elseif (csid == 138) then -- Trading Ten thousand for 100 Hundreds
      if (player:getFreeSlotsCount() <= 1) then
         ...
         player:messageSpecial(text.ITEMS_OBTAINED,1453,CURRENCY_EXCHANGE_RATE);
      end
   end
end;