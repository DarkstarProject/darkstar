...
DEN_OF_RANCOR = {
    ...
    onTradeLanternChamber = function(player, npc, trade)
        if npcUtil.tradeHas(trade, 1139) then -- Rancor Flame
            if npc:getAnimation() == dsp.anim.OPEN_DOOR then
                ...
                local total = GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 0):getAnimation() +
                              GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 1):getAnimation() +
                              GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 2):getAnimation() +
                              GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 3):getAnimation()

                player:messageSpecial(ID.text.LANTERN_OFFSET + 44 - total)
                
                if total == 32 then
                    for i = ID.npc.RANCOR_LANTERN_OFFSET, ID.npc.RANCOR_LANTERN_OFFSET + 3 do
                        local lantern = GetNPCByID(i)
                        lantern:closeDoor(1)
                        lantern:openDoor(30)
                    end
                    GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 4):openDoor(30)
                end
            end
        end
    end,
    ...
    onTradeLanternHaku = function(player, npc, trade)
        if npcUtil.tradeHas(trade, 1139) then -- Rancor Flame
            if npc:getAnimation() == dsp.anim.OPEN_DOOR then
                ...
                local total = GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 6):getAnimation() +
                              GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 7):getAnimation()

                if total == 17 then
                    player:messageSpecial(ID.text.LANTERN_OFFSET + 9)
                elseif total == 16 then
                    player:messageSpecial(ID.text.LANTERN_OFFSET + 12)
                    for i = ID.npc.RANCOR_LANTERN_OFFSET + 6, ID.npc.RANCOR_LANTERN_OFFSET + 7 do
                        local lantern = GetNPCByID(i)
                        lantern:closeDoor(1)
                        lantern:openDoor(30)
                    end
                    GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 8):openDoor(30)
                end
            end
        end
    end,
    ...
    onTradeLanternBoss = function(player, npc, trade)
        local itemId = 1131 + npc:getID() - ID.npc.RANCOR_LANTERN_OFFSET
        ...
        if npcUtil.tradeHas(trade, itemId) then -- Flame of Crimson or Blue Rancor
            if npc:getAnimation() == dsp.anim.OPEN_DOOR then
                ...
                local total = GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 9):getAnimation() +
                              GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 10):getAnimation()

                if total == 17 then
                    player:messageSpecial(ID.text.LANTERN_OFFSET + 9)
                elseif total == 16 then
                    player:messageSpecial(ID.text.LANTERN_OFFSET + 12)
                    for i = ID.npc.RANCOR_LANTERN_OFFSET + 9, ID.npc.RANCOR_LANTERN_OFFSET + 10 do
                        local lantern = GetNPCByID(i)
                        lantern:closeDoor(1)
                        lantern:openDoor(30)
                    end
                    GetNPCByID(ID.npc.RANCOR_LANTERN_OFFSET + 11):openDoor(30)
                end
            end
        end
    end,
