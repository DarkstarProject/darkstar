...
function onZoneIn(player,prevZone)
    ...
    elseif (player:getCurrentMission(ACP) == THOSE_WHO_LURK_IN_SHADOWS_I) then
        cs = 29;
    elseif (prevZone == dsp.zone.QUBIA_ARENA and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_FIRST_MEETING) == QUEST_ACCEPTED and not player:hasKeyItem(dsp.ki.LETTER_FROM_DALZAKK)) then
        cs = 16; -- MNK AF
