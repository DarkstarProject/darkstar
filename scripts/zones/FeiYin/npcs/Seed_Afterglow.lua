...
function onTrigger(player,npc)
    ...
    local MARK_EFFECT = player:hasStatusEffect(EFFECT.MARK_OF_SEED);
    ...
    elseif (NeedToZone == true and player:hasStatusEffect(EFFECT.MARK_OF_SEED) == false) then
    ...
    elseif (Mission >= THOSE_WHO_LURK_IN_SHADOWS_II) then
        if (Afterglow == First and player:getMaskBit(PROGRESS,0) == false) then -- Red
            ...
            elseif (NeedToZone == false and MARK_EFFECT == false) then
                ...
                player:addStatusEffectEx(EFFECT.MARK_OF_SEED, 0, 0, 30, 1800);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 28) then
        ...
        player:delStatusEffectSilent(EFFECT.MARK_OF_SEED);
    end
end;