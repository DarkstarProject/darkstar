...
function onTrigger(player,npc)
    local offset = npc:getID() - UNDERGROUND_POOL_OFFSET
    ...
    if player:getQuestStatus(JEUNO, SCATTERED_INTO_SHADOW) == QUEST_ACCEPTED then
        ...
        elseif offset == 1 and player:hasKeyItem(dsp.ki.AQUAFLORA3) and not GetMobByID(DABOTZS_GHOST):isSpawned() then
            SpawnMob(DABOTZS_GHOST):updateClaim(player)
