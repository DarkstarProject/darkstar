...
function onTrigger(player, npc)
    local TheFumblingFriar = player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.THE_FUMBLING_FRIAR)
...
function onEventFinish(player, csid, option)
    if csid == 26 then
        player:addQuest(CRYSTAL_WAR, dsp.quests.ids.crystalWar.THE_FUMBLING_FRIAR)
    elseif csid == 28 and npcUtil.completeQuest(player, CRYSTAL_WAR, dsp.quests.ids.crystalWar.THE_FUMBLING_FRIAR, {item = 4688}) then
