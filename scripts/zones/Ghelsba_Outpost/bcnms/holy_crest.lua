...
-- print(leave code ..leavecode);
    ...
    if (leavecode == 2) then --play end CS. Need time and battle id for record keeping + storage
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_HOLY_CREST) == QUEST_ACCEPTED) then
...
-- print("bc finish csid: "..csid.."and option: "..option);
    ...
    if (csid == 32001 and option ~= 0 and player:hasKeyItem(dsp.ki.DRAGON_CURSE_REMEDY) == true) then
        ...
        player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_HOLY_CREST);
