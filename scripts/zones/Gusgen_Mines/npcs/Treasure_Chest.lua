...
function onTrade(player,npc,trade)
    ...
    if ((trade:hasItemQty(1031,1) or trade:hasItemQty(1115,1) or trade:hasItemQty(1023,1) or trade:hasItemQty(1022,1)) and trade:getItemCount() == 1) then
        ...
        if (player:getQuestStatus(JEUNO,THE_GOBLIN_TAILOR) >= QUEST_ACCEPTED and VanadielRSELocation() == 1 and VanadielRSERace() == player:getRace() and player:hasKeyItem(dsp.kis.MAGICAL_PATTERN) == false) then
            questItemNeeded = 1;
        end
        --------------------------------------
