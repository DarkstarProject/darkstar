...
function onTrade(player,npc,trade)
    ...
    elseif (
        player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.BLADE_OF_DEATH) == QUEST_ACCEPTED and
...
function onEventFinish(player,csid,option)
    if (csid == 10 and npcUtil.completeQuest(player, BASTOK, dsp.quests.ids.bastok.BLADE_OF_DEATH, {item=16637, title=dsp.title.BLACK_DEATH, var="ChaosbringerKills"})) then
        player:confirmTrade();
