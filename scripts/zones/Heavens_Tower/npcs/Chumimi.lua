...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_THREE_MAGI) == QUEST_ACCEPTED) then
    ...
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.RECOLLECTIONS) == QUEST_ACCEPTED and player:getVar("recollectionsQuest") < 2) then
        if (trade:hasItemQty(1105,1) and trade:getItemCount() == 1) then
            player:startEvent(271);
        end
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_ROOT_OF_THE_PROBLEM) == QUEST_ACCEPTED and player:getVar("rootProblem") == 1) then
...
function onTrigger(player,npc)
    ...
    local theThreeMagi = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_THREE_MAGI);
    local recollections = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.RECOLLECTIONS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 260) then
        ...
        player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_THREE_MAGI);
    ...
    elseif (csid == 269) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.THE_THREE_MAGI);
        end
    elseif (csid == 270) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.RECOLLECTIONS);
    ...
    elseif (csid == 275) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.RECOLLECTIONS);
        end
    elseif (csid == 276) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_ROOT_OF_THE_PROBLEM);
    ...
    elseif (csid == 281) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.THE_ROOT_OF_THE_PROBLEM);
