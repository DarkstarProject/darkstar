...
--printf("RESULT: %u",option);
	...
	elseif(csid == 0x010d) then
		if(player:getFreeSlotsCount() == 0) then
			...
			if(choosetitle == 3) then
				player:addTitle(title.PROFESSOR_KORUMORU_SUPPORTER);
			elseif(choosetitle == 2) then
				player:addTitle(title.DOCTOR_SHANTOTTO_SUPPORTER);
			else
				player:addTitle(title.DOCTOR_YORANORAN_SUPPORTER);
			end
	...
	elseif(csid == 0x119) then
		if (player:getFreeSlotsCount() == 0) then
			...
			player:addTitle(title.PARAGON_OF_BLACK_MAGE_EXCELLENCE);
