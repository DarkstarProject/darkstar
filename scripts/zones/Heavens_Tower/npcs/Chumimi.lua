...
--printf("RESULT: %u",option);
	...
	elseif(csid == 0x010d) then
		if(player:getFreeSlotsCount() == 0) then
			...
			if(choosetitle == 3) then
				player:addTitle(TITLE_PROFESSOR_KORUMORU_SUPPORTER);
			elseif(choosetitle == 2) then
				player:addTitle(TITLE_DOCTOR_SHANTOTTO_SUPPORTER);
			else
				player:addTitle(TITLE_DOCTOR_YORANORAN_SUPPORTER);
			end
	...
	elseif(csid == 0x119) then
		if (player:getFreeSlotsCount() == 0) then
			...
			player:addTitle(TITLE_PARAGON_OF_BLACK_MAGE_EXCELLENCE);
