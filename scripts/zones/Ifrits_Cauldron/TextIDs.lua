-- Variable TextID   Description text

-- General Texts
   ITEM_CANNOT_BE_OBTAINED =  6380; -- You cannot obtain the item <item>. Come back after sorting your inventory.
FULL_INVENTORY_AFTER_TRADE =  6384; -- You cannot obtain the #. Try trading again after sorting your inventory.
             ITEM_OBTAINED =  6386; -- Obtained: <item>.
              GIL_OBTAINED =  6387; -- Obtained <number> gil.
          KEYITEM_OBTAINED =  6389; -- Obtained key item: <keyitem>.
            ITEMS_OBTAINED =  7271; -- You obtain
             HOMEPOINT_SET = 11505; -- Home point set!

-- Other dialog
   NOTHING_OUT_OF_ORDINARY =  7268; -- There is nothing out of the ordinary here.
   EGGSHELLS_LIE_SCATTERED =  7264; -- Eggshells lie scattered around the place...

-- Altar of Ashes Dialog (unable to find csid)
           ALTAR_COMPLETED =  7236; -- You have already made an offering today.
             ALTAR_INSPECT =  7237; -- This looks like the altar where offerings are to be placed.
            ALTAR_OFFERING =  7238; -- You place your offering of
            ALTAR_STANDARD =  7239; -- It is an altar for offerings.

-- Treasure Coffer/Chest Dialog
            CHEST_UNLOCKED =  7247; -- You unlock the chest!
                CHEST_FAIL =  7248; -- Fails to open the chest.
                CHEST_TRAP =  7249; -- The chest was trapped!
                CHEST_WEAK =  7250; -- You cannot open the chest when you are in a weakened state.
               CHEST_MIMIC =  7251; -- The chest was a mimic!
              CHEST_MOOGLE =  7252; -- You cannot open the chest while participating in the moogle event.
            CHEST_ILLUSION =  7253; -- The chest was but an illusion...
              CHEST_LOCKED =  7254; -- The chest appears to be locked.

-- Mining
   MINING_IS_POSSIBLE_HERE =  7255; -- Mining is possible here if you have

-- conquest Base
             CONQUEST_BASE =  7047; -- Tallying conquest results...

