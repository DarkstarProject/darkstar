...
function onTrade(player,npc,trade)
    ...
    if ((trade:hasItemQty(1053,1) or trade:hasItemQty(1115,1) or trade:hasItemQty(1023,1) or trade:hasItemQty(1022,1)) and trade:getItemCount() == 1) then
        ...
        if (player:hasKeyItem(dsp.kis.MAP_OF_IFRITS_CAULDRON) == false) then
            questItemNeeded = 3;
        end

        if (AFHandsActivated == 13 and player:hasKeyItem(dsp.kis.OLD_GAUNTLETS) == false) then
            questItemNeeded = 1;
        else
        ...
        if (success ~= -2) then
            ...
            if (math.random() <= success) then
                ...
                elseif (questItemNeeded == 3) then
                    player:addKeyItem(dsp.kis.MAP_OF_IFRITS_CAULDRON);
                    player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.MAP_OF_IFRITS_CAULDRON); -- Map of Ifrit's Cauldron (KI)
                else
