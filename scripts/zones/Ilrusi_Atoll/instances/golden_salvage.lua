...
function afterInstanceRegister(player)
    ...
    player:messageSpecial(IDs.text.ASSAULT_41_START, 41);
    player:messageSpecial(IDs.text.TIME_TO_COMPLETE, instance:getTimeLimit());
end;	
...
function onInstanceCreated(instance)
    ...
    for i,v in pairs(IDs.mobs[41]) do
        SpawnMob(v, instance);
    end
...
function onInstanceTimeUpdate(instance, elapsed)
    ...
    if (wipeTime == 0) then
        ...
        if (wipe) then
            for i,v in pairs(players) do
                v:messageSpecial(IDs.text.PARTY_FALLEN, 3);
            end
    ...
    if (message ~= 0) then
        for i,v in pairs(players) do
            if (timeRemaining >= 60) then
                v:messageSpecial(IDs.text.TIME_REMAINING_MINUTES, timeRemaining / 60);
            else
                v:messageSpecial(IDs.text.TIME_REMAINING_SECONDS, timeRemaining);
            end
        end
...
function onInstanceFailure(instance)
    ...
    for i,v in pairs(chars) do
        v:messageSpecial(IDs.text.MISSION_FAILED,10,10);
        v:startEvent(0x66);
    end
end;
...
function onInstanceComplete(instance)
    ...
    for i,v in pairs(chars) do
        v:messageSpecial(IDs.text.RUNE_UNLOCKED, 7, 8);
    end
    
    local rune = instance:getEntity(bit.band(IDs.npcs.RUNE_OF_RELEASE, 0xFFF), TYPE_NPC);
    local box = instance:getEntity(bit.band(IDs.npcs.ANCIENT_LOCKBOX, 0xFFF), TYPE_NPC);
