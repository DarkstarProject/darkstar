-- Variable TextID   Description text

-- General Texts
 ITEM_CANNOT_BE_OBTAINED = 6403; -- You cannot obtain the item <item>. Come back after sorting your inventory.
           ITEM_OBTAINED = 6409; -- Obtained: <item>.
            GIL_OBTAINED = 6410; -- Obtained <number> gil.
        KEYITEM_OBTAINED = 6411; -- Obtained key item: <keyitem>.
         BEASTMEN_BANNER = 7150; -- There is a beastmen's banner.
  FISHING_MESSAGE_OFFSET = 7702; -- You can't fish here.

-- Conquest
                CONQUEST = 8046; -- You've earned conquest points!

-- Logging
LOGGING_IS_POSSIBLE_HERE = 7895; -- Logging is possible here if you have

-- Other Dialog
         NOTHING_HAPPENS =  141; -- Nothing happens...


-- conquest Base
           CONQUEST_BASE = 7069; -- Tallying conquest results...

--chocobo digging
          DIG_THROW_AWAY = 7715; -- You dig up$, but your inventory is full. You regretfully throw the # away.
            FIND_NOTHING = 7717; -- You dig and you dig, but find nothing.
