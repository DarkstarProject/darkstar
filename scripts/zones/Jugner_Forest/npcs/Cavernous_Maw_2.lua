...
function onTrigger(player,npc)
    if (ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30) then
        ...
        and player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.THE_BEAST_OF_BASTORE) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    if (csid == 48) then
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.THE_BEAST_OF_BASTORE);
    elseif (csid == 49) then
        -- Killed Sedna
