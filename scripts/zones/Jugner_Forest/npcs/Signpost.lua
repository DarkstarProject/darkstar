...
function onTrigger(player,npc)
    ...
    if ((X > -79.3 and X < -67.3) and (Z > 94.5 and Z < 106.5)) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GRIMY_SIGNPOSTS) == QUEST_ACCEPTED and not player:getMaskBit(player:getVar("CleanSignPost"),0)) then
            player:startEvent(6,1);
        else
            player:startEvent(1);
        end
    elseif ((X > -266.2 and X < -254.2) and (Z > -29.2 and Z < -17.2)) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GRIMY_SIGNPOSTS) == QUEST_ACCEPTED and not player:getMaskBit(player:getVar("CleanSignPost"),1)) then
            player:startEvent(7,1);
        else
            player:startEvent(2);
        end
    elseif ((X > -463.7 and X < -451.7) and (Z > -422.1 and Z < -410.1)) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GRIMY_SIGNPOSTS) == QUEST_ACCEPTED and not player:getMaskBit(player:getVar("CleanSignPost"),2)) then
            player:startEvent(8,1);
        else
            player:startEvent(3);
        end
    elseif ((X > 295.4 and X < 307.3) and (Z > 412.8 and Z < 424.8)) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GRIMY_SIGNPOSTS) == QUEST_ACCEPTED and not player:getMaskBit(player:getVar("CleanSignPost"),3)) then
