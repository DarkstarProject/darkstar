...
function onTrade(player,npc,trade)
    ...
    if (trade:hasItemQty(1544,1) == true and player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_FIRE) == QUEST_ACCEPTED  and player:getMainJob() == dsp.job.SMN) then
...
function onTrigger(player,npc)
    local TrialSizeFire = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_FIRE);
...
function onEventFinish(player,csid,option)
    if (csid == 286 and option == 1) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_FIRE);
            player:addItem(1544);
