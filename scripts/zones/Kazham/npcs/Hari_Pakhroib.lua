...
function onTrigger(player,npc)
    Guardian = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.GREETINGS_TO_THE_GUARDIAN);
...
function onEventFinish(player,csid,option)
    if (csid == 68 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.GREETINGS_TO_THE_GUARDIAN);
    ...
    elseif (csid == 71) then
        if (Pamamas == 1) then --First completion of quest; set title, complete quest, and give higher fame
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.GREETINGS_TO_THE_GUARDIAN);
