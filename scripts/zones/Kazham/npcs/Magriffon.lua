...
function onTrigger(player,npc)
    ...
    local magriffonGilRequest = gulliblesTravelsStatus == 1 and  --Checks if Gullible's Travels is started and gets previously requested gil #
        player:getVar("MAGRIFFON_GIL_REQUEST");   
    if(gulliblesTravelsStatus == QUEST_AVAILABLE and player:getFameLevel(KAZHAM) >= 6) then -- Gullible's Travels: Fame check and quest available check  
