...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.GULLIBLES_TRAVELS) == QUEST_ACCEPTED) then
        if (trade:getGil() >= player:getVar("MAGRIFFON_GIL_REQUEST")) then
            player:startEvent(146);
        end
    elseif (player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.EVEN_MORE_GULLIBLES_TRAVELS) == QUEST_ACCEPTED and player:getVar("EVEN_MORE_GULLIBLES_PROGRESS") == 0) then
...
function onTrigger(player,npc)
    local gulliblesTravelsStatus = player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.GULLIBLES_TRAVELS);
    local evenmoreTravelsStatus = player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.EVEN_MORE_GULLIBLES_TRAVELS);
...
function onEventFinish(player,csid,option)
    if (csid == 144 and option == 1)  then                     -- Gullible's Travels: First CS
        player:addQuest(OUTLANDS, dsp.quests.ids.outlands.GULLIBLES_TRAVELS);
    elseif (csid == 146) then                                  -- Gullible's Travels: Final CS
        ...
        player:completeQuest(OUTLANDS, dsp.quests.ids.outlands.GULLIBLES_TRAVELS);
        player:needToZone(true);
    elseif (csid == 148 and option == 1) then                  -- Even More Guillible's Travels First CS
        player:addQuest(OUTLANDS, dsp.quests.ids.outlands.EVEN_MORE_GULLIBLES_TRAVELS);
    ...
    elseif (csid == 152) then
        ...
        player:addFame(KAZHAM, 30);
        player:completeQuest(OUTLANDS, dsp.quests.ids.outlands.EVEN_MORE_GULLIBLES_TRAVELS);
    end
end;
