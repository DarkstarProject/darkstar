...
function onTrade(player,npc,trade)
    ...
    -- 1147      Ancient Salt
    -- 4600      Lucky Egg
    local OpoOpoAndIStatus = player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.THE_OPO_OPO_AND_I);
...
function onEventFinish(player,csid,option,npc)
    ...
    if (csid == 241) then    -- correct trade, finished quest and receive opo opo crown and 3 pamamas
        ...
        if (FreeSlots >= 4) then
            ...
            player:completeQuest(OUTLANDS, dsp.quests.ids.outlands.THE_OPO_OPO_AND_I);
