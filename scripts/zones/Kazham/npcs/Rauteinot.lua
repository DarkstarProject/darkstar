...
function onTrigger(player,npc)
    MissionaryMan = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.MISSIONARY_MAN);
...
function onEventFinish(player,csid,option)
    if (csid == 137 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.MISSIONARY_MAN);
    ...
    elseif (csid == 141) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.MISSIONARY_MAN);
        end
    end
end;