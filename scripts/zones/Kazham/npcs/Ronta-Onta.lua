...
function onTrigger(player,npc)
    ...
    TrialByFire = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_FIRE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 270 and option == 1) then
        if (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_FIRE) == QUEST_COMPLETED) then
            player:delQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_FIRE);
        end
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_FIRE);
    ...
    elseif (csid == 273) then
        ...
        if (player:getFreeSlotsCount() == 0 and (option ~= 5 or option ~= 6)) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_FIRE);
        end
    end
