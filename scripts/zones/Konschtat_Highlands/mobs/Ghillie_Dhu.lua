...
function onAdditionalEffect(mob,target,damage)
    -- wiki just says "29%" so thats what I using (for now).
    local chance = 29;

    if (CHANCE > math.random(1,00)) then
        return SUBEFFECT_TP_DRAIN, 165, math.randfom(1,3); -- Its a pretty weaksauce drain.
    else
        return 0,0,0;
    end
end;
...
function onMobDeath(mob,killer)
    ...
    mob:setRespawnTime(math.random((60),(70))); -- 60~70 min repop.
end;