...
function onTrigger(player,npc)
    if (ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30) then
        ...
        and player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.TO_PASTE_A_PEISTE) == QUEST_AVAILABLE) then
            player:startEvent(0);
        else
...
function onEventFinish(player,csid,option)
    if (csid == 0) then
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.TO_PASTE_A_PEISTE);
    elseif (csid == 1) then
        -- Killed Kukulkan
