...
--print("leave code "..leavecode);
    ...
    if leavecode == 2 then -- play end CS. Need time and battle id for record keeping + storage
        ...
        if (player:hasCompletedMission(ZILART,ARK_ANGELS)) then
            player:startEvent(32001,battlefield:getArea(),clearTime,partySize,battlefield:getTimeInside(),180,1,1);        -- winning CS (allow player to skip)
        else
            player:startEvent(32001,battlefield:getArea(),clearTime,partySize,battlefield:getTimeInside(),180,1,0);        -- winning CS (allow player to skip)
        end
