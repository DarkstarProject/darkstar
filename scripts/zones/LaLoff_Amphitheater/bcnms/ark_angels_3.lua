...
print("leave code "..leavecode);
	...
	if(leavecode == 2) then -- play end CS. Need time and battle id for record keeping + storage
		...
		if(player:hasCompletedMission(ZILART,ARK_ANGELS)) then
			player:startEvent(0x7d01,1,1,1,instance:getTimeInside(),1,2,1);		-- winning CS (allow player to skip)
		else
			player:startEvent(0x7d01,1,1,1,instance:getTimeInside(),1,2,0);		-- winning CS
		end
