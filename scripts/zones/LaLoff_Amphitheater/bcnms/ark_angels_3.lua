...
function onEventFinish(player,csid,option)
 -- print("bc finish csid "..csid.." and option "..option);
   ...
   local AAKeyitems = (player:hasKeyItem(dsp.kis.SHARD_OF_APATHY) and player:hasKeyItem(dsp.kis.SHARD_OF_ARROGANCE)
         and player:hasKeyItem(dsp.kis.SHARD_OF_COWARDICE) and player:hasKeyItem(dsp.kis.SHARD_OF_RAGE));

    if (csid == 32001) then
      if (player:getCurrentMission(ZILART) == ARK_ANGELS  and player:getVar("ZilartStatus") == 1) then
         player:addKeyItem(dsp.kis.SHARD_OF_ENVY);
         player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.SHARD_OF_ENVY);
