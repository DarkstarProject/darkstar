...
function onEventFinish(player,csid,option)
 -- print("bc finish csid "..csid.." and option "..option);
   ...
   local AAKeyitems = (player:hasKeyItem(dsp.kis.SHARD_OF_APATHY) and player:hasKeyItem(dsp.kis.SHARD_OF_COWARDICE)
         and player:hasKeyItem(dsp.kis.SHARD_OF_ENVY) and player:hasKeyItem(dsp.kis.SHARD_OF_RAGE));

    if (csid == 32001) then
      if (player:getCurrentMission(ZILART) == ARK_ANGELS  and player:getVar("ZilartStatus") == 1) then
         player:addKeyItem(dsp.kis.SHARD_OF_ARROGANCE);
         player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.SHARD_OF_ARROGANCE);
   ...
   local AAKeyitems = (player:hasKeyItem(dsp.kis.SHARD_OF_APATHY) and player:hasKeyItem(dsp.kis.SHARD_OF_ARROGANCE)
         and player:hasKeyItem(dsp.kis.SHARD_OF_COWARDICE) and player:hasKeyItem(dsp.kis.SHARD_OF_ENVY)
         and player:hasKeyItem(dsp.kis.SHARD_OF_RAGE));
    if (AAKeyitems == true) then
      player:completeMission(ZILART,ARK_ANGELS);
      player:addMission(ZILART,THE_SEALED_SHRINE);
      player:setVar("ZilartStatus",0);
