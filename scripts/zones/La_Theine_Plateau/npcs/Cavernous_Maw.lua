...
function onTrigger(player,npc)
    if (ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30) then
        ...
        and player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.A_GOLDSTRUCK_GIGAS) == QUEST_AVAILABLE) then
            player:startEvent(9);
        else
...
function onEventFinish(player,csid,option)
    if (csid == 9) then
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.A_GOLDSTRUCK_GIGAS);
    elseif (csid == 10) then
        -- Killed Briareus
