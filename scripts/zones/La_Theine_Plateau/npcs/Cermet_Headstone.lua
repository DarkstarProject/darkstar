...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(ZILART) == HEADSTONE_PILGRIMAGE) then
        if (player:hasKeyItem(dsp.kis.WATER_FRAGMENT) == false) then
            player:startEvent(200,dsp.kis.WATER_FRAGMENT);
        ...
        elseif (player:hasKeyItem(dsp.kis.WATER_FRAGMENT) == true) then
            player:messageSpecial(ALREADY_OBTAINED_FRAG,dsp.kis.WATER_FRAGMENT);
        end
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 200 and option == 1) then
        player:addKeyItem(dsp.kis.WATER_FRAGMENT);
        ...
        if (player:hasKeyItem(dsp.kis.FIRE_FRAGMENT) and player:hasKeyItem(dsp.kis.EARTH_FRAGMENT) and player:hasKeyItem(dsp.kis.ICE_FRAGMENT) and
           player:hasKeyItem(dsp.kis.WIND_FRAGMENT) and player:hasKeyItem(dsp.kis.LIGHTNING_FRAGMENT) and player:hasKeyItem(dsp.kis.LIGHT_FRAGMENT)) then
            ...
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.WATER_FRAGMENT);
        end
    end
