...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.I_CAN_HEAR_A_RAINBOW) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    if (csid == 124) then
        ...
        player:completeQuest(WINDURST, dsp.quests.ids.windurst.I_CAN_HEAR_A_RAINBOW);
