...
function onTrigger(player,npc)
    if (player:hasKeyItem(WASHUS_TASTY_WURST) and not GetMobByID(UBUME):isSpawned()) then
        player:startEvent(0);
    elseif (player:getVar("yomiOkuriKilledNM") == 1 and not player:hasKeyItem(YOMOTSU_FEATHER)) then
        player:startEvent(1);
    else
