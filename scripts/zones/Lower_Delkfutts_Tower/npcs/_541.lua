...
function onTrigger(player, npc)
    ...
    if currentMission == dsp.mission.id.sandoria.APPOINTMENT_TO_JEUNO and player:getVar("MissionStatus") == 3 and not player:hasKeyItem(dsp.ki.DELKFUTT_KEY) then
        player:messageSpecial(ID.text.THE_DOOR_IS_FIRMLY_SHUT_OPEN_KEY)
    elseif currentMission == dsp.mission.id.sandoria.APPOINTMENT_TO_JEUNO and player:getVar("MissionStatus") == 3 and player:hasKeyItem(dsp.ki.DELKFUTT_KEY) then
        player:startEvent(0)
    else
