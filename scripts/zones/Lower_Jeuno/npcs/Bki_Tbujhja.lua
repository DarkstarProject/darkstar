...
function onTrade(player,npc,trade)
    local theRequiem = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_REQUIEM);
...
function onTrigger(player,npc)
    local aMinstrelInDespair = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.A_MINSTREL_IN_DESPAIR);
    local painfulMemory = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.PAINFUL_MEMORY);
    local theRequiem = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_REQUIEM);
...
function onEventFinish(player,csid,option)
    ...
    elseif ((csid == 145 or csid == 148) and option == 1) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.THE_REQUIEM);
    ...
    elseif (csid == 150) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.THE_REQUIEM);
        end;
    end;
end;
