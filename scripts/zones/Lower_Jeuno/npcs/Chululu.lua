...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.COLLECT_TARUT_CARDS) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    local CollectTarutCards = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.COLLECT_TARUT_CARDS);
    local RubbishDay = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.RUBBISH_DAY);
    local SearchingForTheRightWords = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.SEARCHING_FOR_THE_RIGHT_WORDS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 28 and option == 0) then
        ...
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(JEUNO,dsp.quests.ids.jeuno.COLLECT_TARUT_CARDS);
    ...
    elseif (csid == 200) then
        ...
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.COLLECT_TARUT_CARDS);
    ...
    elseif (csid == 198 and option == 0) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.RUBBISH_DAY);
    ...
    elseif (csid == 197) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addFame(JEUNO, 30);
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.RUBBISH_DAY);
        end
    end
end;