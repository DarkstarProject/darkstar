...
function onTrigger(player,npc)
    AirshipKI = player:hasKeyItem(dsp.kis.AIRSHIP_PASS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 230 and option == 10) then
        if (player:hasKeyItem(dsp.kis.AIRSHIP_PASS) == true) then
    ...
    elseif (csid == 231) then
        ...
        player:setVar("saveTheClockTowerNPCz1",0);
        player:setVar("saveTheClockTowerNPCz2",0);
        player:addTitle(dsp.titles.CLOCK_TOWER_PRESERVATIONIST);
