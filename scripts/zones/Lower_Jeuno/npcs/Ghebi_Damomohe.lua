...
function onTrade(player,npc,trade)
    ...
    elseif (player:hasKeyItem(dsp.ki.PSOXJA_PASS)==false and (trade:hasItemQty(1692,1) or trade:hasItemQty(1694,1) or trade:hasItemQty(1693,1)) and trade:getItemCount() == 1) then
...
function onTrigger(player,npc)
    ...
    local PsoXjaPass = player:hasKeyItem(dsp.ki.PSOXJA_PASS);
    ...
    elseif (player:hasKeyItem(dsp.ki.TENSHODO_APPLICATION_FORM) == true) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 107) then
        ...
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addTitle(dsp.title.TENSHODO_MEMBER);
            player:delKeyItem(dsp.ki.TENSHODO_APPLICATION_FORM);
            player:addKeyItem(dsp.ki.TENSHODO_MEMBERS_CARD);
            player:messageSpecial(KEYITEM_OBTAINED,dsp.ki.TENSHODO_MEMBERS_CARD);
            player:addItem(548);
