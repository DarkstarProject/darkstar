...
function onTrade(player,npc,trade)
    ...
    elseif (player:hasKeyItem(dsp.kis.PSOXJA_PASS)==false and (trade:hasItemQty(1692,1) or trade:hasItemQty(1694,1) or trade:hasItemQty(1693,1)) and trade:getItemCount() == 1) then
...
function onTrigger(player,npc)
    ...
    local PsoXjaPass = player:hasKeyItem(dsp.kis.PSOXJA_PASS);
    ...
    elseif (player:hasKeyItem(dsp.kis.TENSHODO_APPLICATION_FORM) == true) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 107) then
        ...
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addTitle(dsp.titles.TENSHODO_MEMBER);
            player:delKeyItem(dsp.kis.TENSHODO_APPLICATION_FORM);
            player:addKeyItem(dsp.kis.TENSHODO_MEMBERS_CARD);
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.TENSHODO_MEMBERS_CARD);
            player:addItem(548);
    ...
    elseif (csid == 52) then
        player:tradeComplete();
        player:addKeyItem(dsp.kis.PSOXJA_PASS);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.PSOXJA_PASS);
        player:addGil(500);
