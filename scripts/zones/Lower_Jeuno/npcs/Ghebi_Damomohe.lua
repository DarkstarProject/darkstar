...
function onTrade(player,npc,trade)
    if player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.TENSHODO_MEMBERSHIP) ~= QUEST_COMPLETED and npcUtil.tradeHas(trade, 548) then
...
function onTrigger(player,npc)
    ...
    if player:getFameLevel(JEUNO) >= 2 and player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.TENSHODO_MEMBERSHIP) == QUEST_AVAILABLE then
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 106 and option == 2 then
        player:addQuest(JEUNO, dsp.quests.ids.jeuno.TENSHODO_MEMBERSHIP)
    elseif csid == 107 then
        -- Finish Quest: Tenshodo Membership (Application Form)
        if npcUtil.completeQuest(player, JEUNO, dsp.quests.ids.jeuno.TENSHODO_MEMBERSHIP, { item=548, title=dsp.title.TENSHODO_MEMBER, keyItem=dsp.ki.TENSHODO_MEMBERS_CARD }) then
