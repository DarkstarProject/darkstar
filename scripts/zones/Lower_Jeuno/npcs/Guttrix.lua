...
function onTrigger(player,npc)
    ...
    local questStatus = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_GOBLIN_TAILOR);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10016) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.THE_GOBLIN_TAILOR);
    elseif (csid == 10018 and option >= 1 and option <= 4) then
        ...
        if (player:getFreeSlotsCount() < 1) then
            ...
            if (questStatus == QUEST_ACCEPTED) then
                ...
                player:completeQuest(JEUNO,dsp.quests.ids.jeuno.THE_GOBLIN_TAILOR);
            end
