...
function onTrigger(player,npc)
    ...
    if (player:hasKeyItem(dsp.kis.LETTER_FROM_THE_TENSHODO)) then
        player:startEvent(10021,0,dsp.kis.LETTER_FROM_THE_TENSHODO); -- During Quest "The Tenshodo Showdown"
    elseif (player:hasKeyItem(dsp.kis.SIGNED_ENVELOPE)) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10021) then
        ...
        player:delKeyItem(dsp.kis.LETTER_FROM_THE_TENSHODO);
        player:addKeyItem(dsp.kis.TENSHODO_ENVELOPE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.TENSHODO_ENVELOPE);
    elseif (csid == 10022) then
        if (player:getFreeSlotsCount() == 0 or player:hasItem(16764)) then
            ...
            player:delKeyItem(dsp.kis.SIGNED_ENVELOPE);
