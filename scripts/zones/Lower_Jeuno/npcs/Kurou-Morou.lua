...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.YOUR_CRYSTAL_BALL) == QUEST_ACCEPTED and trade:getItemCount() == 1) then
    ...
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.NEVER_TO_RETURN) == QUEST_ACCEPTED and trade:hasItemQty(12507,1) == true and trade:getItemCount() == 1) then
...
function onTrigger(player,npc)
    ...
    local YourCrystalBall = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.YOUR_CRYSTAL_BALL);
    local SearchingForTheRightWords = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.SEARCHING_FOR_THE_RIGHT_WORDS);
    local ACandlelightVigil = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.A_CANDLELIGHT_VIGIL);
    local RubbishDay = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.RUBBISH_DAY);
    local NeverToReturn = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.NEVER_TO_RETURN);
    ...
    elseif (JFame >= 5 and YourCrystalBall == QUEST_COMPLETED and player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.NEVER_TO_RETURN) == QUEST_AVAILABLE and player:getVar("QuestNeverToReturn_day") ~= VanadielDayOfTheYear()) then
...
function onEventFinish(player,csid,option)
    if (csid == 194 and option == 0) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.YOUR_CRYSTAL_BALL);
    elseif (csid == 196) then
        player:addTitle(dsp.title.FORTUNETELLER_IN_TRAINING);
        player:addFame(JEUNO, 30);
        player:tradeComplete(trade);
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.YOUR_CRYSTAL_BALL);
    ...
    elseif (csid == 202 and option == 0) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.NEVER_TO_RETURN);
    ...
    elseif (csid == 203) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addFame(JEUNO, 30);
            player:tradeComplete(trade);
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.NEVER_TO_RETURN);
        end
    elseif (csid == 17) then
