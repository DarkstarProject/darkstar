...
function onTrigger(player,npc)
    local TheWonderMagicSet = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_WONDER_MAGIC_SET);
    local CooksPride = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.COOK_S_PRIDE);
    local TheKindCardian = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_KIND_CARDIAN);
    local WildcatJeuno = player:getVar("WildcatJeuno");

    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.LURE_OF_THE_WILDCAT_JEUNO) == QUEST_ACCEPTED and player:getMaskBit(WildcatJeuno,13) == false) then
...
function onEventFinish(player,csid,option)
    if ((csid == 189 or csid == 188) and option == 0) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.COOK_S_PRIDE);
    ...
    elseif (csid == 187) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.COOK_S_PRIDE);
        end
