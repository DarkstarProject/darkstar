...
function onTrigger(player,npc)
    TheWonderMagicSet = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_WONDER_MAGIC_SET);
    WonderMagicSetKI = player:hasKeyItem(dsp.ki.WONDER_MAGIC_SET);
    TheLostCardianCS = player:getVar("theLostCardianVar");
    TheKindCardian = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_KIND_CARDIAN);
    ...
    elseif (TheWonderMagicSet == QUEST_COMPLETED and player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.COOK_S_PRIDE) ~= QUEST_COMPLETED) then
        player:startEvent(40); -- Standard dialog
    elseif (TheWonderMagicSet == QUEST_COMPLETED and player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_LOST_CARDIAN) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    if (csid == 77 and option == 1) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.THE_WONDER_MAGIC_SET);
    elseif (csid == 33) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.THE_WONDER_MAGIC_SET);
        end
    elseif (csid == 30) then
    ...
    elseif (csid == 35) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addFame(JEUNO, 30);
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.THE_KIND_CARDIAN);
        end
    end
end;
