...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_WONDER_MAGIC_SET) == QUEST_AVAILABLE) then
        player:startEvent(34); -- Base Standard CS & dialog
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.COOK_S_PRIDE) ~= QUEST_COMPLETED) then
    ...
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_LOST_CARDIAN) == QUEST_AVAILABLE) then
