...
function onTrade(player,npc,trade)
    FistfulOfFury = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.FISTFUL_OF_FURY);
...
function onTrigger(player,npc)
    FistfulOfFury = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.FISTFUL_OF_FURY);
    BeatAroundTheBushin = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.BEAT_AROUND_THE_BUSHIN);
...
function onEventFinish(player,csid,option)
    if (csid == 216 and option == 1) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.FISTFUL_OF_FURY);
    elseif (csid == 213) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.FISTFUL_OF_FURY);
        end
    elseif (csid == 160 and player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.BEAT_AROUND_THE_BUSHIN) == QUEST_AVAILABLE) then
