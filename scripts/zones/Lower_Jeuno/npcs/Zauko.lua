...
function onTrigger(player,npc)
    ...
    local doneCommService = (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.COMMUNITY_SERVICE) == QUEST_COMPLETED) and 1 or 0;
...
function onEventUpdate(player,csid,option)
    if (csid == 116 and option == 0) then
        ...
        if (playerOnQuestId == 0 and (hour >= 18 or hour < 1)) then
            ...
            player:addQuest(JEUNO,dsp.quests.ids.jeuno.COMMUNITY_SERVICE);
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 117) then
        ...
        if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.COMMUNITY_SERVICE) ~= QUEST_COMPLETED) then
            -- first victory
            params.fame = 30;
        else
        ...
        npcUtil.completeQuest(player, JEUNO, dsp.quests.ids.jeuno.COMMUNITY_SERVICE, params);
