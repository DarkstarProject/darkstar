...
function onTrigger(player,npc)
    ...
    local ANewDawn = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.A_NEW_DAWN);
    ...
    -- Save My Son
    elseif (player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.SAVE_MY_SON) == QUEST_AVAILABLE and mLvl >= 30) then
        player:startEvent(164);
    elseif (player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.SAVE_MY_SON) == QUEST_ACCEPTED) then
        if (SaveMySon == 0) then
            player:startEvent(229);
        elseif (SaveMySon == 1) then
            player:startEvent(163);
        end
    elseif (player:needToZone() == false and player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.SAVE_MY_SON) == QUEST_COMPLETED and SaveMySon == 2) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 164 and option == 0) then
        player:addQuest(JEUNO, dsp.quests.ids.jeuno.SAVE_MY_SON);
    elseif (csid == 163) then
        if (player:getFreeSlotsCount(0) >= 1) then
            ...
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.SAVE_MY_SON);
        else
    ...
    elseif (csid == 5) then
        ...
        if (option == 1) then
            player:addQuest(JEUNO, dsp.quests.ids.jeuno.A_NEW_DAWN);
