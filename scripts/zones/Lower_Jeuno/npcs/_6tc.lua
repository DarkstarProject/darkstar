...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.BEAT_AROUND_THE_BUSHIN) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 155) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.BEAT_AROUND_THE_BUSHIN);
    ...
    elseif (csid == 159) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:tradeComplete();
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.BEAT_AROUND_THE_BUSHIN);
        end
    end
end;