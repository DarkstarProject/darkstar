...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(WINDURST,ECO_WARRIOR_WIN) ~= QUEST_AVAILABLE and player:getVar("ECO_WARRIOR_ACTIVE") == 238) then
        ...
        elseif (player:hasStatusEffect(EFFECT.LEVEL_RESTRICTION) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 62 and option == 1) then
        player:addStatusEffect(EFFECT.LEVEL_RESTRICTION,20,0,0);
    elseif (csid == 65) then
        player:setVar("ECOR_WAR_WIN-NMs_killed",0);
        player:delStatusEffect(EFFECT.LEVEL_RESTRICTION);
    elseif (csid == 64) then
        player:delStatusEffect(EFFECT.LEVEL_RESTRICTION);
    end
end;
