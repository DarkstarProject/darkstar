...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.TRUE_STRENGTH) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    local trueStrength = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.TRUE_STRENGTH);
    local WildcatBastok = player:getVar("WildcatBastok");

    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK) == QUEST_ACCEPTED and player:getMaskBit(WildcatBastok,9) == false) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 748) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.TRUE_STRENGTH);
    elseif (csid == 749) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.TRUE_STRENGTH);
        end
