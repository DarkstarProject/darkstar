...
function onTrigger(player,npc)
    ...
    local CidsSecret = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.CID_S_SECRET)
    ...
    elseif (player:getMainJob() == dsp.job.DRK and player:getMainLvl() >= AF2_QUEST_LEVEL and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.DARK_LEGACY) == QUEST_COMPLETED and player:getQuestStatus(BASTOK,DARK_PUPPET) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 760) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.DARK_PUPPET)
    ...
    elseif (csid == 507) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.CID_S_SECRET)
    elseif (csid == 509) then
        if (player:getFreeSlotsCount(0) >= 1) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.CID_S_SECRET)
        else
