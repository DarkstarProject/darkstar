...
function onTrigger(player,npc)
    ...
    local BeaSmog = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BEADEAUX_SMOG);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 731) then
            player:addQuest(BASTOK,dsp.quests.ids.bastok.BEADEAUX_SMOG);
    elseif (csid == 732) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.BEADEAUX_SMOG);
