...
function onTrigger(player,npc)
    ...
    elseif (player:getVar("Flagbastok") == 1) then
        if (player:getFreeSlotsCount() == 0) then 
            player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED,182);
        else
            player:setVar("Flagbastok",0);
            player:addItem(182);
            player:messageSpecial(text.ITEM_OBTAINED,182);
        end
    ...
    elseif (currentMission == THE_CHAINS_THAT_BIND_US) and (missionStatus == 2) then
        player:showText(npc, text.8596); -- Dialogue after first cutscene
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 0x030e) then
        ...
        player:messageSpecial(text.GIL_OBTAINED,GIL_RATE*100000);
        player:setTitle(296);
