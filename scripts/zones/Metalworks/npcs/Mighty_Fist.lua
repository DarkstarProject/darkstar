...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_DARKSMITH) ~= QUEST_AVAILABLE) then
...
function onTrigger(player,npc)
    ...
    elseif (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_DARKSMITH) == QUEST_AVAILABLE and player:getFameLevel(BASTOK) >= 3) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 565) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_DARKSMITH);
    elseif (csid == 566) then
        TheDarksmith = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_DARKSMITH);
        ...
        if (TheDarksmith == QUEST_ACCEPTED) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.THE_DARKSMITH);
        else
