...
function onTrade(player,npc,trade)
    ...
    local MeanMachine = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.MEAN_MACHINE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 556) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.MEAN_MACHINE);
    elseif (csid == 557) then
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.MEAN_MACHINE);
