...
function onTrigger(player,npc)
    ...
    local TheSandCharm = player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.THE_SAND_CHARM);
...
function onEventFinish(player,csid,option)
    if (csid == 125) then
        player:addQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.THE_SAND_CHARM);
