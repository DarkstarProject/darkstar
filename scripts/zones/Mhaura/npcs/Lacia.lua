...
function onTrade(player,npc,trade)
    if (trade:hasItemQty(1548,1) == true and player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_SIZE_TRIAL_BY_LIGHTNING) == QUEST_ACCEPTED and player:getMainJob() == dsp.job.SMN) then
...
function onTrigger(player,npc)
    ...
    local TrialSizeLightning = player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_SIZE_TRIAL_BY_LIGHTNING);
...
function onEventFinish(player,csid,option)
    if (csid == 10025 and option == 1) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_SIZE_TRIAL_BY_LIGHTNING);
            player:addItem(1548);
