...
function onTrade(player,npc,trade)
    if player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.A_POTTER_S_PREFERENCE) == QUEST_ACCEPTED or player:getVar("QuestAPotterPrefeRepeat_var") == 1 then
...
function onTrigger(player,npc)
    if player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.A_POTTER_S_PREFERENCE) == QUEST_AVAILABLE and player:getFameLevel(WINDURST) > 5 then
        player:startEvent(111,569) -- start quest A Potter's Preference
    elseif player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.A_POTTER_S_PREFERENCE) == QUEST_ACCEPTED then
        player:startEvent(114,569) -- get me dish_of_gusgen_clay  as soon as you can
    elseif player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.A_POTTER_S_PREFERENCE) == QUEST_COMPLETED then
...
function onEventFinish(player,csid,option)
    if csid == 111 and option == 1 then  --accept quest
        player:addQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.A_POTTER_S_PREFERENCE)
    elseif csid == 113 then --quest completed
        ...
        player:completeQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.A_POTTER_S_PREFERENCE)
