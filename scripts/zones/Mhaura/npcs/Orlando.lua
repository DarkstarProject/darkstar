...
function onTrade(player,npc,trade)
    local QuestStatus = player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ORLANDO_S_ANTIQUES);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 101) then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ORLANDO_S_ANTIQUES);
    elseif (csid == 102) then
        player:tradeComplete();
        player:addFame(WINDURST,10);
        player:addGil(payout);
        player:messageSpecial(ID.text.GIL_OBTAINED,payout);
        player:completeQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ORLANDO_S_ANTIQUES);
