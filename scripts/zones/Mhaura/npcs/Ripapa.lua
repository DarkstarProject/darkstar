...
function onTrigger(player,npc)
    ...
    local TrialByLightning = player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_BY_LIGHTNING);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10016 and option == 1) then
        if (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_BY_LIGHTNING) == QUEST_COMPLETED) then
            player:delQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_BY_LIGHTNING);
        end
        player:addQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_BY_LIGHTNING);
    ...
    elseif (csid == 10019) then
        ...
        if (player:getFreeSlotsCount() == 0 and (option ~= 5 or option ~= 6)) then
            ...
            player:completeQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.TRIAL_BY_LIGHTNING);
        end
