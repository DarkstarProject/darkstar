...
if (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.RYCHARDE_THE_CHEF)==QUEST_AVAILABLE) then -- if available and allready talked to mayor assistant
...
elseif (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.EXPERTISE)==QUEST_AVAILABLE and player:getQuestStatus(OTHER_AREAS_LOG,HIS_NAME_IS_VALGEIR )==QUEST_COMPLETED) then --
    player:startEvent(61);-- accept expertice quest


elseif (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.EXPERTISE)==QUEST_ACCEPTED) then --
...
elseif (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.EXPERTISE)==QUEST_COMPLETED and player:getQuestStatus(OTHER_AREAS_LOG,THE_CLUE)==QUEST_AVAILABLE) then --
    player:startEvent(64); -- after expertice quest
elseif (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.THE_CLUE)==QUEST_ACCEPTED) then--
    player:startEvent(65); -- good luck
elseif (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.THE_CLUE)==QUEST_COMPLETED and player:getQuestStatus(OTHER_AREAS_LOG,THE_BASICS)==QUEST_AVAILABLE) then --
    player:startEvent(66);-- Valgeir cook was delicious
elseif (player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.THE_BASICS)==QUEST_COMPLETED) then--
    player:startEvent(67);-- after back to basics i think
else
    player:startEvent(59); -- talk abaout something else
end
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 61) then  -- accept quest EXPERTICE
        player:addQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.EXPERTISE);
    elseif (csid == 62) then   -- end quest expertice
        ...
        if (player:getFreeSlotsCount() < 1) then
            ...
            player:completeQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.EXPERTISE);
        end
    end
end;
