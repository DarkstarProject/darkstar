...
function onTrade(player,npc,trade)
    ...
    local questStatus = player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.THE_OLD_LADY);
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ELDER_MEMORIES) ~= QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 131 and option == 40) then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.THE_OLD_LADY);
    ...
    elseif (csid == 137) then
        ...
        player:completeQuest(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.THE_OLD_LADY);
    end
