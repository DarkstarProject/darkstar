...
function onRegionEnter(player, region)
    ...
    if regionId == 8 and player:getQuestStatus(BASTOK, dsp.quests.ids.bastok.BLADE_OF_EVIL) == QUEST_ACCEPTED and player:getVar("bladeOfEvilCS") == 1 then
...
function onEventFinish(player, csid, option)
    ...
    -- BLADE OF EVIL
    elseif csid == 14 and option == 0 and npcUtil.completeQuest(player, BASTOK, dsp.quests.ids.bastok.BLADE_OF_EVIL, {item=12516, title=dsp.title.PARAGON_OF_DARK_KNIGHT_EXCELLENCE, fame=AF3_FAME}) then
