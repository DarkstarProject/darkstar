...
function onTrigger(player,npc)
    ...
    if (circleOfTime == QUEST_ACCEPTED and player:hasKeyItem(dsp.kis.STAR_RING1) and player:hasKeyItem(dsp.kis.MOON_RING)) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 3) then
        ...
        player:delKeyItem(dsp.kis.MOON_RING);
        player:delKeyItem(dsp.kis.STAR_RING1);
    end;
end;
