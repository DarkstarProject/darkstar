...
function onMobFight(mob, target)
    ...
    if (mob:hasStatusEffect(EFFECT.CHAINSPELL) == true) then
        mob:setTP(2000);
    else
    ...
    if (mob:hasStatusEffect(EFFECT.BLAZE_SPIKES) == true) then
...
function onAdditionalEffect(mob, player)
    ...
    if (math.random(0,99) >= chance or resist <= 0.5) then
        ...
        if (player:hasStatusEffect(EFFECT.POISON) == false) then
            player:addStatusEffect(EFFECT.POISON, 50, 3, duration); -- Don't know potency on the poison.
        end
