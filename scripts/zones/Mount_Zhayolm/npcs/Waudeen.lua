...
function onTrigger(player,npc)
    ...
    if (toauMission == IMMORTAL_SENTRIES) then
        if (player:hasKeyItem(dsp.kis.SUPPLIES_PACKAGE)) then
            player:startEvent(4);
    ...
    elseif (beginnings == QUEST_ACCEPTED) then
        if (not player:hasKeyItem(dsp.kis.BRAND_OF_THE_FLAMESERPENT)) then
    ...
    elseif (toauMission >= PRESIDENT_SALAHEEM) then
        ...
        if (player:hasKeyItem(dsp.kis.LEBROS_ASSAULT_ORDERS) and player:hasKeyItem(dsp.kis.ASSAULT_ARMBAND) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 4 and option == 1) then
        player:delKeyItem(dsp.kis.SUPPLIES_PACKAGE);
        player:setVar("AhtUrganStatus",1);

    -- BEGINNINGS
    elseif (csid == 10) then
        player:addKeyItem(dsp.kis.BRAND_OF_THE_FLAMESERPENT);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.BRAND_OF_THE_FLAMESERPENT);

    -- ASSAULT
    elseif (csid == 209 and option == 1) then
        player:delCurrency("imperial_standing", 50);
        player:addKeyItem(dsp.kis.ASSAULT_ARMBAND);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.ASSAULT_ARMBAND);
    end;
end;
