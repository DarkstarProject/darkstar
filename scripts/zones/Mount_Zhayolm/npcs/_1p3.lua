...
function onTrigger(player,npc)
    if (player:hasKeyItem(dsp.kis.LEBROS_ASSAULT_ORDERS)) then
...
function onEventUpdate(player,csid,option,target)
    ...
    if (party ~= nil) then
        for i,v in ipairs(party) do
            if (not (v:hasKeyItem(dsp.kis.LEBROS_ASSAULT_ORDERS) and v:getCurrentAssault() == assaultid)) then
        ...
        if (party ~= nil) then
            for i,v in ipairs(party) do
                if v:getID() ~= player:getID() and v:getZoneID() == player:getZoneID() then
                    ...
                    v:startEvent(208, 2);
                    v:delKeyItem(dsp.kis.LEBROS_ASSAULT_ORDERS);
                end
            end
        end
    else
