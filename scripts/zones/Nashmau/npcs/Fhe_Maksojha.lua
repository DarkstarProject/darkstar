...
function onTrigger(player,npc)
    local notmeanttobe = player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.NOT_MEANT_TO_BE);
...
function onEventFinish(player,csid,option)
    if (csid == 293) then
        ...
        player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.NOT_MEANT_TO_BE);
    ...
    elseif (csid == 297) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.NOT_MEANT_TO_BE);
        end
    end
end;
