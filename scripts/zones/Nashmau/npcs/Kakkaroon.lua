...
function onTrigger(player,npc)
    local ratrace = player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.RAT_RACE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 308) then
        ...
        player:addQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.RAT_RACE);
    elseif (csid == 312) then
        if (player:getFreeSlotsCount() <= 2) then
            ...
            player:completeQuest(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.RAT_RACE);
        end
    end
end;
