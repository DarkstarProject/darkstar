...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.NAVIGATING_THE_UNFRIENDLY_SEAS) == QUEST_ACCEPTED and player:getVar("NavigatingtheUnfriendlySeas") <= 2) then
...
function onTrigger(player,npc)
    if (player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.AGAINST_ALL_ODDS) >= QUEST_ACCEPTED) then
