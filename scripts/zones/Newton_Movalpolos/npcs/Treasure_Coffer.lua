...
function onTrade(player,npc,trade)
    ...
    if ((trade:hasItemQty(1063,1) or trade:hasItemQty(1115,1) or trade:hasItemQty(1023,1) or trade:hasItemQty(1022,1)) and trade:getItemCount() == 1) then
        ...
        if (player:hasKeyItem(dsp.ki.MAP_OF_NEWTON_MOVALPOLOS) == false) then
            questItemNeeded = 3;
        end
        --------------------------------------
        ...
        if (success ~= -2) then
            ...
            if (math.random() <= success) then
                ...
                if (questItemNeeded == 3) then
                    player:addKeyItem(dsp.ki.MAP_OF_NEWTON_MOVALPOLOS);
                    player:messageSpecial(ID.text.KEYITEM_OBTAINED,dsp.ki.MAP_OF_NEWTON_MOVALPOLOS); -- Map of Newton Movalpolos (KI)
                else
...
function onTrigger(player,npc)
    player:messageSpecial(ID.text.CHEST_LOCKED,1063);
end;
