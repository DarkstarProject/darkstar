...
function onTrigger(player,npc)
    ...
    local TrialByWater = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WATER);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 109 and option == 1) then
        if (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WATER) == QUEST_COMPLETED) then
            player:delQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WATER);
        end
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WATER);
    ...
    elseif (csid == 112) then
        ...
        if (player:getFreeSlotsCount() == 0 and (option ~= 5 or option ~= 6)) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WATER);
        end
    end
