...
function onTrade(player,npc,trade)
    ...
    ShiningSubligar = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.LIKE_A_SHINING_SUBLIGAR);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 123) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.LIKE_A_SHINING_SUBLIGAR);
    elseif (csid == 125) then
        ...
        player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.LIKE_A_SHINING_SUBLIGAR);
    end
