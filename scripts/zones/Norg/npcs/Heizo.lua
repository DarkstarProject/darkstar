...
function onTrade(player,npc,trade)
    ...
    ShiningLeggings = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.LIKE_A_SHINING_LEGGINGS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 127) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.LIKE_A_SHINING_LEGGINGS);
    elseif (csid == 129) then
        ...
        player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.LIKE_A_SHINING_LEGGINGS);
    end
