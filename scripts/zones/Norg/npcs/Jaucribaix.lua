...
function onTrade(player,npc,trade)
    ...
    elseif (player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.FORGE_YOUR_DESTINY) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, {1152, 1153})) then -- Bomb Steel, Sacred Branch
        player:startEvent(27)
    elseif (player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.THE_SACRED_KATANA) == QUEST_ACCEPTED and player:hasKeyItem(dsp.ki.HANDFUL_OF_CRYSTAL_SCALES) and npcUtil.tradeHas(trade, 17809)) then -- Mumeito
        player:startEvent(141)
    elseif (player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.A_THIEF_IN_NORG) == QUEST_ACCEPTED and player:hasKeyItem(dsp.ki.CHARRED_HELM) and npcUtil.tradeHas(trade, 823)) then -- Gold Thread
        player:startEvent(162)
    end
end

function onTrigger(player,npc)
    local forgeYourDestiny  = player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.FORGE_YOUR_DESTINY)
    local theSacredKatana   = player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.THE_SACRED_KATANA)
    local yomiOkuri         = player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.YOMI_OKURI)
    local aThiefinNorg      = player:getQuestStatus(OUTLANDS, dsp.quests.ids.outlands.A_THIEF_IN_NORG)
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 25 and option == 1) then
        player:addQuest(OUTLANDS, dsp.quests.ids.outlands.FORGE_YOUR_DESTINY)
    elseif (csid == 27) then
        player:confirmTrade()
        player:setVar("ForgeYourDestiny_timer", os.time() + 10368) -- 3 game days
    elseif (csid == 29 and npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.FORGE_YOUR_DESTINY, {item=17809, fame=30, fameArea=NORG, title=dsp.title.BUSHIDO_BLADE, var={"ForgeYourDestiny_timer", "ForgeYourDestiny_Event"}})) then -- Mumeito
    ...
    elseif (csid == 139 and option == 1) then
        player:addQuest(OUTLANDS, dsp.quests.ids.outlands.THE_SACRED_KATANA)
    elseif (csid == 141 and npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.THE_SACRED_KATANA, {item=17812, fame=AF1_FAME, fameArea=NORG})) then -- Magoroku
        player:confirmTrade()
    ...
    elseif (csid == 146 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.YOMI_OKURI)
    ...
    elseif (csid == 156 and npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.YOMI_OKURI, {item=14100, fame=AF2_FAME, fameArea=NORG, var="yomiOkuriCS"})) then -- Myochin Sune-Ate
        player:delKeyItem(dsp.ki.FADED_YOMOTSU_HIRASAKA)
        player:needToZone(true)

    -- A THIEF IN NORG
    elseif (csid == 158 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.A_THIEF_IN_NORG)
    ...
    elseif (csid == 164) then
        npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.A_THIEF_IN_NORG, {item=13868, title=dsp.title.PARAGON_OF_SAMURAI_EXCELLENCE, fame=AF3_FAME, fameArea=NORG, var={"aThiefinNorgCS"}})
