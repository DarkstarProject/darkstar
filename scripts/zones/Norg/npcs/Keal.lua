...
function onTrigger(player,npc)
    ...
    local Vault = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.ITS_NOT_YOUR_VAULT);
...
function onEventFinish(player,csid,option,npc)
    if (csid == 36 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.ITS_NOT_YOUR_VAULT);
    elseif (csid == 38) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.ITS_NOT_YOUR_VAULT);
        end
    end
