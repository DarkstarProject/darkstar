...
function onTrigger(player,npc)
    Stash = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.THE_SAHAGINS_STASH);
...
function onEventFinish(player,csid,option)
    if (csid == 33 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.THE_SAHAGINS_STASH);
    elseif (csid == 35) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.THE_SAHAGINS_STASH);
        end
    end
