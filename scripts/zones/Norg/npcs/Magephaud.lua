...
function onTrade(player,npc,trade)
    EveryonesGrudge = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.EVERYONES_GRUDGE);
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.EVERYONES_GRUDGE) == QUEST_AVAILABLE and player:getVar("EVERYONES_GRUDGE_KILLS") >= 1 and nFame >= 2) then
    ...
    elseif (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.EVERYONES_GRUDGE) == QUEST_COMPLETED) then
...
function onEventFinish(player,csid,option)
    if (csid == 116) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.EVERYONES_GRUDGE);
        player:setVar("EveryonesGrudgeStarted",1);
    elseif (csid == 118) then
        player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.EVERYONES_GRUDGE);
