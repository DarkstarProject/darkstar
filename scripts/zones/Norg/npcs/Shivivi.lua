...
function onTrigger(player,npc)
    DampScroll = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.SECRET_OF_THE_DAMP_SCROLL);
...
function onEventFinish(player,csid,option,npc)
    if (csid == 31) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.SECRET_OF_THE_DAMP_SCROLL);
    end
