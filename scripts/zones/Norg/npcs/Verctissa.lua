...
function onTrade(player,npc,trade)
    ...
    if (trade:hasItemQty(1549,1) and player:getQuestStatus(OUTLANDS,TRIAL_SIZE_TRIAL_BY_WATER) == QUEST_ACCEPTED and player:getMainJob() == dsp.jobs.SMN) then
...
function onTrigger(player,npc)
    ...
    if (player:getMainLvl() >= 20 and player:getMainJob() == dsp.jobs.SMN and TrialSizeWater == QUEST_AVAILABLE and player:getFameLevel(NORG) >= 2) then --Requires player to be Summoner at least lvl 20
