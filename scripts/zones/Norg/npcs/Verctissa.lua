...
function onTrade(player,npc,trade)
    ...
    if (trade:hasItemQty(1549,1) and player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_WATER) == QUEST_ACCEPTED and player:getMainJob() == dsp.job.SMN) then
...
function onTrigger(player,npc)
    ...
    local TrialSizeWater = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_WATER);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 199 and option == 1) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_WATER);
            player:addItem(1549);
