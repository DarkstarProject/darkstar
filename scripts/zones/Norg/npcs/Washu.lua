...
function onTrade(player,npc,trade)
    -- YOMI OKURI (SAM AF2)
    if (
        player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.YOMI_OKURI) == QUEST_ACCEPTED and
...
function onTrigger(player,npc)
    local stopYourWhining = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.STOP_YOUR_WHINING);

    -- YOMI OKURI (SAM AF2)
    if (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.YOMI_OKURI) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 21 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.STOP_YOUR_WHINING);
        npcUtil.giveKeyItem(player, dsp.ki.EMPTY_BARREL);
    elseif (csid == 23 and npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.STOP_YOUR_WHINING, {item=4952, fame=75, fameArea=NORG, title=dsp.title.APPRENTICE_SOMMELIER})) then -- Scroll of Hojo: Ichi
