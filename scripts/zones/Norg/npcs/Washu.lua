...
function onTrade(player,npc,trade)
    ...
    if (
        ...
        not player:hasKeyItem(WASHUS_TASTY_WURST) and
        not player:hasKeyItem(YOMOTSU_FEATHER) and
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(OUTLANDS,YOMI_OKURI) == QUEST_ACCEPTED) then
        ...
        elseif (player:hasKeyItem(WASHUS_TASTY_WURST)) then
            player:startEvent(151); -- remind objective
        elseif (player:getVar("yomiOkuriKilledNM") == 0 and not player:hasKeyItem(WASHUS_TASTY_WURST)) then
    ...
    elseif (stopYourWhining == QUEST_ACCEPTED and player:hasKeyItem(EMPTY_BARREL)) then
        player:startEvent(22); -- remind objective
    elseif (stopYourWhining == QUEST_ACCEPTED and player:hasKeyItem(BARREL_OF_OPOOPO_BREW)) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 23 and npcUtil.completeQuest(player, OUTLANDS, STOP_YOUR_WHINING, {item=4952, fame=75, fameArea=NORG, title=APPRENTICE_SOMMELIER})) then -- Scroll of Hojo: Ichi
        player:delKeyItem(BARREL_OF_OPOOPO_BREW);
    end
end;
