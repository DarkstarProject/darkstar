...
function onTrade(player,npc,trade)
    ...
    if (
        ...
        not player:hasKeyItem(dsp.kis.WASHUS_TASTY_WURST) and
        not player:hasKeyItem(dsp.kis.YOMOTSU_FEATHER) and
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(OUTLANDS,YOMI_OKURI) == QUEST_ACCEPTED) then
        ...
        elseif (player:hasKeyItem(dsp.kis.WASHUS_TASTY_WURST)) then
            player:startEvent(151); -- remind objective
        elseif (player:getVar("yomiOkuriKilledNM") == 0 and not player:hasKeyItem(dsp.kis.WASHUS_TASTY_WURST)) then
    ...
    elseif (stopYourWhining == QUEST_ACCEPTED and player:hasKeyItem(dsp.kis.EMPTY_BARREL)) then
        player:startEvent(22); -- remind objective
    elseif (stopYourWhining == QUEST_ACCEPTED and player:hasKeyItem(dsp.kis.BARREL_OF_OPOOPO_BREW)) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 150) then
        player:confirmTrade();
        npcUtil.giveKeyItem(player, dsp.kis.WASHUS_TASTY_WURST);
