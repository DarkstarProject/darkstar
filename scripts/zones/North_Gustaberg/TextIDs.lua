-- Variable TextID   Description text

-- General Texts
ITEM_CANNOT_BE_OBTAINED_TWICE = 6560; -- You cannot obtain the item <item>.
      ITEM_CANNOT_BE_OBTAINED = 6561; -- You cannot obtain the item <item>. Come back after sorting your inventory.
   FULL_INVENTORY_AFTER_TRADE = 6565; -- You cannot obtain the #. Try trading again after sorting your inventory.
                ITEM_OBTAINED = 6567; -- Obtained: <item>.
                 GIL_OBTAINED = 6568; -- Obtained <number> gil.
             KEYITEM_OBTAINED = 6570; -- Obtained key item: <keyitem>.
               ITEMS_OBTAINED = 6573; -- You obtain
       FISHING_MESSAGE_OFFSET = 7228; -- You can't fish here.

-- Conquest
CONQUEST = 7479; -- You've earned conquest points!

-- Quests
SHINING_OBJECT_SLIPS_AWAY = 7436; -- The shining object slips through your fingers and is washed further down the stream.

-- Other Dialog
        NOTHING_HAPPENS = 300;  -- Nothing happens...
NOTHING_OUT_OF_ORDINARY = 6581; -- There is nothing out of the ordinary here.
  REACH_WATER_FROM_HERE = 7443; -- You can reach the water from here.

-- conquest Base
CONQUEST_BASE = 0;

--chocobo digging
DIG_THROW_AWAY = 7241; -- You dig up$, but your inventory is full. You regretfully throw the # away.
FIND_NOTHING = 7243; -- You dig and you dig, but find nothing.
