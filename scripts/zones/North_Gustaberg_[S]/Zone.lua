...
function onZoneIn(player,prevZone)
    ...
    if (prevZone == dsp.zone.BASTOK_MARKETS_S and player:getCampaignAllegiance() > 0 and player:getQuestStatus(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BETTER_PART_OF_VALOR) == QUEST_AVAILABLE) then
        cs = 1;
    end
    return cs;
end;
...
function onEventFinish(player,csid,option)
    if (csid == 1) then
        player:addQuest(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BETTER_PART_OF_VALOR);
