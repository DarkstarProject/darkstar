...
function onTrade(player,npc,trade)
    ...
    theTraderInTheForest = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_TRADER_IN_THE_FOREST);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 524 and option == 0 or csid == 592 and option == 0) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_TRADER_IN_THE_FOREST);
    ...
    elseif (csid == 525) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_TRADER_IN_THE_FOREST);
        end
    -- "The Medicine Woman" Quest
    elseif (csid == 613 and option == 0 or csid == 615 and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_MEDICINE_WOMAN);
    elseif (csid == 614) then
        ...
        player:addFame(SANDORIA,30);
        player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_MEDICINE_WOMAN);
    end
