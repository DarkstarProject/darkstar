...
function onTrigger(player,npc)
    sermonQuest = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_VICASQUE_S_SERMON);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 600) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_VICASQUE_S_SERMON );
        end
    elseif (csid == 589) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_VICASQUE_S_SERMON );
