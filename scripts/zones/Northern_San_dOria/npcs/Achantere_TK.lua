...
local guardnation = dsp.conquest.NATION_SANDORIA; -- SANDORIA, BASTOK, WINDURST, JEUNO
local guardtype   = 1;        -- 1: city, 2: foreign, 3: outpost, 4: border
local size        = #dsp.conquest.SANDORIA_INVENTORY;
local inventory   = dsp.conquest.SANDORIA_INVENTORY;
...
function onTrigger(player,npc)
    ...
    if (player:getNation() == guardnation and player:getVar("supplyQuest_started") > 0 and dsp.conquest.supplyRunFresh(player) == 0) then
        ...
        player:startEvent(32762,Menu1,Menu2,Menu3,Menu4,Menu5,Menu6,Menu7,Menu8);
    end
