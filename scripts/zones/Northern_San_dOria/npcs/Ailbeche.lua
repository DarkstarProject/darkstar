...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.FATHER_AND_SON) == QUEST_COMPLETED and player:getVar("returnedAilbecheRod") ~= 1) then
...
function onTrigger(player,npc)
    fatherAndSon = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.FATHER_AND_SON);
    sharpeningTheSword = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.SHARPENING_THE_SWORD);
    aBoysDream = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.A_BOY_S_DREAM);
...
function onEventFinish(player,csid,option)
    ...
    -- "Father and Son"
    if (csid == 508) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.FATHER_AND_SON);
    elseif (csid == 509) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.FATHER_AND_SON);
        end
    elseif (csid == 61) then
    ...
    elseif ((csid == 45 or csid == 43) and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.SHARPENING_THE_SWORD);
    ...
    elseif (csid == 44) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addFame(SANDORIA,30);
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.SHARPENING_THE_SWORD);
        end
    -- "A Boy's Dream"
    elseif ((csid == 41 or csid == 40) and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.A_BOY_S_DREAM);
