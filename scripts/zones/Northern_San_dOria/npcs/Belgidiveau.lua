...
function onTrigger(player,npc)
    ...
    troubleAtTheSluice = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.TROUBLE_AT_THE_SLUICE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 57 and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.TROUBLE_AT_THE_SLUICE);
    ...
    elseif (csid == 56) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.TROUBLE_AT_THE_SLUICE);
        end
    end
