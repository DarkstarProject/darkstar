...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.FLYERS_FOR_REGINE) == QUEST_ACCEPTED and trade:hasItemQty(532,1) and count == 1) then
        player:messageSpecial(ID.text.FLYER_REFUSED);
    elseif (trade:hasItemQty(1545,1) and player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.TRIAL_SIZE_TRIAL_BY_ICE) == QUEST_ACCEPTED and player:getMainJob() == dsp.job.SMN and count == 1) then -- Trade mini fork of ice
...
function onTrigger(player,npc)
    ...
    local TrialSizeByIce = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.TRIAL_SIZE_TRIAL_BY_ICE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 733 and option == 1) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.TRIAL_SIZE_TRIAL_BY_ICE);
            player:addItem(1545);
