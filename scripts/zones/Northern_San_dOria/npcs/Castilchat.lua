...
function onTrade(player,npc,trade)
    ...
    elseif (trade:hasItemQty(1545,1) and player:getQuestStatus(SANDORIA,TRIAL_SIZE_TRIAL_BY_ICE) == QUEST_ACCEPTED and player:getMainJob() == dsp.jobs.SMN and count == 1) then -- Trade mini fork of ice
...
function onTrigger(player,npc)
    ...
    if (player:getMainLvl() >= 20 and player:getMainJob() == dsp.jobs.SMN and TrialSizeByIce == QUEST_AVAILABLE and player:getFameLevel(SANDORIA) >= 2) then -- Requires player to be Summoner at least lvl 20
