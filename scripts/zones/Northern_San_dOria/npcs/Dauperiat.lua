...
function onTrade(player,npc,trade)
    ...
    local Black = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.BLACKMAIL);
...
function onTrigger(player,npc)
    ...
    local blackMail = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.BLACKMAIL);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 643) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.BLACKMAIL);
    ...
    elseif (csid == 648) then
        ...
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.BLACKMAIL) == QUEST_ACCEPTED) then
            player:addFame(SANDORIA,30);
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.BLACKMAIL);
        else
