...
function onTrigger(player,npc)
	...
	if(AltanaSorrow == QUEST_ACCEPTED and player:hasKeyItem(LETTER_FROM_VIRNAGE) == true) then
		player:startEvent(0x02a7); -- Finish quest "Altana's Sorrow"
	elseif(ActingInGoodFaith == QUEST_ACCEPTED and player:hasKeyItem(GANTINEUXS_LETTER) == true) then
	...
	elseif(HealingTheLand == QUEST_ACCEPTED and player:hasKeyItem(SEAL_OF_BANISHING) == true) then
	...
	elseif(HealingTheLand == QUEST_COMPLETED and SorceryOfTheNorth == QUEST_AVAILABLE and player:needToZone() == true) then
	...
	elseif(SorceryOfTheNorth == QUEST_ACCEPTED and player:hasKeyItem(FEIYIN_MAGIC_TOME) == true) then
...
--printf("RESULT: %u",option);
	...
	elseif(csid == 0x02a8) then
		if (player:getFreeSlotsCount() == 0) then 
			player:messageSpecial(ITEM_CANNOT_BE_OBTAINED,SCROLL_OF_TELEPORT_MEA);
		else
			player:setTitle(PILGRIM_TO_MEA);
			player:delKeyItem(GANTINEUXS_LETTER);
			player:addItem(SCROLL_OF_TELEPORT_MEA);
			player:messageSpecial(ITEM_OBTAINED,SCROLL_OF_TELEPORT_MEA);
	...
	elseif(csid == 0x02ab) then
		if (player:getFreeSlotsCount() == 0) then 
			player:messageSpecial(ITEM_CANNOT_BE_OBTAINED,SCROLL_OF_TELEPORT_HOLLA);
		else
			player:setTitle(PILGRIM_TO_HOLLA);
			player:addItem(SCROLL_OF_TELEPORT_HOLLA);
			player:messageSpecial(ITEM_OBTAINED,SCROLL_OF_TELEPORT_HOLLA);
	...
	elseif(csid == 0x02af) then 
		if (player:getFreeSlotsCount() == 0) then 
			player:messageSpecial(ITEM_CANNOT_BE_OBTAINED,SCROLL_OF_TELEPORT_VAHZL);
		else
			player:delKeyItem(FEIYIN_MAGIC_TOME);
			player:addItem(SCROLL_OF_TELEPORT_VAHZL);
			player:messageSpecial(ITEM_OBTAINED,SCROLL_OF_TELEPORT_VAHZL);
			player:addFame(SANDORIA,SAN_FAME*30);
