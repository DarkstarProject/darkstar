...
function onEventFinish(player,csid,option)
    ...
    if (csid == 706 and option == 1) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.TRIAL_BY_ICE) == QUEST_COMPLETED) then
            player:delQuest(SANDORIA,dsp.quests.ids.sandoria.TRIAL_BY_ICE);
        end
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.TRIAL_BY_ICE);
    ...
    elseif (csid == 709) then
        ...
        if (player:getFreeSlotsCount() == 0 and (option ~= 5 or option ~= 6)) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.TRIAL_BY_ICE);
        end
