...
itemQuality = 0;
    ...
    GrowingFlowers = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GROWING_FLOWERS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 605 and option == 1002) then
        ...
        player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.GROWING_FLOWERS);
        player:addFame(SANDORIA,120);
        player:moghouseFlag(1);
        player:messageSpecial(ID.text.MOGHOUSE_EXIT);
    elseif (csid == 605 and option == 1) then
        player:tradeComplete();
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.GROWING_FLOWERS);
    end
end;