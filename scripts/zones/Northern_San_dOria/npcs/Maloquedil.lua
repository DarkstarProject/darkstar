...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.WARDING_VAMPIRES) ~= QUEST_AVAILABLE) then
        if (trade:hasItemQty(1018,2) and trade:getItemCount() == 2) then -- Trade Shaman Garlic
            player:startEvent(23);
        end
    end

    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.RIDING_ON_THE_CLOUDS) == QUEST_ACCEPTED and player:getVar("ridingOnTheClouds_1") == 4) then
...
function onTrigger(player,npc)
    ...
    local warding = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.WARDING_VAMPIRES);
    local WildcatSandy = player:getVar("WildcatSandy");

    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.LURE_OF_THE_WILDCAT_SAN_D_ORIA) == QUEST_ACCEPTED and player:getMaskBit(WildcatSandy,7) == false) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 24 and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.WARDING_VAMPIRES);
    elseif (csid == 23) then
        player:tradeComplete();
        player:addTitle(dsp.title.VAMPIRE_HUNTER_DMINUS);
        player:addGil(GIL_RATE*900);
        player:messageSpecial(ID.text.GIL_OBTAINED,GIL_RATE*900);
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.WARDING_VAMPIRES) == QUEST_ACCEPTED) then
            player:addFame(SANDORIA,30);
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.WARDING_VAMPIRES);
        else
