...
function onTrigger(player,npc)
    ...
    local aCraftsmansWork = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.A_CRAFTSMAN_S_WORK);
...
function onEventFinish(player,csid,option)
    ...
    elseif ((csid == 73 or csid == 71) and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.A_CRAFTSMAN_S_WORK);
    ...
    elseif (csid == 70) then -- This is only if player has Altepa Polishing Stone
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.A_CRAFTSMAN_S_WORK);
        end
    elseif (csid == 67) then
