...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.MESSENGER_FROM_BEYOND) == QUEST_ACCEPTED) then
    ...
    elseif (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.PRELUDE_OF_BLACK_AND_WHITE) == QUEST_ACCEPTED) then
    ...
    elseif (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.PIEUJE_S_DECISION) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    messengerFromBeyond = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.MESSENGER_FROM_BEYOND);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 689) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.MESSENGER_FROM_BEYOND);
    elseif (csid == 690) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.MESSENGER_FROM_BEYOND);
        end
    elseif (csid == 691) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.PRELUDE_OF_BLACK_AND_WHITE);
        end
    elseif (csid == 692) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.PIEUJE_S_DECISION);
        end
    end
