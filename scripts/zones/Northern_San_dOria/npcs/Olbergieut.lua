...
function onTrigger(player,npc)
    ...
    gates = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GATES_TO_PARADISE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 619 and option == 0) then
        player:addQuest(SANDORIA, dsp.quests.ids.sandoria.GATES_TO_PARADISE);
    ...
    elseif (csid == 620) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.GATES_TO_PARADISE);
