...
function onTrigger(player,npc)
    ...
    elseif (gates == QUEST_ACCEPTED) then
        player:showText(npc, text.OLBERGIEUT_DIALOG, SCRIPTURE_OF_WIND);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 0x026b and option == 0) then 
        ...
        player:messageSpecial(text.KEYITEM_OBTAINED, SCRIPTURE_OF_WIND);
    elseif (csid == 0x026c) then
        if (player:getFreeSlotsCount() == 0) then
            player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED, 13584);
        else
            ...
            player:messageSpecial(text.ITEM_OBTAINED,13584);
        end;
    end;
