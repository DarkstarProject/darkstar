...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.UNDYING_FLAMES) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    undyingFlames = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.UNDYING_FLAMES);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 562 and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.UNDYING_FLAMES);
    elseif (csid == 563) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.UNDYING_FLAMES);
        end
    elseif (csid == 37) then
        player:tradeComplete();
