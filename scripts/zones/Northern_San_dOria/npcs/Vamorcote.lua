...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_SETTING_SUN) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    theSettingSun = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_SETTING_SUN);

    if (theSettingSun == QUEST_AVAILABLE  and
        player:getFameLevel(SANDORIA) >= 5 and
        player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.BLACKMAIL) ~= QUEST_COMPLETED)
    then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 654 and option == 1) then --Player accepts the quest
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_SETTING_SUN);
    elseif (csid == 658) then --The player trades the Engraved Key to the NPC. Here come the rewards!
        ...
        player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_SETTING_SUN);
    end;
end;
