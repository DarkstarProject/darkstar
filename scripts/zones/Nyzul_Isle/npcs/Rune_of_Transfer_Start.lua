...
local npc = instance:getEntity(bit.band(ID.npc.RUNE_TRANSFER_START, 0xFFF), dsp.objType.NPC)
    ...
    if csid == 94 then
        for k, v in pairs(nyzul.floorCost) do
            if k == option then
                if player:getCurrency("nyzul_isle_assault_point") >= v.cost then
                    player:delCurrency("nyzul_isle_assault_point", v.cost)
                    npc:setLocalVar("[Nyzul_Isle]StartingFloor", v.level)
                    npc:setLocalVar("[Nyzul_Isle]Leader", player:getID())
                    npc:setLocalVar("[Nyzul_Isle]FloorLayout", math.random(1,16))
                    npc:setLocalVar("[Nyzul_Current_Floor", v.level)
                
                    printf("layout %s", npc:getLocalVar("[Nyzul_Isle]FloorLayout"))
