...
function onTrigger(player,npc)
	...
	if(player:getCurrentMission(WINDURST) == THE_HEART_OF_THE_MATTER) then
		...
		if(windurst_mission_1_2 == 2) then
			-- Entered a Dark Orb
			if(player:getVar("windy_miss_1_2_orb6") == 1) then
				player:startEvent(0x33);
				-- Set the progress
				player:setVar("windy_miss_1_2_orb6",2);
			else
				player:messageSpecial(ORB_ALREADY_PLACED);
			end
		elseif(windurst_mission_1_2 == 5) then
			-- Took out a Glowing Orb
			if(player:getVar("windy_miss_1_2_orb6") == 2) then
				player:startEvent(0x33);
				-- Set the progress
				player:setVar("windy_miss_1_2_orb6",3);
			else
...
--printf("RESULT: %u",option);
	...
	-- Entered an orb
	if(csid == 0x33) then
		orb_value = player:getVar("windy_miss_1_2_orb6");
		if(orb_value == 2) then
