...
function onTrade(player,npc,trade)
    if (trade:getGil() == 300 and trade:getItemCount() == 1 and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK) == QUEST_COMPLETED and player:getCurrentMission(TOAU) > IMMORTAL_SENTRIES) then
...
function onTrigger(player,npc)
    local LureBastok = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK);
...
function onEventFinish(player,csid,option)
    if (csid == 357) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK);
    ...
    elseif (csid == 360) then
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK);
