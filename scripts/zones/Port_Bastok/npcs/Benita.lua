...
BombAsh = trade:hasItemQty(928,1);
    ...
    if (count == 1 and BombAsh == true) then
        TheWisdom = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_WISDOM_OF_ELDERS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 174) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_WISDOM_OF_ELDERS);
        player:setVar("TheWisdomVar",1);
    elseif (csid == 176) then
        if (player:getFreeSlotsCount() == 0) then
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED,12500);
        else
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.THE_WISDOM_OF_ELDERS);
