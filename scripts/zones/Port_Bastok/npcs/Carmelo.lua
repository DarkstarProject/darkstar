...
function onTrigger(player,npc)
    local SirensTear = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_SIREN_S_TEAR);
    local SirensTearProgress = player:getVar("SirensTear");
    local TheStarsOfIfrit = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_STARS_OF_IFRIT);
    local LoveAndIce = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LOVE_AND_ICE);
    local LoveAndIceProgress = player:getVar("LoveAndIceProgress");
    local ATestOfTrueLove = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.A_TEST_OF_TRUE_LOVE);
    local ATestOfTrueLoveProgress = player:getVar("ATestOfTrueLoveProgress");
    local LoversInTheDusk = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LOVERS_IN_THE_DUSK);
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 185) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.LOVE_AND_ICE);
    ...
    elseif (csid == 186) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.LOVE_AND_ICE);
        end
    elseif (csid == 270) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.A_TEST_OF_TRUE_LOVE);
    ...
    elseif (csid == 274) then
        ...
        player:needToZone(true);
        player:addFame(BASTOK,120);
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.A_TEST_OF_TRUE_LOVE);
    elseif (csid == 275) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.LOVERS_IN_THE_DUSK);
