...
function onTrigger(player,npc)
    ...
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.BEAUTY_AND_THE_GALKA)) then
        BastokQuests = BastokQuests - 2;         -- Beauty and the Galka.
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.WELCOME_TO_BASTOK)) then
        BastokQuests = BastokQuests - 4;         -- Welcome to Bastok.
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.GUEST_OF_HAUTEUR)) then
        BastokQuests = BastokQuests - 8;         -- Guest of Hauteur.
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.CID_S_SECRET)) then
        BastokQuests = BastokQuests - 16;        -- Cid's Secret.
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.THE_USUAL)) then
        BastokQuests = BastokQuests - 32;        -- The Usual.
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.LOVE_AND_ICE)) then
        BastokQuests = BastokQuests - 64;        -- Love and Ice(pt.1).
        BastokQuests = BastokQuests - 128;     -- Love and Ice(pt.2).
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.A_TEST_OF_TRUE_LOVE)) then
    ...
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.LOVERS_IN_THE_DUSK)) then
        BastokQuests = BastokQuests - 2048;      -- Lovers in the Dusk
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.GHOSTS_OF_THE_PAST)) then
    ...
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.AYAME_AND_KAEDE)) then
...
-- *Need to determine the correct csid/appropriate options for this cutscene
    --if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.TRIAL_BY_EARTH)) then
    --    BastokQuests = BastokQuests - 2097152;   -- Trial by Earth.
    --end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.THE_WALLS_OF_YOUR_MIND)) then
    ...
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.FADED_PROMISES)) then
        BastokQuests = BastokQuests - 33554432;  -- Faded Promises.
    end
    if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.OUT_OF_THE_DEPTHS)) then
...
-- *Need the correct csids
    -- if (player:hasCompletedQuest(BASTOK,dsp.quests.ids.bastok.CHASING_DREAMS)) then
