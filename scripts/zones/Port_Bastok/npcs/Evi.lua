...
function onTrigger(player,npc)
    ...
    PastPerfect = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.PAST_PERFECT);
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 130) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.PAST_PERFECT);
    elseif (csid == 131) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            if (player:addItem(12560)) then
                ...
                player:completeQuest(BASTOK,dsp.quests.ids.bastok.PAST_PERFECT);
            end
        end
    end
