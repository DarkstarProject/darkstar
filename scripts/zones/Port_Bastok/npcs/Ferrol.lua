...
function onTrade(player,npc,trade)
    ...
    if (trade:hasItemQty(1547,1) and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.TRIAL_SIZE_TRIAL_BY_EARTH) == QUEST_ACCEPTED and player:getMainJob() == dsp.job.SMN) then
...
function onTrigger(player,npc)
    ...
    local TrialSizeEarth = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.TRIAL_SIZE_TRIAL_BY_EARTH);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 297 and option == 1) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(BASTOK,dsp.quests.ids.bastok.TRIAL_SIZE_TRIAL_BY_EARTH);
            player:addItem(1547);
