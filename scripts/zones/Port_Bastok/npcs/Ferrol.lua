...
function onTrade(player,npc,trade)
    ...
    if (trade:hasItemQty(1547,1) and player:getQuestStatus(BASTOK,TRIAL_SIZE_TRIAL_BY_EARTH) == QUEST_ACCEPTED and player:getMainJob() == dsp.jobs.SMN) then
...
function onTrigger(player,npc)
    ...
    if (player:getMainLvl() >= 20 and player:getMainJob() == dsp.jobs.SMN and TrialSizeEarth == QUEST_AVAILABLE and player:getFameLevel(BASTOK) >= 2) then -- Requires player to be Summoner at least lvl 20
