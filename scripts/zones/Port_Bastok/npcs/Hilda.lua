...
function onTrade(player,npc,trade)
    ...
    if (trade:getGil() == 0 and trade:getItemCount() == 1) then
        ...
        elseif (trade:hasItemQty(4386,1) and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_USUAL) == QUEST_ACCEPTED) then -- Trade King Truffle
            player:startEvent(135);
        end
    end

    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.RIDING_ON_THE_CLOUDS) == QUEST_ACCEPTED and player:getVar("ridingOnTheClouds_2") == 5) then
...
function onTrigger(player,npc)
    ...
    elseif (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK) == QUEST_ACCEPTED and player:getMaskBit(WildcatBastok,3) == false) then
        player:startEvent(356);
    elseif (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_USUAL) ~= QUEST_COMPLETED) then
        if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.CID_S_SECRET) == QUEST_ACCEPTED) then
            player:startEvent(132);
            if (player:getVar("CidsSecret_Event") ~= 1) then
                player:setVar("CidsSecret_Event",1);
            end
        elseif (player:getFameLevel(BASTOK) >= 5 and player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.CID_S_SECRET) == QUEST_COMPLETED) then
            if (player:getVar("TheUsual_Event") == 1) then
                player:startEvent(136);
            elseif (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_USUAL) == QUEST_ACCEPTED) then
    ...
    elseif (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_USUAL) == QUEST_COMPLETED and player:getQuestStatus(BASTOK,CID_S_SECRET) == QUEST_COMPLETED) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 134 and option == 0) then
        if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_USUAL) == QUEST_AVAILABLE) then
            player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_USUAL);
        end
    ...
    elseif (csid == 136) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.THE_USUAL);
        end
