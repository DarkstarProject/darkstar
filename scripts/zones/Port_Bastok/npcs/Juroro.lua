...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 249 and option == 1) then
        if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.TRIAL_BY_EARTH) == QUEST_COMPLETED) then
            player:delQuest(BASTOK,dsp.quests.ids.bastok.TRIAL_BY_EARTH);
        end
        player:addQuest(BASTOK,dsp.quests.ids.bastok.TRIAL_BY_EARTH);
    ...
    elseif (csid == 252) then
        ...
        if (player:getFreeSlotsCount() == 0 and (option ~= 5 or option ~= 6)) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.TRIAL_BY_EARTH);
        end
    end
