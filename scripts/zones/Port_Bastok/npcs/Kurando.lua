...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.FEAR_OF_FLYING) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    local FearofFlying = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.FEAR_OF_FLYING);
    -- csid 173 ?
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 170) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.FEAR_OF_FLYING);
    ...
    elseif (csid == 171) then
        if    (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.FEAR_OF_FLYING);
