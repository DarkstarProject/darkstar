...
function onTrade(player,npc,trade)
    ...
    elseif (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.GHOSTS_OF_THE_PAST) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    local ghostsOfThePast = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.GHOSTS_OF_THE_PAST)
    local theFirstMeeting = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.THE_FIRST_MEETING)
...
function onEventFinish(player,csid,option)
    if (csid == 231) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.GHOSTS_OF_THE_PAST)
    elseif (csid == 232) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.GHOSTS_OF_THE_PAST)
        end
    elseif (csid == 233) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.THE_FIRST_MEETING)
    elseif (csid == 234) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.THE_FIRST_MEETING)
        end
    else
