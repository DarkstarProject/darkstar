...
function onTrade(player,npc,trade)
    local SilenceOfTheRams = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.SILENCE_OF_THE_RAMS);
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.LURE_OF_THE_WILDCAT_BASTOK) == QUEST_ACCEPTED and player:getMaskBit(WildcatBastok,2) == false) then
...
function onEventFinish(player,csid,option)
    if (csid == 195) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.SILENCE_OF_THE_RAMS);
    elseif (csid == 196) then
        player:tradeComplete();
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.SILENCE_OF_THE_RAMS);
