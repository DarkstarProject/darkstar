...
function onTrigger(player,npc)
    ...
    local WelcometoBastok = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.WELCOME_TO_BASTOK);
    local GuestofHauteur = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.GUEST_OF_HAUTEUR);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 50 and option == 0) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.WELCOME_TO_BASTOK);
    elseif (csid == 53) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.WELCOME_TO_BASTOK);
        end
    elseif (csid == 55 and option == 0) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.GUEST_OF_HAUTEUR);
    elseif (csid == 58) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addFame(BASTOK,80);
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.GUEST_OF_HAUTEUR);
        end
    end
