...
function onTrigger(player,npc)
    ...
    local ForevertoHold = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.FOREVER_TO_HOLD);
    local TilldeathdousPart = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.TILL_DEATH_DO_US_PART);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 128) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.TILL_DEATH_DO_US_PART);
    elseif (csid == 129) then
        ...
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.TILL_DEATH_DO_US_PART);
    end
