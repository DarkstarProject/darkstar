...
itemQuality = 0;
    ...
    ALadysHeart = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.A_LADY_S_HEART);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 160 and option == 2002) then
        ...
        player:completeQuest(BASTOK,dsp.quests.ids.bastok.A_LADY_S_HEART);
        player:addFame(BASTOK,120);
        player:moghouseFlag(2);
        player:messageSpecial(ID.text.MOGHOUSE_EXIT);
    elseif (csid == 160 and option == 1) then
        player:tradeComplete();
        player:addQuest(BASTOK,dsp.quests.ids.bastok.A_LADY_S_HEART);
    end
end;