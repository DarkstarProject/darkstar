...
function onTrade(player,npc,trade)
    ...
    BiteDust = player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BITE_THE_DUST);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 191) then
        player:addQuest(BASTOK,dsp.quests.ids.bastok.BITE_THE_DUST);
    elseif (csid == 193) then
        if (player:getQuestStatus(BASTOK,dsp.quests.ids.bastok.BITE_THE_DUST) == QUEST_ACCEPTED) then
            player:addTitle(dsp.title.SAND_BLASTER)
            player:addFame(BASTOK,120);
            player:completeQuest(BASTOK,dsp.quests.ids.bastok.BITE_THE_DUST);
        else
