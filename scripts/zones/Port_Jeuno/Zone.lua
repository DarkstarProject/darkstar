...
function onZoneIn(player,prevZone)
    ...
    and player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.A_JOURNEY_BEGINS) == QUEST_AVAILABLE) then
        cs = 324;
    end
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 324) then
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.A_JOURNEY_BEGINS);
    end
end;