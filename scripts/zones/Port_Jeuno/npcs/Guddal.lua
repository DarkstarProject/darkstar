...
function onTrade(player,npc,trade)
    if (player:hasKeyItem(dsp.kis.AIRSHIP_PASS_FOR_KAZHAM) == false) then
...
function onEventUpdate(player,csid,option)
    ...
    if (csid == 300) then
        if (player:delGil(148000)) then
            player:addKeyItem(dsp.kis.AIRSHIP_PASS_FOR_KAZHAM);
...
function onEventFinish(player,csid,option)
    ...
    -- printf("RESULT: %u",option);
    if (csid == 300) then
        if (player:hasKeyItem(dsp.kis.AIRSHIP_PASS_FOR_KAZHAM) == true) then
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.AIRSHIP_PASS_FOR_KAZHAM);
        end
