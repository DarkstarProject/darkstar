...
function onTrade(player,npc,trade)
    local theAntiqueCollector = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_ANTIQUE_COLLECTOR);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 13 and option == 1) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.THE_ANTIQUE_COLLECTOR);
    elseif (csid == 15) then
        ...
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.THE_ANTIQUE_COLLECTOR);
