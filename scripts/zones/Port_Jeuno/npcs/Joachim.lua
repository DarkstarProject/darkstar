...
-- TODO: logic to increase traverser stone count...Based on time between 2 vars?
    ...
    if (player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.A_JOURNEY_BEGINS) == QUEST_ACCEPTED) then
        player:startEvent(325);
    elseif (player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.THE_TRUTH_BECKONS) == QUEST_ACCEPTED and player:getVar("1stTimeAbyssea") == 1) then
        player:startEvent(327,0,0,MaxKI); -- cs for "The Truth Beckons" completion
    elseif (player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.THE_TRUTH_BECKONS) ~= QUEST_COMPLETED) then
        player:startEvent(326); -- Pre "The Truth Beckons" Menu
    elseif (player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.DAWN_OF_DEATH) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    if (csid == 325) then
        ...
        player:completeQuest(ABYSSEA, dsp.quests.ids.abyssea.A_JOURNEY_BEGINS);
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.THE_TRUTH_BECKONS);
    elseif (csid == 327) then
        player:completeQuest(ABYSSEA, dsp.quests.ids.abyssea.THE_TRUTH_BECKONS);
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.DAWN_OF_DEATH);
