...
function onTrade(player,npc,trade)
    ...
    local LastEquipmentKey = player:getCharVar("LastEquipmentKey"); -- When last Crimson key was obtained
    ...
    if (ENABLE_ACP == 0 and ENABLE_AMK == 0 and ENABLE_ASA ==0) then
        ...
        if (ENABLE_ACP == 1 and sLux and sLuna and sAstrum and count == 3 and ACPm >= dsp.mission.id.acp.GATHERER_OF_LIGHT_I and CrimsonKey == false and now ~= LastEquipmentKey) then -- and timer stuff here) then
            player:tradeComplete();
            player:addKeyItem(dsp.ki.CRIMSON_KEY);
            player:setCharVar("LastEquipmentKey", os.date("%j"));
            player:messageSpecial(ID.text.DRYEYES_2);
            player:messageSpecial(ID.text.KEYITEM_OBTAINED,dsp.ki.CRIMSON_KEY);
        elseif (sLux and sLuna and sAstrum and count == 3 and (now == LastEquipmentKey or CrimsonKey == true)) then
...
function onEventFinish(player,csid,option)
    ...
    local LastEquipmentKey = player:getCharVar("LastEquipmentKey"); -- When last Viridian key was obtained

    if (csid == 323) then
        if (option == 1) then
            player:showText(player,ID.text.DRYEYES_1);
        elseif (option == 100) then
            if (salad and juice and grub and ACPm >= dsp.mission.id.acp.GATHERER_OF_LIGHT_I and ViridianKey == false and now ~= LastEquipmentKey) then
            ...
            elseif (now == LastEquipmentKey or ViridianKey == true) then
