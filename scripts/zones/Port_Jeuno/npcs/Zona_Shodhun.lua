...
function onTrade(player,npc,trade)
    ...
    local PrettyLittleThings = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.PRETTY_LITTLE_THINGS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10023 and option == 4002) then
        ...
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.PRETTY_LITTLE_THINGS);
    elseif (csid == 10023 and option == 1) then
        player:tradeComplete();
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.PRETTY_LITTLE_THINGS);
    end
end;
