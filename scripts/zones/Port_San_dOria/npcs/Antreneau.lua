...
function onTrigger(player,npc)
    ...
    local aTasteForMeat = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.A_TASTE_FOR_MEAT)
    ...
    if player:getVar("aTasteForMeat") == 0 and aTasteForMeat == QUEST_COMPLETED and fameLevel >= 8 and medicineWoman == QUEST_COMPLETED and diaryPage >= 4 then
        local overTheHillsAndFarAway = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.OVER_THE_HILLS_AND_FAR_AWAY)
...
function onEventFinish(player,csid,option)
    ...
    if csid == 725 then
        player:addQuest(SANDORIA, dsp.quests.ids.sandoria.OVER_THE_HILLS_AND_FAR_AWAY)
    else
        if csid == 527 then
