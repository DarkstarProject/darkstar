...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.CHASING_QUOTAS) == QUEST_ACCEPTED and player:getVar("ChasingQuotas_Progress") == 0 and
...
function onTrigger(player,npc)
    local Quotas_Status = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.CHASING_QUOTAS);
    local Quotas_Progress = player:getVar("ChasingQuotas_Progress");
    local Quotas_No = player:getVar("ChasingQuotas_No");
    local Stalker_Status = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.KNIGHT_STALKER);
    local Stalker_Progress = player:getVar("KnightStalker_Progress");

    if (player:getMainLvl() >= ADVANCED_JOB_LEVEL and player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_HOLY_CREST) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 18) then
        if option == 0 then
            ...
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.CHASING_QUOTAS);
        end
    ...
    elseif (csid == 15) then
        if (player:getFreeSlotsCount() < 1) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.CHASING_QUOTAS);
