...
function onTrade(player,npc,trade)
    ...
    if (MoatCarp + ForestCarp > 0 and MoatCarp + ForestCarp == count) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_RIVALRY) == QUEST_ACCEPTED and totalFish >= 10000) then -- ultimate reward
        ...
        elseif (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_RIVALRY) >= QUEST_ACCEPTED) then -- regular turn-ins. Still allowed after completion of the quest.
            player:tradeComplete();
            player:addFame(SANDORIA,30);
            player:addGil((GIL_RATE*10*MoatCarp) + (GIL_RATE*15*ForestCarp));
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_COMPETITION) == QUEST_AVAILABLE and player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_RIVALRY) == QUEST_AVAILABLE) then -- If you haven't started either quest yet
        player:startEvent(300, 4401, 4289); -- 4401 = Moat Carp, 4289 = Forest Carp
    elseif (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_RIVALRY) == QUEST_ACCEPTED) then
        player:showText(npc, ID.text.GALLIJAUX_CARP_STATUS, 0, player:getVar("theCompetitionFishCountVar"));
    elseif ((player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_COMPETITION)) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 303) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_RIVALRY);
        end
    elseif (csid == 300 and option == 700) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_RIVALRY);
    end
end;
