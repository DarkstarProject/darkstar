...
function onTrigger(player,npc)
    ...
    theDismayedCustomer = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.THE_DISMAYED_CUSTOMER);
    ...
    elseif (theDismayedCustomer == QUEST_AVAILABLE and player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.A_TASTE_FOR_MEAT) == QUEST_COMPLETED) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 605 and option == 0) then
        player:addQuest(SANDORIA, dsp.quests.ids.sandoria.THE_DISMAYED_CUSTOMER);
    ...
    elseif (csid == 607) then
        ...
        player:completeQuest(SANDORIA, dsp.quests.ids.sandoria.THE_DISMAYED_CUSTOMER);
