...
function onTrade(player,npc,trade)
    ...
    if (MoatCarp + ForestCarp > 0 and MoatCarp + ForestCarp == count) then
        if (player:getQuestStatus(SANDORIA,dsp.quest.id.sandoria.THE_COMPETITION) == QUEST_ACCEPTED and totalFish >= 10000) then -- ultimate reward
        ...
        elseif (player:getQuestStatus(SANDORIA,dsp.quest.id.sandoria.THE_COMPETITION) >= QUEST_ACCEPTED) then -- regular turn-ins. Still allowed after completion of the quest.
            player:tradeComplete();
            player:addFame(SANDORIA,30);
            player:addGil((GIL_RATE*10*MoatCarp) + (GIL_RATE*15*ForestCarp));
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 304 and option == 700) then
        player:addQuest(SANDORIA,dsp.quest.id.sandoria.THE_COMPETITION);
    end
end;
