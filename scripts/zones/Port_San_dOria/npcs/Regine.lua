...
function onTrade(player,npc,trade)
    local flyersForRegine = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.FLYERS_FOR_REGINE);
    local theBrugaireConsortium = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_BRUGAIRE_CONSORTIUM);
...
function onTrigger(player,npc)
    ...
    -- FLYERS FOR REGINE
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.FLYERS_FOR_REGINE) == QUEST_AVAILABLE and ffr == 0) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 510 and option == 2) then
        if (npcUtil.giveItem(player, {{532,12}, {532,3}} )) then
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.FLYERS_FOR_REGINE);
            player:setVar("FFR",17);
        end
    elseif (csid == 603) then
        if (npcUtil.completeQuest(player, SANDORIA, dsp.quests.ids.sandoria.FLYERS_FOR_REGINE, {gil=440, title=dsp.title.ADVERTISING_EXECUTIVE})) then
