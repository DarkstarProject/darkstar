...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THICK_SHELLS) ~= QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 516) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THICK_SHELLS) == QUEST_AVAILABLE) then
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THICK_SHELLS);
        end
    elseif (csid == 514) then
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THICK_SHELLS) == QUEST_ACCEPTED) then
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THICK_SHELLS);
