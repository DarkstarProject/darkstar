...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_AMENDS) == QUEST_ACCEPTED) then
        if (trade:hasItemQty(937,1) and trade:getItemCount() == 1) then
            player:startEvent(277,1500);
        else
            player:startEvent(275,0,937);
        end
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WONDER_WANDS) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    MakingAmends = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_AMENDS);
    MakingAmens = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_AMENS); --Second quest in series
    WonderWands = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WONDER_WANDS); --Third and final quest in series
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 274 and option == 1) then
            player:addQuest(WINDURST,dsp.quests.ids.windurst.MAKING_AMENDS);
    elseif (csid == 277) then
            player:addGil(GIL_RATE*1500);
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.MAKING_AMENDS);
            player:addFame(WINDURST,75);
            player:addTitle(dsp.title.QUICK_FIXER);
            player:needToZone(true);
            player:tradeComplete();
    elseif (csid == 259 and option == 1) then
            player:addQuest(WINDURST,dsp.quests.ids.windurst.WONDER_WANDS);
    ...
    elseif (csid == 265) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.WONDER_WANDS);
        end
        -- ~[ Windurst Mission 6-1 Full Moon Fountain ]~ --
