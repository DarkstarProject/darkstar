...
WildCard   = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WILD_CARD);
    ...
    elseif (ThePromise == QUEST_ACCEPTED) then
        ...
        if (InvisibleManSticker == true) then
            ...
            if (ThePromiseCS_Seen == 1) then
                ...
                if (FreeSlots >= 1) then
                    player:completeQuest(WINDURST,dsp.quests.ids.windurst.THE_PROMISE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 368 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.TRUTH_JUSTICE_AND_THE_ONION_WAY);
    elseif (csid == 378) then
        FreeSlots = player:getFreeSlotsCount();

        if (FreeSlots >= 1) then
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.TRUTH_JUSTICE_AND_THE_ONION_WAY);
    ...
    elseif (csid == 391) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.KNOW_ONE_S_ONIONS);
    ...
    elseif (csid == 386 or csid == 400) then
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.KNOW_ONE_S_ONIONS);
    ...
    elseif (csid == 413 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.INSPECTOR_S_GADGET);
    elseif (csid == 421) then
        FreeSlots = player:getFreeSlotsCount();

        if (FreeSlots >= 1) then
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.INSPECTOR_S_GADGET);
    ...
    elseif (csid == 430) then
        ...
        if (OnionRings == QUEST_AVAILABLE) then
            ...
            player:addQuest(WINDURST,dsp.quests.ids.windurst.ONION_RINGS);
    ...
    elseif (csid == 496) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.CRYING_OVER_ONIONS);
    elseif (csid == 497) then
        player:setVar("CryingOverOnions",3);
    elseif (csid == 513) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_PROMISE);
