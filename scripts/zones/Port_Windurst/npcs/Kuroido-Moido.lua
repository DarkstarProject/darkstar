...
function onTrigger(player,npc)
    ...
    local makingAmends = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_AMENDS) --First quest in series
    local makingAmens = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_AMENS) --Second quest in series
    local wonderWands = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WONDER_WANDS) --Third and final quest in series
...
function onEventFinish(player,csid,option)
    if (csid == 280) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.MAKING_AMENS)
    elseif (csid == 284) then
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.MAKING_AMENS)
    else
