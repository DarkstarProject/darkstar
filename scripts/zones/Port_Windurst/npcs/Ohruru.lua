...
-- ======== FOR TESTING ONLY ==========-----
--    if (player:getVar("QuestCatchItIfYouCan_var") == 0 and player:hasStatusEffect(EFFECT.MUTE) == false and player:hasStatusEffect(EFFECT.BANE) == false and player:hasStatusEffect(EFFECT.PLAGUE) == false) then
--        rand = math.random(1,3);
--        if (rand == 1) then
--            player:addStatusEffect(EFFECT.MUTE,0,0,100);
--        elseif (rand == 2) then
--            player:addStatusEffect(EFFECT.BANE,0,0,100);
--        elseif (rand == 3) then
--            player:addStatusEffect(EFFECT.PLAGUE,0,0,100);
--        end
--    end
-- ======== FOR TESTING ONLY ==========-----
    ...
    elseif (Catch >= 1 and (player:hasStatusEffect(EFFECT.MUTE) == true or player:hasStatusEffect(EFFECT.BANE) == true or player:hasStatusEffect(EFFECT.PLAGUE) == true)) then
    ...
    elseif (Catch == 1 and player:hasStatusEffect(EFFECT.MUTE) == false and player:hasStatusEffect(EFFECT.BANE) == false and player:hasStatusEffect(EFFECT.PLAGUE) == false) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 246 and option == 0) then
        ...
        if (player:hasStatusEffect(EFFECT.MUTE) == true) then
            player:delStatusEffect(EFFECT.MUTE);
            player:addGil(GIL_RATE*1000);
            player:messageSpecial(GIL_OBTAINED,GIL_RATE*1000);
        elseif (player:hasStatusEffect(EFFECT.BANE) == true) then
            player:delStatusEffect(EFFECT.BANE);
            player:addGil(GIL_RATE*1200);
            player:messageSpecial(GIL_OBTAINED,GIL_RATE*1200);
        elseif (player:hasStatusEffect(EFFECT.PLAGUE) == true) then
            player:delStatusEffect(EFFECT.PLAGUE);
