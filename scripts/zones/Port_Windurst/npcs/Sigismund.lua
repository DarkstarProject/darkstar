...
function onTrade(player,npc,trade)
    starstatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TO_CATCH_A_FALLIHG_STAR);
...
function onEventFinish(player,csid,option)
    if (csid == 196) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.TO_CATCH_A_FALLIHG_STAR);
    elseif (csid == 199) then
        player:tradeComplete(trade);
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.TO_CATCH_A_FALLIHG_STAR);
