...
SomethingFishy = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.SOMETHING_FISHY);
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.BLAST_FROM_THE_PAST) == QUEST_ACCEPTED and player:getVar("BlastFromThePast_Prog") == 0) then
    ...
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.BLAST_FROM_THE_PAST) == QUEST_ACCEPTED and player:getVar("BlastFromThePast_Prog") == 0) then
        player:startEvent(318);
        player:setVar("BlastFromThePast_Prog",1);
    else
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 208) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.SOMETHING_FISHY);
        player:setVar("TokakaSpokenTo",1);
    elseif (csid == 210) then
        SomethingFishy = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.SOMETHING_FISHY);

        if (SomethingFishy == QUEST_ACCEPTED) then
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.SOMETHING_FISHY);
