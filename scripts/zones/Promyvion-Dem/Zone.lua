...
function onZoneIn(player,prevZone)
    ...
    elseif (player:getCurrentMission(COP) == THE_MOTHERCRYSTALS) then
        if (player:hasKeyItem(dsp.kis.LIGHT_OF_HOLLA) and player:hasKeyItem(dsp.kis.LIGHT_OF_MEA)) then
            if (player:getVar("cslastpromy") == 1) then
                player:setVar("cslastpromy",0)
                cs = 52;
            end
        elseif (player:hasKeyItem(dsp.kis.LIGHT_OF_HOLLA) or player:hasKeyItem(dsp.kis.LIGHT_OF_MEA)) then
