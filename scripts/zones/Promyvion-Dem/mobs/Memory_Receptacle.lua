...
function checkStray(mob)
    ...
    local numStrays = ID.mob.DEM_MEMORY_RECEPTACLES[mobId][2];
...
function onMobDeath(mob, player, isKiller)
    if (isKiller) then
        ...
        local mobId = mob:getID();
        local floor = ID.mob.DEM_MEMORY_RECEPTACLES[mobId][1];
        local numAlive = 1;
        ...
        if (math.random(numAlive) == 1) then
            local streamId = ID.mob.DEM_MEMORY_RECEPTACLES[mobId][3];
            local stream = GetNPCByID(streamId);
            local events = ID.npc.DEM_MEMORY_STREAMS[streamId][7];
