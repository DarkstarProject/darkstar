...
function checkStray(mob)
    ...
    local numStrays = MEA_MEMORY_RECEPTACLES[mobId][2];
...
function onMobDeath(mob, player, isKiller)
    if (isKiller) then
        ...
        local mobId = mob:getID();
        local floor = MEA_MEMORY_RECEPTACLES[mobId][1];
        local numAlive = 1;
        for k, v in pairs(MEA_MEMORY_RECEPTACLES) do
            if (k ~= mobId and v[1] == floor and GetMobByID(k):isAlive()) then
                numAlive = numAlive + 1;
            end
        end
        if (math.random(numAlive) == 1) then
            local streamId = MEA_MEMORY_RECEPTACLES[mobId][3];
            local stream = GetNPCByID(streamId);
            local events = MEA_MEMORY_STREAMS[streamId][7];
