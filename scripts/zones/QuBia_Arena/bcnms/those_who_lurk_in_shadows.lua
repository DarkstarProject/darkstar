...
function onBcnmLeave(player,instance,leavecode)
    ...
    if (player:hasKeyItem(dsp.kis.MARK_OF_SEED)) then
        player:delKeyItem(dsp.kis.MARK_OF_SEED);
    end
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 32001) then
        ...
        if (player:hasKeyItem(dsp.kis.IVORY_KEY) == false and player:getCurrentMission(ACP) >= THOSE_WHO_LURK_IN_SHADOWS_III) then
            player:addKeyItem(dsp.kis.IVORY_KEY);
            player:setVar("LastIvoryKey", os.date("%j"));
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.IVORY_KEY);
        end
    end
end;