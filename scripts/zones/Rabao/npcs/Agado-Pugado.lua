...
function onTrigger(player,npc)
    ...
    local TrialByWind = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WIND);
...
function onEventFinish(player,csid,option)
    if (csid == 66 and option == 1) then
        if (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WIND) == QUEST_COMPLETED) then
            player:delQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WIND);
        end
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WIND);
    ...
    elseif (csid == 69) then
        ...
        if (player:getFreeSlotsCount() == 0 and (option ~= 5 or option ~= 6)) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_BY_WIND);
        end
