...
function onTrade(player,npc,trade)
    ForgetTheAntidote = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.DONT_FORGET_THE_ANTIDOTE);
...
function onEventFinish(player,csid,option)
    if (csid == 2 and option == 1) then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.DONT_FORGET_THE_ANTIDOTE);
    ...
    elseif (csid == 4 and player:getVar("DontForgetAntidoteVar") == 1) then --If completing for the first time
        ...
        player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.DONT_FORGET_THE_ANTIDOTE);
