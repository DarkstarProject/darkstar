...
function onTrade(player,npc,trade)
    local Indomitable = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.INDOMITABLE_SPIRIT)
    local ImmortalLuShang = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.THE_IMMORTAL_LU_SHANG)
...
function onEventFinish(player,csid,option)
    if csid == 77 then
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.THE_IMMORTAL_LU_SHANG)
    elseif csid == 78 and npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.THE_IMMORTAL_LU_SHANG, {item=17386, fameArea=RABAO, fame=60, title=dsp.title.THE_IMMORTAL_FISHER_LU_SHANG}) then
        player:confirmTrade()
    elseif csid == 131 then
        player:addQuest(OUTLANDS, dsp.quests.ids.outlands.INDOMITABLE_SPIRIT)
    elseif csid == 132 then
        player:confirmTrade()
        player:setVar("IndomitableSpiritVar", os.date("%j")) -- %M for next minute, %j for next day
    elseif csid == 134 then
        npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.INDOMITABLE_SPIRIT, {item=17011, fameArea=RABAO, fame=100, title=dsp.title.INDOMITABLE_FISHER})
    end
end