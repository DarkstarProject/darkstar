...
function onTrade(player,npc,trade)
    local Indomitable = player:getQuestStatus(OUTLANDS,INDOMITABLE_SPIRIT)
    local ImmortalLuShang = player:getQuestStatus(OUTLANDS,THE_IMMORTAL_LU_SHANG)
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 78 and npcUtil.completeQuest(player, OUTLANDS, THE_IMMORTAL_LU_SHANG, {item=17386, fameArea=RABAO, fame=60, title=dsp.title.THE_IMMORTAL_FISHER_LU_SHANG}) then
        player:confirmTrade()
    elseif csid == 131 then
        player:addQuest(OUTLANDS, INDOMITABLE_SPIRIT)
