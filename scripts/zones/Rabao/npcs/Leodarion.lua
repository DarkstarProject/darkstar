...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.I_LL_TAKE_THE_BIG_BOX) == QUEST_ACCEPTED and player:getVar("illTakeTheBigBoxCS") == 2) then
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.I_LL_TAKE_THE_BIG_BOX) == QUEST_ACCEPTED) then
    ...
    elseif (player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRUE_WILL) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 99) then
        if (player:getFreeSlotsCount() < 1) then
            ...
            player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.TRUE_WILL);
        end
    end
