...
function onTrade(player,npc,trade)
    ...
    if (trade:hasItemQty(1546,1) and player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_WIND) == QUEST_ACCEPTED and player:getMainJob() == dsp.job.SMN) then
        player:startEvent(109,0,1546,3,20);
    end
end;

function onTrigger(player,npc)
    local TrialSizeWind = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_WIND);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 108 and option == 1) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(OUTLANDS,dsp.quests.ids.outlands.TRIAL_SIZE_TRIAL_BY_WIND);
            player:addItem(1546);
