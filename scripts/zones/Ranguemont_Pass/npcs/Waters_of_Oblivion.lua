...
function onTrigger(player,npc)
    ...
    if (player:hasKeyItem(dsp.kis.MERTAIRES_BRACELET) and
    ...
    elseif (player:hasKeyItem(dsp.kis.MERTAIRES_BRACELET) and TrosKilled == 1) then
...
function onEventFinish(player,csid,option)
    if (csid == 8) then
        if (npcUtil.completeQuest(player, JEUNO, PAINFUL_MEMORY, {item=16766})) then
            player:delKeyItem(dsp.kis.MERTAIRES_BRACELET);
