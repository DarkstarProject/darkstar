...
-- print("leave code "..leavecode);
    ...
    if (leavecode == 2) then -- play end CS. Need time and battle id for record keeping + storage
        if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.STORMS_OF_FATE) == QUEST_COMPLETED) then
...
-- print("bc finish csid "..csid.." and option "..option);
    ...
    if (csid == 32001) then
        if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.STORMS_OF_FATE) == QUEST_ACCEPTED and player:getVar('StormsOfFate') == 2) then
