...
function onTrade(player,npc,trade)
    ...
    if ((Hour >= 18 or Hour < 6) and IsMoonFull() == true) then
        if (DMfirst == QUEST_ACCEPTED or DMRepeat == QUEST_ACCEPTED) then -- allow for Ark Pentasphere on both first and repeat quests
            ...
            elseif (DMRepeat == QUEST_ACCEPTED and trade:hasItemQty(1261,1) and trade:getItemCount() == 1 and player:hasKeyItem(dsp.kis.MOONLIGHT_ORE) == false) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 8) then
        player:tradeComplete();
        player:addKeyItem(dsp.kis.MOONLIGHT_ORE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.MOONLIGHT_ORE);
