...
function onEventFinish(player, csid, option)
    ...
    elseif csid == 10094 then
        if option == 1 and npcUtil.giveItem(player, 2184) then
    ...
    elseif (csid == 161) then
		player:addQuest(JEUNO,dsp.quest.id.jeuno.SHADOWS_OF_THE_DEPARTED)
		npcUtil.giveKeyItem(player, dsp.ki.NOTE_WRITTEN_BY_ESHANTARL)
	elseif (csid == 162) then
		player:completeQuest(JEUNO,dsp.quest.id.jeuno.SHADOWS_OF_THE_DEPARTED)
    end
end