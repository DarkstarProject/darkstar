    ...
    if (regionID == 1) then
        ...
        elseif (player:getCurrentMission(COP) == DAWN) then
            ...
            elseif (player:getVar("PromathiaStatus") == 7) then
                if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.STORMS_OF_FATE) == QUEST_AVAILABLE) then
                    player:startEvent(142);
                elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.STORMS_OF_FATE) == QUEST_ACCEPTED and player:getVar('StormsOfFate') == 3) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 142) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.STORMS_OF_FATE);
    elseif (csid == 143) then
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.STORMS_OF_FATE);
