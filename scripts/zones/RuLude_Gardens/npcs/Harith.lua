...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.EMPTY_MEMORIES) >= QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.EMPTY_MEMORIES) == QUEST_AVAILABLE and player:getCurrentMission(COP) >= THE_MOTHERCRYSTALS) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.EMPTY_MEMORIES);
        player:startEvent(114);
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.EMPTY_MEMORIES) >= QUEST_ACCEPTED) then
        player:startEvent(114);
    else
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 110 or csid == 109) then
        ...
        if (player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.EMPTY_MEMORIES) == QUEST_ACCEPTED) then
            ...
            player:completeQuest(JEUNO, dsp.quests.ids.jeuno.EMPTY_MEMORIES)
        else
