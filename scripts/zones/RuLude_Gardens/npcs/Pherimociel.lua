...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(COP) == dsp.mission.id.cop.BELOW_THE_ARKS and player:getVar("PromathiaStatus") == 0) then
        player:startEvent(24);
    elseif (player:getCurrentMission(COP) == dsp.mission.id.cop.BELOW_THE_ARKS and player:getVar("PromathiaStatus") == 1) then
        player:startEvent(25);
    elseif (player:getCurrentMission(COP) == dsp.mission.id.cop.THREE_PATHS  and  player:getVar("COP_Tenzen_s_Path") == 3) then
        player:startEvent(58);
    elseif (player:getCurrentMission(COP) == dsp.mission.id.cop.FOR_WHOM_THE_VERSE_IS_SUNG  and  player:getVar("PromathiaStatus") == 0) then
        player:startEvent(10046);
    elseif (player:getCurrentMission(COP) == dsp.mission.id.cop.A_PLACE_TO_RETURN and player:getVar("PromathiaStatus") == 1) then
        if (Hrandom<0.2) then
