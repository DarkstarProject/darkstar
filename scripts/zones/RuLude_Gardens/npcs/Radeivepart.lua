...
function onTrade(player,npc,trade)
    ...
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.NORTHWARD) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    if (player:getFameLevel(JEUNO) >= 4 and player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.NORTHWARD) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 000 and option == 0) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.NORTHWARD);
    elseif (csid == 61) then
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.NORTHWARD);
