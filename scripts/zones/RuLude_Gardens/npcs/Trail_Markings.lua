...
function onTrigger(player,npc)
    ...
    elseif (player:getVar("DynaJeuno_Win") == 1) then
        player:startEvent(10026,dsp.kis.HYDRA_CORPS_TACTICAL_MAP); -- Win CS
    elseif (player:hasKeyItem(dsp.kis.VIAL_OF_SHROUDED_SAND)) then
        local firstDyna = 0;
        ...
        elseif ((dynaWaitxDay + (BETWEEN_2DYNA_WAIT_TIME * 60 * 60)) < realDay or (player:getVar("DynamisID") == dynaUniqueID and dynaUniqueID > 0)) then
            player:startEvent(10012,4,firstDyna,0,BETWEEN_2DYNA_WAIT_TIME,64,dsp.kis.VIAL_OF_SHROUDED_SAND,4236,4237);
        else
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10016) then
        player:addKeyItem(dsp.kis.VIAL_OF_SHROUDED_SAND);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.VIAL_OF_SHROUDED_SAND);
