...
function onTrigger(player,npc)
    ...
    if (cop == THE_SECRETS_OF_WORSHIP and copStat == 3 and not player:hasKeyItem(dsp.kis.RELIQUIARIUM_KEY) and not prof:isSpawned() and profLoc == 2) then
    ...
    elseif (cop == THE_SECRETS_OF_WORSHIP and copStat == 4 and not player:hasKeyItem(dsp.kis.RELIQUIARIUM_KEY)) then
        player:addKeyItem(dsp.kis.RELIQUIARIUM_KEY);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.RELIQUIARIUM_KEY);
