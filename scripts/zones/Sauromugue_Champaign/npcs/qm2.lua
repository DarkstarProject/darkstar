    ...
    if (thickAsThievesGrapplingCS >= 2 and thickAsThievesGrapplingCS <= 7) then
        if (trade:hasItemQty(17474,1) and trade:getItemCount() == 1) then -- Trade grapel
            if (player:getEquipID(SLOT_MAIN) == 0 and player:getEquipID(SLOT_SUB) == 0 and 
                ...
                player:messageSpecial(text.THF_AF_WALL_OFFSET+2,0,17474); --  You try climbing the wall using the [Grapnel], but you are too heavy.
            end    
        end
    end
end;
...
function onTrigger(player,npc)
    ...
    if (thickAsThieves == QUEST_ACCEPTED) then
        ...
        elseif (thickAsThievesGrapplingCS == 0 or thickAsThievesGrapplingCS == 1 or
            ...
            player:messageSpecial(text.THF_AF_WALL_OFFSET);    --  It is impossible to climb this wall with your bare hands.
        elseif (thickAsThievesGrapplingCS == 8) then
            player:messageSpecial(text.THF_AF_WALL_OFFSET+1); -- There is no longer any need to climb the tower.
        end    
    else 
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 0x0002) then
        ...
        player:messageSpecial(text.KEYITEM_OBTAINED,FIRST_SIGNED_FORGED_ENVELOPE);
