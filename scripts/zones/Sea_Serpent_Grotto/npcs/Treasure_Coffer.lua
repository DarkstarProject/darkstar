...
function onTrade(player,npc,trade)
    ...
    if ((trade:hasItemQty(1059,1) or trade:hasItemQty(1115,1) or trade:hasItemQty(1023,1) or trade:hasItemQty(1022,1)) and trade:getItemCount() == 1) then
        ...
        if (AFHandsActivated == 15 and player:hasKeyItem(dsp.kis.OLD_GAUNTLETS) == false) then
            questItemNeeded = 1;
        else
        ...
        if (success ~= -2) then
            ...
            if (math.random() <= success) then
                ...
                elseif (questItemNeeded == 3) then
                    ...
                    player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.MAP_OF_THE_SEA_SERPENT_GROTTO); -- Map of the Sea Serpent Grotto (KI)
                else
