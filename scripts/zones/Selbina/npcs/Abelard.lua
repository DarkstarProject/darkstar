...
function onTrade(player,npc,trade)
    if player:getQuestStatus(OTHER_AREAS_LOG, EN_EXPLORER_S_FOOTSTEPS) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, 570) then
...
function onTrigger(player,npc)
    local anExplorersFootsteps = player:getQuestStatus(OTHER_AREAS_LOG, EN_EXPLORER_S_FOOTSTEPS)
