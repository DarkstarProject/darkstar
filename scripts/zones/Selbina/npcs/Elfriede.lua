...
function onTrade(player,npc,trade)
    ...
    if (player:getVar("theTenshodoShowdownCS") == 3) then
        if (trade:hasItemQty(4569,1) and trade:getItemCount() == 1) then -- Trade Quadav Stew
            player:startEvent(10004,0,dsp.kis.TENSHODO_ENVELOPE,4569);
        end
    end
...
function onTrigger(player,npc)
    ...
    if (theTenshodoShowdownCS == 2) then
        player:startEvent(10002,0,dsp.kis.TENSHODO_ENVELOPE,4569); -- During Quest "The Tenshodo Showdown"
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10004) then
        ...
        player:delKeyItem(dsp.kis.TENSHODO_ENVELOPE);
        player:addKeyItem(dsp.kis.SIGNED_ENVELOPE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.SIGNED_ENVELOPE);
    end
