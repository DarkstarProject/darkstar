...
questStatus = player:getQuestStatus(OTHER_AREAS,ELDER_MEMORIES);
	...
	if (questStatus == QUEST_ACCEPTED and trade:getItemCount() == 1) then
		...
		if (IsacioElderMemVar == 1 and trade:hasItemQty(538,1) == true) then
			player:startEvent(0x0073,537);
		elseif (IsacioElderMemVar == 2 and trade:hasItemQty(537,1) == true) then
			player:startEvent(0x0074,539);
		elseif (IsacioElderMemVar == 3 and trade:hasItemQty(539,1) == true) then
