...
function onTrade(player,npc,trade)
    if
        ...
        player:getVar("ridingOnTheClouds_3") == 3 and
    ...
    elseif player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ONLY_THE_BEST) ~= QUEST_AVAILABLE then
...
function onTrigger(player,npc)
    if player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ONLY_THE_BEST) == QUEST_AVAILABLE then
...
function onEventFinish(player,csid,option)
    if csid == 60 and option == 10 then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ONLY_THE_BEST)
    elseif csid == 62 then
        player:addGil(100)
        ...
        player:completeQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.ONLY_THE_BEST)
        player:confirmTrade()
    elseif (csid == 63) then
        player:addGil(120)
