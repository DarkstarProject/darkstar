...
function onTrade(player,npc,trade)
    if player:getQuestStatus(OTHER_AREAS_LOG, THE_GIFT) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, 4375) then
        player:startEvent(72, 0, 4375) -- Finish quest "The gift"
    elseif player:getQuestStatus(OTHER_AREAS_LOG, THE_REAL_GIFT) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, 4484) then
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 37 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, UNDER_THE_SEA, {item = 13335, fame_area = SELBINA, title = dsp.title.LIL_CUPID, var = "underTheSeaVar"}) then
        player:delKeyItem(dsp.ki.ETCHED_RING)
    elseif csid == 70 and option == 50 then
        player:addQuest(OTHER_AREAS_LOG, THE_GIFT)
    elseif csid == 72 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, THE_GIFT, {item = 16497, fame_area = SELBINA, title = dsp.title.SAVIOR_OF_LOVE}) then
        player:confirmTrade()
    elseif csid == 73 and option == 50 then
        player:addQuest(OTHER_AREAS_LOG, THE_REAL_GIFT)
    elseif csid == 75 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, THE_REAL_GIFT, {item = 17385, fame_area = SELBINA, title = dsp.title.THE_LOVE_DOCTOR}) then
        player:confirmTrade()
    end
end
