...
function onTrade(player,npc,trade)
    if
        player:getQuestStatus(OTHER_AREAS_LOG, DONATE_TO_RECYCLING) == QUEST_ACCEPTED and
        (
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 21 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, DONATE_TO_RECYCLING, {item = 89, fame_area = SELBINA, title = dsp.title.ECOLOGIST}) then
        player:confirmTrade()
    end
end
