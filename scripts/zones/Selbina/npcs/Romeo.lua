...
function onTrade(player,npc,trade)
    if
        player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.DONATE_TO_RECYCLING) == QUEST_ACCEPTED and
        (
...
function onTrigger(player,npc)
    donateToRecycling = player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.DONATE_TO_RECYCLING)
...
function onEventFinish(player,csid,option)
    if csid == 20 then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.DONATE_TO_RECYCLING)
    elseif csid == 21 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.DONATE_TO_RECYCLING, {item = 89, fame_area = SELBINA, title = dsp.title.ECOLOGIST}) then
        player:confirmTrade()
    end
end
