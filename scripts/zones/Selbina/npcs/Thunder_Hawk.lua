...
function onTrigger(player,npc)
    local theRescue = player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.THE_RESCUE)
...
function onEventFinish(player,csid,option)
    if csid == 80 and option == 70 then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.THE_RESCUE)
    elseif csid == 81 and npcUtil.completeQuest(player, OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.THE_RESCUE,{fame_area = SELBINA, ki = dsp.ki.MAP_OF_THE_RANGUEMONT_PASS, title = dsp.title.HONORARY_CITIZEN_OF_SELBINA, gil = 3000}) then
