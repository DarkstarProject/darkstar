...
function onTrade(player,npc,trade)
    if player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.CARGO) ~= QUEST_AVAILABLE then
...
function onTrigger(player,npc)
    if player:getMainLvl() >= 20 and player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.CARGO) == QUEST_AVAILABLE then
...
function onEventFinish(player,csid,option)
    if csid == 50 then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.CARGO)
    elseif csid == 52 then
        player:setVar("VuntarCanBuyItem_date", os.date("%j"))

        if player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.CARGO) == QUEST_ACCEPTED then
            player:completeQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.CARGO)
