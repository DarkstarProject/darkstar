...
local function giveReward(player, csid)
    if csid == 166 or csid == 167 then
        ...
        if traded then
            ...
            player:setVar("insideBellyItemIdx", 0)
            if player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.INSIDE_THE_BELLY) == QUEST_ACCEPTED then
                player:completeQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.INSIDE_THE_BELLY)
            end
...
function onTrade(player,npc,trade)
    local underTheSea    = player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.UNDER_THE_SEA)
    local insideTheBelly = player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.INSIDE_THE_BELLY)
...
function onTrigger(player,npc)
    ...
    local theRealGift    = player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.THE_REAL_GIFT)
    local insideTheBelly = player:getQuestStatus(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.INSIDE_THE_BELLY)
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 161 then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.INSIDE_THE_BELLY)
