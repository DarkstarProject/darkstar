...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GRAVE_CONCERNS) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    Tomb = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GRAVE_CONCERNS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 541 and option == 0) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.GRAVE_CONCERNS);
    ...
    elseif (csid == 624) then
        ...
        player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.GRAVE_CONCERNS);
    end
