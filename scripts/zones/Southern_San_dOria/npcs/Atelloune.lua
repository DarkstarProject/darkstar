...
function onTrade(player,npc,trade)
    ...
    -----lady bug
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.ATELLOUNE_S_LAMENT) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    atellounesLament = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.ATELLOUNE_S_LAMENT)
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 890) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.ATELLOUNE_S_LAMENT);
    elseif (csid == 891) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.ATELLOUNE_S_LAMENT);
        end
    end
