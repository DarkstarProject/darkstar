...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 0x0269) then
        if (player:getFreeSlotsCount(0) >= 1) then
           ...
           player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED, 16565); -- Spatha
        end
    ...
    elseif (csid == 0x0272) then
        ...
        player:messageSpecial(text.KEYITEM_OBTAINED, SQUIRE_CERTIFICATE);
        player:addFame(SANDORIA,SAN_FAME*30);
    ...
    elseif (csid == 0x0273) then
        if (option == 0) then
            ...
            player:messageSpecial(text.KEYITEM_OBTAINED, BOOK_OF_TASKS);
        else
    ...
    elseif (csid == 0x0274) then
        if (player:getFreeSlotsCount(0) >= 1) then
            ...
            player:messageSpecial(text.ITEM_OBTAINED, 12306); -- Kite Shield
            player:unlockJob(7); --Paladin
            player:messageSpecial(text.UNLOCK_PALADIN);
            player:addFame(SANDORIA,SAN_FAME*30);
            player:completeQuest(SANDORIA,A_KNIGHT_S_TEST);
        else
           player:messageSpecial(text.ITEM_CANNOT_BE_OBTAINED, 12306); -- Kite Shield
        end
    elseif (csid == 63) then
