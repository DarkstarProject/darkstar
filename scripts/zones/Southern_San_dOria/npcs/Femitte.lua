...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.LURE_OF_THE_WILDCAT_SAN_D_ORIA) == QUEST_ACCEPTED and player:getMaskBit(WildcatSandy,3) == false) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 663 and option == 0) then
        ...
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.DISTANT_LOYALTIES);
    ...
    elseif (csid == 665) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.DISTANT_LOYALTIES);
        end;
    end;
