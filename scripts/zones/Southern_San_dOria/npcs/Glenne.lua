...
function onTrade(player,npc,trade)
	...
	elseif(player:getQuestStatus(SANDORIA,A_SENTRY_S_PERIL) == QUEST_ACCEPTED) then
		if(trade:hasItemQty(OintmentCase,1) and trade:getItemCount() == 1) then
...
function onTrigger(player,npc)
	...
	elseif(aSentrysPeril == QUEST_ACCEPTED) then
		if (player:hasItem(DoseOfOintment) == true or player:hasItem(OintmentCase) == true) then
...
--printf("RESULT: %u",option);
	...
	if (csid == 0x01fe and option == 0) then
		if (player:getFreeSlotsCount() > 0) then
			...
			player:addItem(DoseOfOintment);
			player:messageSpecial(ITEM_OBTAINED,DoseOfOintment);
		else
			player:messageSpecial(ITEM_CANNOT_BE_OBTAINED,DoseOfOintment);
		end 	
	...
	elseif(csid == 0x0201) then
		if (player:getFreeSlotsCount() == 0) then 
			player:messageSpecial(ITEM_CANNOT_BE_OBTAINED,BronzeSubligar);
		else
			player:tradeComplete();
			player:setTitle(RONFAURIAN_RESCUER);
			player:addItem(BronzeSubligar);
			player:messageSpecial(ITEM_OBTAINED,BronzeSubligar);
