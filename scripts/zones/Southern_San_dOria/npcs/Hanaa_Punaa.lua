...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_SEAMSTRESS) ~= QUEST_AVAILABLE) then
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.BLACK_TIGER_SKINS) == QUEST_ACCEPTED) then
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.LIZARD_SKINS) ~= QUEST_AVAILABLE) then
    ...
    -- "Flyers for Regine"
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.FLYERS_FOR_REGINE) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    theSteamStress = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_SEAMSTRESS);
    lizardSkins = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.LIZARD_SKINS);
    blackTigerSkins = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.BLACK_TIGER_SKINS);
...
function onEventFinish(player,csid,option)
    ...
    if ((csid == 528 or csid == 531) and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_SEAMSTRESS);
    ...
    elseif (csid == 530) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_SEAMSTRESS) == QUEST_ACCEPTED) then
                player:addFame(SANDORIA,30);
                player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_SEAMSTRESS);
            else
    ...
    elseif ((csid == 559 or csid == 562) and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.LIZARD_SKINS);
    ...
    elseif (csid == 561) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.LIZARD_SKINS) == QUEST_ACCEPTED) then
                player:addFame(SANDORIA,30);
                player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.LIZARD_SKINS);
            else
                player:addFame(SANDORIA,5);
            end
        end
    ...
    elseif ((csid == 576 or csid == 579) and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.BLACK_TIGER_SKINS);
    ...
    elseif (csid == 577) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addFame(SANDORIA,30);
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.BLACK_TIGER_SKINS);
        end
    end
