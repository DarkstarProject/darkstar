...
function onTrigger(player,npc)
    ...
    quest_fas = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.FATHER_AND_SON);      -- 1st Quest in Series
    quest_poa = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.A_PURCHASE_OF_ARMS);  -- 2nd Quest in Series
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 594 and option == 0) then
        player:addQuest(SANDORIA, dsp.quests.ids.sandoria.A_PURCHASE_OF_ARMS);
    ...
    elseif (csid == 607) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA, dsp.quests.ids.sandoria.A_PURCHASE_OF_ARMS);
        end
    end
