...
function onTrigger(player,npc)
    local grimySignpost = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.GRIMY_SIGNPOSTS);
...
function onEventFinish(player,csid,option)
    if (csid == 45 and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.GRIMY_SIGNPOSTS);
    elseif (csid == 44) then
        player:setVar("CleanSignPost",0);
        player:addFame(SANDORIA,30);
        player:addGil(GIL_RATE*1500);
        player:messageSpecial(ID.text.GIL_OBTAINED,GIL_RATE*1500);
        player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.GRIMY_SIGNPOSTS);
    end
end;
