...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.SLEEPLESS_NIGHTS) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    sleeplessNights = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.SLEEPLESS_NIGHTS);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 85 and option == 1) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.SLEEPLESS_NIGHTS);
    elseif (csid == 84) then
        ...
        player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.SLEEPLESS_NIGHTS);
    end
