...
function onTrade(player,npc,trade)
    ...
    local theSweetestThings = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_SWEETEST_THINGS);
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 533) then
        if (option == 0) then
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_SWEETEST_THINGS);
    ...
    elseif (csid == 535) then
        ...
        if (player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.THE_SWEETEST_THINGS) == QUEST_ACCEPTED) then
            player:addFame(SANDORIA,30);
            player:completeQuest(SANDORIA, dsp.quests.ids.sandoria.THE_SWEETEST_THINGS);
        else
