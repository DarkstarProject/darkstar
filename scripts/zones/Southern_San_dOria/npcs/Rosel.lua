...
function onTrigger(player,npc)
    ...
    local RoselTheArmorer = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.ROSEL_THE_ARMORER);
...
function onEventFinish(player,csid,option)
    ...
    if ((csid == 523 or csid == 524) and option == 0) then
        player:addQuest(SANDORIA, dsp.quests.ids.sandoria.ROSEL_THE_ARMORER);
    ...
    elseif (csid == 527) then
        npcUtil.completeQuest(player, SANDORIA, dsp.quests.ids.sandoria.ROSEL_THE_ARMORER, {
