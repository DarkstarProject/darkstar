...
function onTrigger(player,npc)
    ...
    theCrimsonTrial = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.THE_CRIMSON_TRIAL);
    envelopedInDarkness = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.ENVELOPED_IN_DARKNESS);
    peaceForTheSpirit = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.PEACE_FOR_THE_SPIRIT);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 70 or csid == 71) then
        ...
        elseif (option == 1) then
            player:addQuest(SANDORIA,dsp.quests.ids.sandoria.THE_CRIMSON_TRIAL);
    ...
    elseif (csid == 75) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.THE_CRIMSON_TRIAL);
        end
    elseif (csid == 64) then
