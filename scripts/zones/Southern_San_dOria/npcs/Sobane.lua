...
function onTrade(player,npc,trade)
    -- FLYERS FOR REGINE
    if npcUtil.tradeHas(trade, 532) and player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.FLYERS_FOR_REGINE) == QUEST_ACCEPTED then
        player:messageSpecial(ID.text.FLYER_REFUSED)

    -- SIGNED IN BLOOD
    elseif npcUtil.tradeHas(trade, 1662) and player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.SIGNED_IN_BLOOD) == QUEST_ACCEPTED and player:getVar("SIGNED_IN_BLOOD_Prog") < 1 then
...
function onTrigger(player,npc)
    local blood = player:getQuestStatus(SANDORIA, dsp.quests.ids.sandoria.SIGNED_IN_BLOOD)
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 732 and option == 1 then
        player:addQuest(SANDORIA, dsp.quests.ids.sandoria.SIGNED_IN_BLOOD)
    elseif csid == 734 then
        player:setVar("SIGNED_IN_BLOOD_Prog", 1)
    elseif csid == 736 and npcUtil.completeQuest(player, SANDORIA, dsp.quests.ids.sandoria.SIGNED_IN_BLOOD, {item = 14760, gil = 3500, var = "dsp.quests.ids.sandoria.SIGNED_IN_BLOOD_Prog"}) then
