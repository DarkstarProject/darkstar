...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.TIGER_S_TEETH) ~= QUEST_AVAILABLE) then
...
function onTrigger(player,npc)
    ...
    local tigersTeeth = player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.TIGER_S_TEETH);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 574 and option == 0) then
        player:addQuest(SANDORIA,dsp.quests.ids.sandoria.TIGER_S_TEETH);
    elseif (csid == 572) then
        ...
        if (player:getQuestStatus(SANDORIA,dsp.quests.ids.sandoria.TIGER_S_TEETH) == QUEST_ACCEPTED) then
            player:addFame(SANDORIA,30);
            player:completeQuest(SANDORIA,dsp.quests.ids.sandoria.TIGER_S_TEETH);
        else
