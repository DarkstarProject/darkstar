...
function onTrade(player,npc,trade)
    ...
    local beansAhoy = player:getQuestStatus(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BEANS_AHOY);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 334) then
        player:addQuest(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BEANS_AHOY);
    ...
    elseif (csid == 340 or csid == 342) then
        if (player:hasItem(5704,1) or player:getFreeSlotsCount() < 1) then
            ...
            if (csid == 340) then
                player:completeQuest(CRYSTAL_WAR,dsp.quests.ids.crystalWar.BEANS_AHOY);
