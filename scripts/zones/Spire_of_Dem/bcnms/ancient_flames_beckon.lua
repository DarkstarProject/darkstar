...
function onBcnmLeave(player,instance,leavecode)
    ...
    if (leavecode == 2) then -- play end CS. Need time and battle id for record keeping + storage
        if (player:getCurrentMission(COP) == THE_MOTHERCRYSTALS) then    
            if (player:hasKeyItem(dsp.kis.LIGHT_OF_MEA) and player:hasKeyItem(dsp.kis.LIGHT_OF_HOLLA)) then 
                player:startEvent(32001,0,0,0,instance:getTimeInside(),0,0,0,3);
            elseif (player:hasKeyItem(dsp.kis.LIGHT_OF_MEA) or player:hasKeyItem(dsp.kis.LIGHT_OF_HOLLA)) then 
    ...
    if (csid == 32001) then
        if (player:getCurrentMission(COP) == THE_MOTHERCRYSTALS) then 
            if (player:hasKeyItem(dsp.kis.LIGHT_OF_MEA) and player:hasKeyItem(dsp.kis.LIGHT_OF_HOLLA)) then
                ...
                player:messageSpecial(CANT_REMEMBER,dsp.kis.LIGHT_OF_DEM);
            ...
            elseif (not(player:hasKeyItem(dsp.kis.LIGHT_OF_DEM))) then
                player:setVar("cspromy3",1)
                player:addKeyItem(dsp.kis.LIGHT_OF_DEM);
                player:addExp(1500);
                player:messageSpecial(CANT_REMEMBER,dsp.kis.LIGHT_OF_DEM);
                player:addStatusEffectEx(dsp.effects.TELEPORT,0,TELEPORT_EXITPROMDEM,0,1);
            end
        elseif (player:getCurrentMission(COP) == BELOW_THE_ARKS) then
            player:addExp(1500);
