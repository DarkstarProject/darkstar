...
function onBcnmLeave(player,instance,leavecode)
	...
	if (leavecode == 2) then -- play end CS. Need time and battle id for record keeping + storage
		if (player:getCurrentMission(COP) == THE_MOTHERCRYSTALS) then	
			...
			elseif ((not(player:hasKeyItem(LIGHT_OF_HOLLA))) and (not(player:hasKeyItem(LIGHT_OF_DEM)))) then
				-- print("this")
...
function onEventFinish(player,csid,option)
	...
	if (csid == 0x7D01) then
		if (player:hasCompletedMission(COP,THE_MOTHERCRYSTALS)) then 
			player:addExp(1500);
			elseif (player:hasKeyItem(LIGHT_OF_HOLLA) and player:hasKeyItem(LIGHT_OF_DEM)) then
