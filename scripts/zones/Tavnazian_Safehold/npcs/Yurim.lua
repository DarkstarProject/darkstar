...
function onTrade(player,npc,trade)
    local nameOfScience  = player:getQuestStatus(OTHER_AREAS_LOG,dsp.quests.ids.otherAreas.IN_THE_NAME_OF_SCIENCE)
...
function onEventFinish(player,csid,option)
    if csid == 524 then
        player:addQuest(OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.IN_THE_NAME_OF_SCIENCE)
    ...
    elseif csid == 529 then
        ...
        if itemComplete > 0 and itemInProgress == itemComplete then
            ...
            if npcUtil.completeQuest(player, OTHER_AREAS_LOG, dsp.quests.ids.otherAreas.IN_THE_NAME_OF_SCIENCE, {item=itemComplete, var={"NAME_OF_SCIENCE_target"}}) then
                player:confirmTrade()
            end
        else
