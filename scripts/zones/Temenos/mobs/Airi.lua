...
function onMobEngaged(mob, target)
    if limbus.isMobDead(ID.mob.TEMENOS_C_MOB[1]+1) then
...
function onMobDeath(mob, player, isKiller)
    ...
    if limbus.isMobDead(ID.mob.TEMENOS_C_MOB[1]+1) and limbus.isMobDead(ID.mob.TEMENOS_C_MOB[1]+2) and
        limbus.isMobDead(ID.mob.TEMENOS_C_MOB[1]+3) and limbus.isMobDead(ID.mob.TEMENOS_C_MOB[1]+4) and
