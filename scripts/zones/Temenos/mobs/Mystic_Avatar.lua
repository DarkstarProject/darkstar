...
function onMobEngaged(mob,target)
    ...
    if (mobID==16929030) then --Carbuncle (Central Temenos 2nd Floor)
        ...
        if (limbus.isMobDead(16929033)==true and limbus.isMobDead(16929039)==true) then
            mob:setMod(dsp.mod.FIREDEF,-128);
        else
            mob:setMod(dsp.mod.FIREDEF,256);
        end

        if (limbus.isMobDead(16929034)==true and limbus.isMobDead(16929040)==true) then
            mob:setMod(dsp.mod.ICEDEF,-128);
        else
            mob:setMod(dsp.mod.ICEDEF,256);
        end

        if (limbus.isMobDead(16929035)==true and limbus.isMobDead(16929041)==true) then
            mob:setMod(dsp.mod.WINDDEF,-128);
        else
            mob:setMod(dsp.mod.WINDDEF,256);
        end

        if (limbus.isMobDead(16929036)==true and limbus.isMobDead(16929042)==true) then
            mob:setMod(dsp.mod.EARTHDEF,-128);
        else
            mob:setMod(dsp.mod.EARTHDEF,256);
        end

        if (limbus.isMobDead(16929037)==true and limbus.isMobDead(16929043)==true) then
            mob:setMod(dsp.mod.THUNDERDEF,-128);
        else
            mob:setMod(dsp.mod.THUNDERDEF,256);
        end

        if (limbus.isMobDead(16929038)==true and limbus.isMobDead(16929044)==true) then
...
function onMobDeath(mob, player, isKiller)
    ...
    elseif (mobID==16929030) then --Carbuncle (Central Temenos 2nd Floor)
        if (limbus.isMobDead(16929031)==true and limbus.isMobDead(16929032)==true ) then
