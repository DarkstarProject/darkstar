-- Variable TextID   Description text

-- General Texts
   ITEM_CANNOT_BE_OBTAINED = 6381 -- You cannot obtain the item <item> come back again after sorting your inventory
             ITEM_OBTAINED = 6387 -- Obtained: <item>
              GIL_OBTAINED = 6388 -- Obtained <number> gil
          KEYITEM_OBTAINED = 6390 -- Obtained key item: <keyitem>
    FISHING_MESSAGE_OFFSET = 7207 -- You can't fish here
         BITS_OF_VEGETABLE = 7498 -- Bits of vegetable matter are strewn around. They appear to have been gnawed on by insects...

-- Treasure Coffer/Chest Dialog
            CHEST_UNLOCKED = 7314 -- You unlock the chest!
                CHEST_FAIL = 7315 -- Fails to open the chest.
                CHEST_TRAP = 7316 -- The chest was trapped!
                CHEST_WEAK = 7317 -- You cannot open the chest when you are in a weakened state.
               CHEST_MIMIC = 7318 -- The chest was a mimic!
              CHEST_MOOGLE = 7319 -- You cannot open the chest while participating in the moogle event.
            CHEST_ILLUSION = 7320 -- The chest was but an illusion...
              CHEST_LOCKED = 7321 -- The chest appears to be locked.

-- Dialog Texts
  NO_REASON_TO_INVESTIGATE = 7322 -- There is no reason to investigate further.
         THE_BOX_IS_LOCKED = 7323 -- The box is locked.
         PAINTBRUSH_OFFSET = 7326 -- projects the deepest, darkest corner of your soul onto the blank canvas...only then will the doors to rancor open
       FALLS_FROM_THE_BOOK = 7336 -- falls from the book!
        THE_DOOR_IS_LOCKED = 7350 -- The door is locked. You might be able to open it with
PROTECTED_BY_UNKNOWN_FORCE = 7351 -- The door is protected by some unknown force.
           YOUR_KEY_BREAKS = 7353 -- breaks!
               DOOR_LOCKED = 7371 -- The door is locked.
     SOME_SORT_OF_CEREMONY = 7443 -- Some sort of ceremony was performed here...
                 NM_OFFSET = 7493 -- It looks like some sort of device. A thin thread leads down to the floor...
           IT_IS_A_BEEHIVE = 7497 -- It is a beehive...

           NOTHING_HAPPENS =  119 -- Nothing happens...
   NOTHING_OUT_OF_ORDINARY = 7501 -- There is nothing out of the ordinary here.

-- Other
                HATE_RESET = 7424 -- The built-up hate has been cleansed...!
                 DOOR_SHUT = 7426 -- The door is firmly shut.
                   NO_HATE = 7427 -- You have no built-up hate to cleanse.

-- conquest Base
             CONQUEST_BASE = 7048 -- Tallying conquest results...

-- WS Quest
    SENSE_OMINOUS_PRESENCE = 7500 -- You sense an ominous presence...
