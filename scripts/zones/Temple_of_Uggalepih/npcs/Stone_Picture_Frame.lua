...
function onTrigger(player,npc)
    ...
    if (X < -60) then
        if (Z < -6) then -- SW frame
            if (player:hasKeyItem(dsp.kis.FINAL_FANTASY)) then
                player:startEvent(50,dsp.kis.FINAL_FANTASY);
            else
        ...
        elseif (Z > -5 and Z < 5) then
            if (GetNPCByID(UGGALEPIH_DOOR_TO_RANCOR):getAnimation() == ANIMATION_OPEN_DOOR) then
                player:messageSpecial(PAINTBRUSH_OFFSET + 23,dsp.kis.PAINTBRUSH_OF_SOULS); -- The <KEY_ITEM> begins to twitch. The canvas is graced with the image from your soul.
            elseif (player:hasKeyItem(dsp.kis.PAINTBRUSH_OF_SOULS) and X >= -53.2 and Z <= 0.1 and Z >= -0.1) then -- has paintbrush of souls + close enough
                player:messageSpecial(PAINTBRUSH_OFFSET + 17,dsp.kis.PAINTBRUSH_OF_SOULS);
                player:setVar("started_painting",os.time());
                player:startEvent(60,dsp.kis.PAINTBRUSH_OF_SOULS);
            elseif (player:hasKeyItem(dsp.kis.PAINTBRUSH_OF_SOULS)) then
                player:messageSpecial(PAINTBRUSH_OFFSET + 15,dsp.kis.PAINTBRUSH_OF_SOULS);
            else
                player:messageSpecial(PAINTBRUSH_OFFSET, dsp.kis.PAINTBRUSH_OF_SOULS); -- When the paintbrush of souls projects the deepest, darkest corner of your soul...
            end
        else
