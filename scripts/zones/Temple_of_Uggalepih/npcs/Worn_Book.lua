...
function onTrigger(player,npc)
    if (player:hasKeyItem(dsp.kis.OLD_RUSTY_KEY) or player:hasKeyItem(dsp.kis.PAINTBRUSH_OF_SOULS)) then
...
function onEventFinish(player,csid,option)
    ...
    if (player:getVar("paintbrushOfSouls_book") == 7) then
        player:messageSpecial(FALLS_FROM_THE_BOOK,dsp.kis.OLD_RUSTY_KEY);
        player:addKeyItem(dsp.kis.OLD_RUSTY_KEY);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.OLD_RUSTY_KEY);
