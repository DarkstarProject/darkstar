    ...
    if (csid == 44 and option == 0) then
        ...
        if (LanternOrder == 0) then
            if (offset == 0) then --(F-9)
                player:messageSpecial(THE_LIGHT_DIMLY,0,0,0,dsp.kis.DUCAL_GUARDS_LANTERN_LIT);
                player:setVar("saveMySisterFireLantern",1);
            else
                player:messageSpecial(REFUSE_TO_LIGHT,0,0,0,dsp.kis.DUCAL_GUARDS_LANTERN_LIT);
            end
        elseif (LanternOrder == 1) then
            if (offset == 1) then --(H-7)
                player:messageSpecial(THE_LIGHT_HAS_INTENSIFIED,0,0,0,dsp.kis.DUCAL_GUARDS_LANTERN_LIT);
                player:setVar("saveMySisterFireLantern",2);
            else
                player:messageSpecial(LANTERN_GOES_OUT,0,0,0,dsp.kis.DUCAL_GUARDS_LANTERN_LIT);
        ...
        elseif (LanternOrder == 3) then
            if (offset == 3) then --(H-9)
                player:messageSpecial(THE_LIGHT_IS_FULLY_LIT,0,0,0,dsp.kis.DUCAL_GUARDS_LANTERN_LIT);
