...
function onTrade(player,npc,trade)
    ...
    if ((trade:hasItemQty(1046,1) or trade:hasItemQty(1115,1) or trade:hasItemQty(1023,1) or trade:hasItemQty(1022,1)) and trade:getItemCount() == 1) then
        ...
        if ((AFHandsActivated == 8 or AFHandsActivated == 5 or AFHandsActivated == 1 or AFHandsActivated == 7) and player:hasKeyItem(dsp.ki.OLD_GAUNTLETS) == false) then
            questItemNeeded = 1;
        else
...
function onTrigger(player,npc)
    player:messageSpecial(ID.text.CHEST_LOCKED,1046);
end;
