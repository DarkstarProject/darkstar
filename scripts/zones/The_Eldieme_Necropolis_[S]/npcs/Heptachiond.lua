...
function onTrigger(player, npc)
    local Rftd = player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.REQUIEM_FOR_THE_DEPARTED)
...
function onEventFinish(player, csid, option)
    if csid == 105 then
        player:addQuest(CRYSTAL_WAR, dsp.quests.ids.crystalWar.REQUIEM_FOR_THE_DEPARTED)
    elseif csid == 107 and npcUtil.completeQuest(player, CRYSTAL_WAR, dsp.quests.ids.crystalWar.REQUIEM_FOR_THE_DEPARTED, {item = 4689}) then
