-- Variable TextID   Description text

-- General Texts
   ITEM_CANNOT_BE_OBTAINED = 6378; -- You cannot obtain the item <item>. Come back after sorting your inventory.
FULL_INVENTORY_AFTER_TRADE = 6380; -- You cannot obtain the <item>. Try trading again after sorting your inventory.
             ITEM_OBTAINED = 6381; -- Obtained: <item>.
              GIL_OBTAINED = 6382; -- Obtained <number> gil.
          KEYITEM_OBTAINED = 6384; -- Obtained key item: <keyitem>.
            ITEMS_OBTAINED = 6387; -- You obtain <param2 number> <param1 item>!
           BEASTMEN_BANNER = 7105; -- There is a beastmen's banner.
    FISHING_MESSAGE_OFFSET = 7525; -- You can't fish here.

-- Conquest
CONQUEST = 7192; -- You've earned conquest points!

-- Quest Dialogs
    SENSE_OF_FOREBODING = 6396; -- You are suddenly overcome with a sense of foreboding...
          STURDY_BRANCH = 7733; -- It is a beautiful, sturdy branch.
NOTHING_OUT_OF_ORDINARY = 6395; -- There is nothing out of the ordinary here.

-- ZM4 Dialog
   CANNOT_REMOVE_FRAG = 7706; -- It is an oddly shaped stone monument. A shining stone is embedded in it, but cannot be removed...
ALREADY_OBTAINED_FRAG = 7707; -- You have already obtained this monument's
      FOUND_ALL_FRAGS = 7709; -- You now have all 8 fragments of light!
      ZILART_MONUMENT = 7710; -- It is an ancient Zilart monument.
