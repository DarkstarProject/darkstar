...
function onTrigger(player,npc)
    ...
    -- SOUL SEARCHING
    elseif player:hasCompletedMission(ZILART, THE_CHAMBER_OF_ORACLES) and not player:hasCompletedQuest(OUTLANDS, dsp.quests.ids.outlands.SOUL_SEARCHING) then
        player:addQuest(OUTLANDS, dsp.quests.ids.outlands.SOUL_SEARCHING)
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 202 then
        npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.SOUL_SEARCHING, {item = 13416, title = dsp.title.GUIDER_OF_SOULS_TO_THE_SANCTUARY})
    end
end
