...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(ZILART) == HEADSTONE_PILGRIMAGE) then
        if (player:hasKeyItem(dsp.ki.LIGHT_FRAGMENT)) then
            player:messageSpecial(ALREADY_OBTAINED_FRAG,dsp.ki.LIGHT_FRAGMENT);
        elseif (os.time() >= npc:getLocalVar("cooldown")) then
            ...
            if (
                ...
                player:messageSpecial(FOUND_ALL_FRAGS,dsp.ki.LIGHT_FRAGMENT);
                ...
                player:messageSpecial(KEYITEM_OBTAINED,dsp.ki.LIGHT_FRAGMENT);
            end
        end
