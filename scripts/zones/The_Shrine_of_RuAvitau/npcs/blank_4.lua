...
function onTrigger(player,npc)
    ...
    local DMStatus = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT);
    local DMRepeat = player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT_REPEAT);
    local AAKeyitems = 0;
    local DMEarrings = 0;
...
function onEventFinish(player,csid,option)
    ...
    elseif ((csid == 54 or csid == 56) and player:getQuestStatus(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT) == QUEST_AVAILABLE) then -- Flag Divine Might
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT);

    elseif (csid == 57) then -- Divine Might Repeat
        player:delQuest(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT_REPEAT);
        player:addQuest(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT_REPEAT);
    ...
    elseif (csid == 55 or csid == 59) then -- Turning in Divine Might or Repeat
        ...
        if (reward ~= 0) then
            if (player:getFreeSlotsCount() >= 1 and player:hasItem(reward) == false) then
                ...
                if (csid == 55) then
                    player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT);
                else
                    player:completeQuest(OUTLANDS,dsp.quests.ids.outlands.DIVINE_MIGHT_REPEAT);
