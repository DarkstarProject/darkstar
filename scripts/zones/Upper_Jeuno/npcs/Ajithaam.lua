...
function onTrade(player,npc,trade)
    if (trade:getGil() == 300 and trade:getItemCount() == 1 and player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.LURE_OF_THE_WILDCAT_JEUNO) == QUEST_COMPLETED and player:getCurrentMission(TOAU) > IMMORTAL_SENTRIES) then
...
function onTrigger(player,npc)
    local LureJeuno = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.LURE_OF_THE_WILDCAT_JEUNO);
...
function onEventFinish(player,csid,option)
    if (csid == 10088) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.LURE_OF_THE_WILDCAT_JEUNO);
    ...
    elseif (csid == 10091) then
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.LURE_OF_THE_WILDCAT_JEUNO);
