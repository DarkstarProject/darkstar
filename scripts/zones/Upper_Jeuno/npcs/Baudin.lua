...
function onTrade(player,npc,trade)
    ...
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.CREST_OF_DAVOI) == QUEST_ACCEPTED and trade:hasItemQty(4377,1) and trade:getItemCount() == 1) then
        player:startEvent(171); -- Finish Quest "Crest of Davoi" Start Quest "Save my Sister" with var, not addquest()
    end
end;

function onTrigger(player,npc)
    local CrestOfDavoi = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.CREST_OF_DAVOI);
    local SaveMySister = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.SAVE_MY_SISTER);
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 174 and option == 1) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.CREST_OF_DAVOI);
    elseif (csid == 171) then
        ...
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.CREST_OF_DAVOI);
    ...
    elseif (csid == 107) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:tradeComplete();
            player:addFame(JEUNO, 30);
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.SAVE_MY_SISTER);
        end
    end
end;