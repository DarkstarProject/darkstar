...
function onTrigger(player, npc)
    ...
    local chocoboOnTheLoose = player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.CHOCOBO_ON_THE_LOOSE)
    local chocoboOnTheLooseStat = player:getVar("ChocoboOnTheLoose")
    local chocobosWounds = player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.CHOCOBO_S_WOUNDS)
    local chocobosWoundsStat = player:getVar("ChocobosWounds_Event")
    local saveMySon = player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.SAVE_MY_SON)
    local pathOfTheBeastmaster = player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.PATH_OF_THE_BEASTMASTER)
    local wingsOfGold = player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.WINGS_OF_GOLD)
    local scatteredIntoShadow = player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.SCATTERED_INTO_SHADOW)
...
function onEventFinish(player, csid, option)
    -- CHOCOBO ON THE LOOSE
    if csid == 10093 then
        player:addQuest(JEUNO, dsp.quests.ids.jeuno.CHOCOBO_ON_THE_LOOSE)
    ...
    elseif csid == 10109 then
        npcUtil.completeQuest(player, JEUNO, dsp.quests.ids.jeuno.CHOCOBO_ON_THE_LOOSE, {item = 2317, var = {"ChocoboOnTheLoose", "ChocoboOnTheLooseDay", "ChocoboOnTheLooseYear"}})

    -- CHOCOBO'S WOUNDS
    elseif csid == 71 and option == 1 then
        player:addQuest(JEUNO, dsp.quests.ids.jeuno.CHOCOBO_S_WOUNDS)
        player:setVar("ChocobosWounds_Event", 1)

    -- PATH OF THE BEASTMASTER
    elseif csid == 70 then
        player:addQuest(JEUNO, dsp.quests.ids.jeuno.PATH_OF_THE_BEASTMASTER)
        npcUtil.completeQuest(player, JEUNO, dsp.quests.ids.jeuno.PATH_OF_THE_BEASTMASTER, {title = dsp.title.ANIMAL_TRAINER})
    ...
    elseif (csid == 137 or csid == 139) and option == 1 then
        player:addQuest(JEUNO, dsp.quests.ids.jeuno.WINGS_OF_GOLD)
        player:setVar("wingsOfGold_shortCS", 0)
    elseif csid == 138 and npcUtil.completeQuest(player, JEUNO, dsp.quests.ids.jeuno.WINGS_OF_GOLD, {item = 16680, fame = AF1_FAME}) then
        player:delKeyItem(dsp.ki.GUIDING_BELL)

    -- SCATTERED INTO SHADOW
    elseif (csid == 141 or csid == 143) and option == 1 then
        player:addQuest(JEUNO, dsp.quests.ids.jeuno.SCATTERED_INTO_SHADOW)
    ...
    elseif csid == 135 then
        npcUtil.completeQuest(player, JEUNO, dsp.quests.ids.jeuno.SCATTERED_INTO_SHADOW, {item = 14097, fame = AF2_FAME, var = "scatIntoShadowCS"})
