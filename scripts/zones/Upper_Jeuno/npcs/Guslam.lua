...
function onTrigger(player,npc)
	...
	if(player:getMainLvl() >= 50 and player:getVar("BorghertzAlreadyActiveWithJob") == 0) then
		if(player:getMainJob() == 1 and 
		   player:getQuestStatus(BASTOK,THE_TALEKEEPER_S_TRUTH) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 2 and 
			   player:getQuestStatus(BASTOK,THE_FIRST_MEETING) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 3 and 
			   player:getQuestStatus(SANDORIA,PRELUDE_OF_BLACK_AND_WHITE) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 4 and 
			   player:getQuestStatus(WINDURST,RECOLLECTIONS) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 5 and 
			   player:getQuestStatus(SANDORIA,ENVELOPED_IN_DARKNESS) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 6 and 
			   player:getQuestStatus(WINDURST,AS_THICK_AS_THIEVES) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 7 and 
			   player:getQuestStatus(SANDORIA,A_BOY_S_DREAM) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 8 and 
			   player:getQuestStatus(BASTOK,DARK_PUPPET) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 9 and 
			   player:getQuestStatus(JEUNO,SCATTERED_INTO_SHADOW) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 10 and 
			   player:getQuestStatus(JEUNO,THE_REQUIEM) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 11 and 
			   player:getQuestStatus(WINDURST,FIRE_AND_BRIMSTONE) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 13 and 
			   player:getQuestStatus(OUTLANDS,I_LL_TAKE_THE_BIG_BOX) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 14 and 
			   player:getQuestStatus(SANDORIA,CHASING_QUOTAS) == QUEST_ACCEPTED and 
		...
		elseif(player:getMainJob() == 15 and 
			   player:getQuestStatus(WINDURST,CLASS_REUNION) == QUEST_ACCEPTED and 
