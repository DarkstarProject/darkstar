...
function onTrigger(player,npc)
    ...
    local SearchingForWords = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.SEARCHING_FOR_THE_RIGHT_WORDS);
...
function onEventFinish(player,csid,option)
    if ((csid == 192 and option == 1) or (csid == 193 and option == 1)) then --just start quest
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.A_CANDLELIGHT_VIGIL);
    ...
    elseif (csid == 194) then --finish quest
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.A_CANDLELIGHT_VIGIL);
        end
    ...
    elseif ((csid == 197 and option == 1) or (csid == 201 and option == 1)) then
        ...
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.SEARCHING_FOR_THE_RIGHT_WORDS);
    ...
    elseif (csid == 198) then --finish quest, note: no title granted
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:addFame(JEUNO, 30);
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.SEARCHING_FOR_THE_RIGHT_WORDS);
