...
function onTrigger(player,npc)
    ...
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.THE_UNFINISHED_WALTZ) == QUEST_ACCEPTED and player:getVar("QuestStatus_DNC_AF1")==1) then
    player:startEvent(10131);
    --Dancer AF: Road to Divadom
    elseif (player:getQuestStatus(JEUNO, dsp.quests.ids.jeuno.THE_ROAD_TO_DIVADOM) == QUEST_ACCEPTED)  then
