...
function onTrigger(player,npc)
    local aClockMostdelicate = player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.A_CLOCK_MOST_DELICATE);
    ...
    elseif (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.SAVE_THE_CLOCK_TOWER) == QUEST_COMPLETED and player:getQuestStatus(JEUNO,THE_CLOCKMASTER) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    if (csid == 119 and option == 1) then
        player:addQuest(JEUNO,dsp.quests.ids.jeuno.A_CLOCK_MOST_DELICATE);
    ...
    elseif (csid == 202) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.A_CLOCK_MOST_DELICATE);
            player:addQuest(JEUNO,dsp.quests.ids.jeuno.SAVE_THE_CLOCK_TOWER); -- Start next quest "Save the Clock Tower"
        end
    elseif (csid == 152) then
        if (player:getFreeSlotsCount() == 0) then
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED,17083);
        else
            player:addQuest(JEUNO,dsp.quests.ids.jeuno.THE_CLOCKMASTER);
            ...
            player:addFame(JEUNO,30);
            player:completeQuest(JEUNO,dsp.quests.ids.jeuno.THE_CLOCKMASTER);
        end
    end
end;
