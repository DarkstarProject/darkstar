...
function onTrigger(player,npc)
    if (ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30) then
        ...
        and player:getQuestStatus(ABYSSEA, dsp.quests.ids.abyssea.A_DELECTABLE_DEMON) == QUEST_AVAILABLE) then
...
function onEventFinish(player,csid,option)
    if (csid == 56) then
        player:addQuest(ABYSSEA, dsp.quests.ids.abyssea.A_DELECTABLE_DEMON);
    elseif (csid == 57) then
        -- Killed Cirein-croin
