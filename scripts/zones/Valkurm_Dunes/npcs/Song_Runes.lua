...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.PATH_OF_THE_BARD) == QUEST_AVAILABLE and player:getVar("PathOfTheBard_Event") == 1) then
        player:startEvent(2);
...
function onEventFinish(player,csid,option)
    if (csid == 2) then
        ...
        player:completeQuest(JEUNO,dsp.quests.ids.jeuno.PATH_OF_THE_BARD);
    end
end;
