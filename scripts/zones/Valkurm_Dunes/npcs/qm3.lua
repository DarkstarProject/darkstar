...
function onTrigger(player,npc)
    ...
    if (player:hasKeyItem(dsp.kis.YOMOTSU_HIRASAKA) and (cstime > 18 or cstime < 5) and not GetMobByID(DOMAN):isSpawned() and not GetMobByID(ONRYO):isSpawned()) then
        if (player:getVar("OkuriNMKilled") >= 1 and player:needToZone()) then
            player:delKeyItem(dsp.kis.YOMOTSU_HIRASAKA);
            player:addKeyItem(dsp.kis.FADED_YOMOTSU_HIRASAKA);
            player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.FADED_YOMOTSU_HIRASAKA);
