...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.OLDUUM) == QUEST_COMPLETED and player:hasItem(15769) == false) then
...
function onTrigger(player,npc)
    ...
    if (player:getQuestStatus(AHT_URHGAN,dsp.quests.ids.ahtUrhgan.NAVIGATING_THE_UNFRIENDLY_SEAS) == QUEST_ACCEPTED and player:getVar("NavigatingtheUnfriendlySeas") == 3) then
