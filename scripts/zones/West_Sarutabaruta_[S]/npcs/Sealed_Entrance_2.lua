    ...
    if (QuestStatus == QUEST_ACCEPTED and HasPutty) then
        if (MaskBit2 == false) then
            if (MaskBit1 == false or MaskBit3 == false) then
                ...
                player:messageSpecial(text.DOOR_OFFSET+1,ZONPAZIPPAS_ALLPURPOSE_PUTTY);
            else
                player:setMaskBit(player:getVar("SEALED_DOORS"),"SEALED_DOORS",1,true);
                player:messageSpecial(text.DOOR_OFFSET+4,ZONPAZIPPAS_ALLPURPOSE_PUTTY);
                player:delKeyItem(ZONPAZIPPAS_ALLPURPOSE_PUTTY);
            end
        else
            player:messageSpecial(text.DOOR_OFFSET+2,ZONPAZIPPAS_ALLPURPOSE_PUTTY);
        end
    elseif (player:getQuestStatus(CRYSTAL_WAR,SNAKE_ON_THE_PLAINS) == QUEST_COMPLETED) then
        player:messageSpecial(text.DOOR_OFFSET+2, ZONPAZIPPAS_ALLPURPOSE_PUTTY);
    else
        player:messageSpecial(text.DOOR_OFFSET+3);
    end
end;
