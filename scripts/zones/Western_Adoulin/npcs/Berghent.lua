...
function onTrigger(player,npc)
    local FOOL = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.FLAVORS_OF_OUR_LIVES);
...
function onEventFinish(player,csid,option)
    if (csid == 80) then
        if (option == 1) then
            -- Starts Quest: 'Flavors of Our Lives'
            player:addQuest(ADOULIN, dsp.quests.ids.adoulin.FLAVORS_OF_OUR_LIVES);
    ...
    elseif (csid == 87) then
        ...
        player:completeQuest(ADOULIN, dsp.quests.ids.adoulin.FLAVORS_OF_OUR_LIVES);
