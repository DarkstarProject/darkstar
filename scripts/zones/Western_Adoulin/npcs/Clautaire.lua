...
function onTrigger(player,npc)
    local FINAO = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.FAILURE_IS_NOT_AN_OPTION);
...
function onEventFinish(player,csid,option)
    if (csid == 78) then
        -- Starting Quest: 'F.A.I.L.ure Is Not an Option'
        player:addQuest(ADOULIN, dsp.quests.ids.adoulin.FAILURE_IS_NOT_AN_OPTION);
    elseif (csid == 76) then
        -- Finishing Quest: 'F.A.I.L.ure Is Not an Option'
        player:delKeyItem(dsp.ki.HUNK_OF_BEDROCK);
        player:completeQuest(ADOULIN, dsp.quests.ids.adoulin.FAILURE_IS_NOT_AN_OPTION);
