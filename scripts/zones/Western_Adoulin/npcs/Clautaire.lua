...
function onTrigger(player,npc)
    ...
    if (FINAO == QUEST_ACCEPTED) then
        if (player:hasKeyItem(dsp.kis.HUNK_OF_BEDROCK)) then
    ...
    elseif ((FINAO == QUEST_AVAILABLE) and (player:getFameLevel(ADOULIN) >= 4) and player:hasKeyItem(dsp.kis.FAIL_BADGE)) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 76) then
        -- Finishing Quest: 'F.A.I.L.ure Is Not an Option'
        player:delKeyItem(dsp.kis.HUNK_OF_BEDROCK);
