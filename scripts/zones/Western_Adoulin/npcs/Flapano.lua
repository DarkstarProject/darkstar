...
function onTrade(player,npc,trade)
    local exoticDelacacies = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.EXOTIC_DELICACIES);
...
function onTrigger(player,npc)
    local theWeatherspoonWar = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.THE_WEATHERSPOON_WAR);
    local exoticDelacacies = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.EXOTIC_DELICACIES);
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 2860 and option == 1) then
        player:addQuest(ADOULIN, dsp.quests.ids.adoulin.EXOTIC_DELICACIES);
    elseif (csid == 2861) then
        if (npcUtil.completeQuest(player, ADOULIN, dsp.quests.ids.adoulin.EXOTIC_DELICACIES, {bayld=500, item=5975, xp=1000})) then
            player:confirmTrade();
            player:setVar("Flapano_Odd_Even", 0);
        end
    end
