...
function onTrigger(player,npc)
    ...
    if ((Order_Up == QUEST_ACCEPTED) and (not Order_Grevan)) then
        ...
        if (player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.HYPOCRITICAL_OATH) == QUEST_COMPLETED) then
            if (player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.NOTSOCLEAN_BILL) == QUEST_COMPLETED) then
