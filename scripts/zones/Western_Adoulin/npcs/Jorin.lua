...
function onTrigger(player,npc)
    ...
    if (TOMATH == QUEST_ACCEPTED) then
        if (player:hasKeyItem(dsp.kis.EXTRAVAGANT_HARPOON)) then
...
function onEventFinish(player,csid,option)
    if (csid == 2540) then
        ...
        player:addKeyItem(dsp.kis.BROKEN_HARPOON);
        player:messageSpecial(KEYITEM_OBTAINED, dsp.kis.BROKEN_HARPOON);
    elseif (csid == 2542) then
        ...
        player:delKeyItem(dsp.kis.EXTRAVAGANT_HARPOON);
