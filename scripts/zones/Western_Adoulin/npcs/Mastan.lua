...
function onTrigger(player,npc)
    local TCCOM = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.THE_CURIOUS_CASE_OF_MELVIEN);
    local TCCOM_Need_KI = player:hasKeyItem(dsp.ki.MELVIENS_TURN) and (not player:hasKeyItem(dsp.ki.MELVIENS_DEATH))
    local Order_Up = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.ORDER_UP);
