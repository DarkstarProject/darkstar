...
function onTrigger(player,npc)
    ...
    local TCCOM_Need_KI = player:hasKeyItem(dsp.kis.MELVIENS_TURN) and (not player:hasKeyItem(dsp.kis.MELVIENS_DEATH))
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 184) then
        -- Progresses Quest: 'The Curious Case of Melvien'
        if (option == 1) then
            player:addKeyItem(dsp.kis.MELVIENS_DEATH);
            player:messageSpecial(KEYITEM_OBTAINED, dsp.kis.MELVIENS_DEATH);
        end
    end
end;
