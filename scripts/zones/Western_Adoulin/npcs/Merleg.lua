...
function onTrigger(player,npc)
    local APBIF = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.A_PIONEERS_BEST_IMAGINARY_FRIEND);
...
function onEventFinish(player,csid,option)
    if (csid == 2520) then
        ...
        player:addQuest(ADOULIN, dsp.quests.ids.adoulin.A_PIONEERS_BEST_IMAGINARY_FRIEND);
        if (player:hasStatusEffect(dsp.effect.IONIS)) then
    ...
    elseif (csid == 2522) then
        -- Finishing Quest: 'A Pioneers Best (Imaginary) Friend'
        player:completeQuest(ADOULIN, dsp.quests.ids.adoulin.A_PIONEERS_BEST_IMAGINARY_FRIEND);
