...
function onTrigger(player,npc)
    ...
    local ATFTA_Need_KI = ((player:getVar("ATFTA_Status") < 2) and (not player:hasKeyItem(dsp.kis.COPY_OF_THE_ALLIANCE_AGREEMENT)));
...
function onEventFinish(player,csid,option)
    if (csid == 5053) then
        -- Progresses Quest: 'A Thirst for the Ages'
        player:addKeyItem(dsp.kis.COPY_OF_THE_ALLIANCE_AGREEMENT);
        player:messageSpecial(KEYITEM_OBTAINED, dsp.kis.COPY_OF_THE_ALLIANCE_AGREEMENT);
    end
end;
