...
function onTrigger(player,npc)
    ...
    if (SOA_Mission >= LIFE_ON_THE_FRONTIER) then
        if ((TOMATH == QUEST_ACCEPTED) and player:hasKeyItem(dsp.kis.BROKEN_HARPOON)) then
            -- Progresses Quest: 'The Old Man and the Harpoon'
            player:startEvent(2543);
        elseif ((Fertile_Ground == QUEST_ACCEPTED) and (not player:hasKeyItem(dsp.kis.BOTTLE_OF_FERTILIZER_X))) then
            -- Progresses Quest: 'Fertile Ground'
            player:startEvent(2850);
        elseif (Wayward_Waypoints and (not player:hasKeyItem(dsp.kis.WAYPOINT_RECALIBRATION_KIT))) then
...
function onEventFinish(player,csid,option)
    if (csid == 2543) then
        -- Progresses Quest: 'The Old Man and the Harpoon'
        player:delKeyItem(dsp.kis.BROKEN_HARPOON);
        player:addKeyItem(dsp.kis.EXTRAVAGANT_HARPOON);
        player:messageSpecial(KEYITEM_OBTAINED, dsp.kis.EXTRAVAGANT_HARPOON);
    elseif (csid == 2850) then
        -- Progresses Quest: 'Fertile Ground'
        player:addKeyItem(dsp.kis.BOTTLE_OF_FERTILIZER_X);
    elseif (csid == 79) then
        player:addKeyItem(dsp.kis.WAYPOINT_RECALIBRATION_KIT);
