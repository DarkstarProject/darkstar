...
function onTrigger(player,npc)
    local TOMATH = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.THE_OLD_MAN_AND_THE_HARPOON);
    local Fertile_Ground = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.FERTILE_GROUND);
    local Wayward_Waypoints = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.WAYWARD_WAYPOINTS);
    Wayward_Waypoints = (Wayward_Waypoints == QUEST_ACCEPTED) and (player:getVar("WW_Need_Shipilolo") > 0)
    local ACSP = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.A_CERTAIN_SUBSTITUTE_PATROLMAN);
