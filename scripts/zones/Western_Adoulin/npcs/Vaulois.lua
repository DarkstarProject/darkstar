...
function onTrigger(player,npc)
    local Transporting = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.TRANSPORTING);
...
function onEventFinish(player,csid,option)
    if (csid == 2590) then
        ...
        player:addQuest(ADOULIN, dsp.quests.ids.adoulin.TRANSPORTING);
    elseif (csid == 2591) then
        -- Finishing Quest: 'Transporting'
        player:completeQuest(ADOULIN, dsp.quests.ids.adoulin.TRANSPORTING);
