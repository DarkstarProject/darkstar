...
function onTrade(player,npc,trade)
    local HS = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.HUNGER_STRIKES);
    local TS = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.THE_STARVING);
    local AMQTR = player:getQuestStatus(ADOULIN, dsp.quests.ids.adoulin.ALWAYS_MORE_QUOTH_THE_RAVENOUS);
...
function onEventFinish(player,csid,option)
    if (csid == 2530) then
        ...
        player:addQuest(ADOULIN, dsp.quests.ids.adoulin.HUNGER_STRIKES);
    elseif ((csid == 2532) or (csid == 3007)) then
        ...
        if (csid == 2532) then
            player:completeQuest(ADOULIN, dsp.quests.ids.adoulin.HUNGER_STRIKES);
        elseif (csid == 3007) then
            player:completeQuest(ADOULIN, dsp.quests.ids.adoulin.THE_STARVING);
        end
    elseif (csid == 3005) then
        -- Starting Quest: 'The Starving'
        player:addQuest(ADOULIN, dsp.quests.ids.adoulin.THE_STARVING);
    elseif (csid == 3010) then
        -- Starting Quest: 'Always More Quoth the Ravenous'
        player:addQuest(ADOULIN, dsp.quests.ids.adoulin.ALWAYS_MORE_QUOTH_THE_RAVENOUS);
    elseif (csid == 3012) then
        -- Finishing Quest: 'Always More Quoth The Ravenous'
        player:tradeComplete();
        player:completeQuest(ADOULIN, dsp.quests.ids.adoulin.ALWAYS_MORE_QUOTH_THE_RAVENOUS);
