...
function onTrigger(player,npc)
    QuestStatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_POSTMAN_ALWAYS_KO_S_TWICE);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 48 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_POSTMAN_ALWAYS_KO_S_TWICE);
    elseif (csid == 52) then
        player:tradeComplete();
        player:addGil(GIL_RATE*50);
        player:addFame(WINDURST,80);
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.THE_POSTMAN_ALWAYS_KO_S_TWICE);
    elseif (csid == 53) then
        player:tradeComplete();
