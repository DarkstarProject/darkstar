...
function onTrigger(player,npc)
    ...
    local CFA2 = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_2);
    ...
    elseif (MakingHeadlines == 1) then
        ...
        if (testflag(tonumber(prog),4) == false) then
            if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_1) == 1) then
