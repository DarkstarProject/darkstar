...
function onTrade(player,npc,trade)
    local qStarStruck = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.STAR_STRUCK);
    local count = trade:getItemCount();

    if (trade:hasItemQty(544,1) and count == 1 and trade:getGil() == 0) then
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_THE_GRADE) == QUEST_ACCEPTED) then
    ...
    elseif (trade:hasItemQty(1127,1) and trade:getItemCount() == 1) then -- Trade Kindred seal
        if (player:getQuestStatus(JEUNO,dsp.quests.ids.jeuno.RIDING_ON_THE_CLOUDS) == QUEST_ACCEPTED and player:getVar("ridingOnTheClouds_4") == 4) then
    ...
    elseif (trade:hasItemQty(16511,1) and count == 1 and trade:getGil() == 0) then
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.BLAST_FROM_THE_PAST) == QUEST_ACCEPTED) then
    ...
    elseif (trade:hasItemQty(829,1) and count == 1 and trade:getGil() == 0) then
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_ROOT_OF_THE_PROBLEM) == QUEST_ACCEPTED) then
    ...
    elseif (trade:hasItemQty(17299,4) and count == 4 and trade:getGil() == 0) then -- trade:getItemCount() is apparently checking total of all 8 slots combined. Could have sworn that wasn't how it worked before.
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CLASS_REUNION) == QUEST_ACCEPTED and player:getVar("ClassReunionProgress") == 2) then
...
function onTrigger(player,npc)
    local qStarStruck = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.STAR_STRUCK);
    local blastFromPast = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.BLAST_FROM_THE_PAST);
    local blastProg = player:getVar("BlastFromThePast_Prog");
    local rootProblem = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_ROOT_OF_THE_PROBLEM);
    ...
    if (blastFromPast == QUEST_AVAILABLE and qStarStruck == QUEST_COMPLETED and player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CLASS_REUNION) ~= QUEST_ACCEPTED and player:getFameLevel(WINDURST) >= 3 and player:needToZone() == false) then
    ...
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_THE_GRADE) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 211) then
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.STAR_STRUCK);
    ...
    elseif (csid == 197 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.STAR_STRUCK);
    elseif (csid == 214 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.BLAST_FROM_THE_PAST);
    elseif (csid == 224) then
        player:tradeComplete();
        player:setVar("BlastFromThePast_Prog",0);
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.BLAST_FROM_THE_PAST);
    ...
    elseif (csid == 404) then
        if (player:getFreeSlotsCount() ~= 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.THE_PUPPET_MASTER);
    ...
    elseif (csid == 410) then
        if (player:getFreeSlotsCount() ~= 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.CLASS_REUNION);
    ...
    elseif (csid == 419) then
        if (player:getFreeSlotsCount() ~= 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.CARBUNCLE_DEBACLE);
