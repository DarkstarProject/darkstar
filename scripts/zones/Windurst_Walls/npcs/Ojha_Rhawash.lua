...
itemQuality = 0;
    ...
    FlowerChild = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.FLOWER_CHILD);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 10000 and option == 3002) then
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.FLOWER_CHILD);
        player:addFame(WINDURST,120);
        player:moghouseFlag(4);
        player:messageSpecial(ID.text.MOGHOUSE_EXIT);
    elseif (csid == 10000 and option == 1) then
        player:tradeComplete();
        player:addQuest(WINDURST,dsp.quests.ids.windurst.FLOWER_CHILD);
    end
end;