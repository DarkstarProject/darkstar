...
function onTrade(player,npc,trade)
    ...
    -- Curses Foiled Again!
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_1) == QUEST_ACCEPTED) then
    ...
    -- Curses,Foiled ... Again!?
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_2) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    local foiledAgain = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_1)
    ...
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.LURE_OF_THE_WILDCAT_WINDURST) == QUEST_ACCEPTED and player:getMaskBit(WildcatWindurst,6) == false) then
        player:startEvent(498)
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CLASS_REUNION) == QUEST_ACCEPTED and player:getVar("ClassReunionProgress") == 3) then
...
function onEventFinish(player,csid,option)
    if (csid == 173) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_1)
        end
    elseif (csid == 171 and option ~= 1) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_1)
    ...
    elseif (csid == 180 and option == 3) then
        ...
        player:addQuest(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_2)
    ...
    elseif (csid == 183) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_AGAIN_2)
            player:needToZone(true)
    ...
    elseif (csid == 340) then
        if (option == 1) then
            player:addQuest(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_A_GOLEM)
        else
    ...
    elseif (csid == 342) then
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.CURSES_FOILED_A_GOLEM)
