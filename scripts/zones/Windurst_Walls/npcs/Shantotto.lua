...
--printf("RESULT: %u",option);
	...
	elseif(csid == 0x00B4 and option == 3) then
		...
		player:setTitle(title.TARUTARU_MURDER_SUSPECT);
	...
	elseif(csid == 0x00B7) then
		if (player:getFreeSlotsCount() == 0) then 
			...
			player:tradeComplete();
			player:setTitle(title.HEXER_VEXER);
	...
	elseif(csid == 0x0154) then
		if(option == 1) then
			...
			player:setTitle(title.TOTAL_LOSER);
		end
	...
	elseif(csid == 0x0156) then
		if (player:getFreeSlotsCount() == 0) then 
			...
			player:setTitle(title.DOCTOR_SHANTOTTOS_FLAVOR_OF_THE_MONTH);
