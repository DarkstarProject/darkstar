...
function onTrade(player,npc,trade)
    local MandragoraMad = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MANDRAGORA_MAD);
    if (MandragoraMad ~= QUEST_AVAILABLE) then
        if (trade:getItemCount() == 1) then
            ...
            if (trade:hasItemQty(17344,1)) then
                ...
                player:completeQuest(WINDURST,dsp.quests.ids.windurst.MANDRAGORA_MAD);
            -- Sulfur
...
function onEventFinish(player,csid,option)
    if (csid == 249) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.MANDRAGORA_MAD);
