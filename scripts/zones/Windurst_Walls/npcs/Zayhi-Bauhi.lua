...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TO_BEE_OR_NOT_TO_BEE) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    local PostmanKOsTwice = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_POSTMAN_ALWAYS_KO_S_TWICE);
    local ToBeeOrNotStatus = player:getVar("ToBeeOrNot_var");
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 75) then -- After Honey#5: ToBee quest Finish (tooth hurts from all the Honey)
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.TO_BEE_OR_NOT_TO_BEE);
