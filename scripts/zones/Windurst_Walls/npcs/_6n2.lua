...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(WINDURST) == LOST_FOR_WORDS and player:getVar("MissionStatus") == 5) then
        ...
        ----------------------------------------------------
        -- SMN unlock quest
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.I_CAN_HEAR_A_RAINBOW) == QUEST_AVAILABLE and player:getMainLvl() >= 30 and player:hasItem(1125)) then
            player:startEvent(384,1125,1125,1125,1125,1125,1125,1125,1125);
        elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.I_CAN_HEAR_A_RAINBOW) == QUEST_ACCEPTED) then
        ...
        elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_PUPPET_MASTER) == QUEST_ACCEPTED and player:getVar("ThePuppetMasterProgress") == 1) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 289) then
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.ONION_RINGS);
    ...
    elseif (csid == 384) then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.I_CAN_HEAR_A_RAINBOW);
    ...
    elseif (csid == 402) then
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_PUPPET_MASTER) == QUEST_COMPLETED) then
            player:delQuest(WINDURST,dsp.quests.ids.windurst.THE_PUPPET_MASTER);
            player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_PUPPET_MASTER); -- this needs only if you repeat this quest
        end;
        player:setVar("ThePuppetMasterProgress",1);
        player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_PUPPET_MASTER);
    elseif (csid == 413) then
        player:setVar("ClassReunionProgress",1);
        player:addQuest(WINDURST,dsp.quests.ids.windurst.CLASS_REUNION);
    ...
    elseif (csid == 415) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.CARBUNCLE_DEBACLE);
