...
function onTrigger(player,npc)
    posestatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_POSE_BY_ANY_OTHER_NAME);
...
function onEventFinish(player,csid,option)
    if (csid == 92) then -------------------------- QUEST START
        player:addQuest(WINDURST,dsp.quests.ids.windurst.A_POSE_BY_ANY_OTHER_NAME);
    ...
    elseif (csid == 96) then  --------------------- QUEST FINFISH
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.A_POSE_BY_ANY_OTHER_NAME)
    ...
    elseif (csid == 102) then  ---------------------- QUEST FAILURE
        player:delQuest(WINDURST,dsp.quests.ids.windurst.A_POSE_BY_ANY_OTHER_NAME);
