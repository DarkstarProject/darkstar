...
function onTrade(player,npc,trade)
    featherstatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP);
...
--    player:delQuest(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP);  -- ================== FOR TESTING ONLY =====================
--    player:addFame(WINDURST,200);   -- ================== FOR TESTING ONLY =====================

    function testflag(set,flag)
        return (set % (2*flag) >= flag)
    end
    hatstatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.HAT_IN_HAND);
    featherstatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP);
...
printf("RESULT: %u",option);
    if (csid == 48 and option == 1) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.HAT_IN_HAND);
...
--        else (option == 1) then     -- 0/nill = NO REWARD      >= 0 NPCS - Option 1
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.HAT_IN_HAND);
    ...
    elseif (csid == 75 and option == 1) then
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP) == QUEST_AVAILABLE) then
            player:addQuest(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP);
        elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP) == QUEST_COMPLETED) then
            player:setVar("QuestFeatherInOnesCap_var",1);
        end
    elseif (csid == 79) then
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP) == QUEST_ACCEPTED) then
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.A_FEATHER_IN_ONE_S_CAP);
            player:addFame(WINDURST,75);
        else
            player:addFame(WINDURST,8);
