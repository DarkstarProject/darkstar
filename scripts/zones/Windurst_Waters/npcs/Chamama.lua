...
function onTrade(player,npc,trade)
    ...
    local InAPickle = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.IN_A_PICKLE);
    ...
    elseif (FakeMoustache == false) then
        local InspectorsGadget = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.INSPECTOR_S_GADGET);
    ...
    elseif (InvisibleManSticker == false) then
        local ThePromise = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_PROMISE);
...
-- player:delQuest(WINDURST,dsp.quests.ids.windurst.IN_A_PICKLE); [[[[[[[[[[[[[ FOR TESTING ONLY ]]]]]]]]]]]]]
end;
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 654 and option == 1) then  -- IN A PICKLE + RARAB TAIL: Quest Begin
        player:addQuest(WINDURST,dsp.quests.ids.windurst.IN_A_PICKLE);
    elseif (csid == 659) then  -- IN A PICKLE: Quest Turn In (1st Time)
        player:tradeComplete(trade);
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.IN_A_PICKLE);
