...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_THE_GRADE) == QUEST_ACCEPTED and player:getVar("QuestMakingTheGrade_prog") == 0) then
...
function onTrigger(player,npc)
    ...
    local gradestatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MAKING_THE_GRADE);
    local prog = player:getVar("QuestMakingTheGrade_prog");
        -- 1 = answers found
        -- 2 = gave test answers to principle
        -- 3 = spoke to chomoro

    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TEACHER_S_PET) == QUEST_COMPLETED and gradestatus == QUEST_AVAILABLE and player:getFameLevel(WINDURST) >=3 and player:getQuestStatus(WINDURST,LET_SLEEPING_DOGS_LIE) ~= QUEST_ACCEPTED) then
    ...
    -------------------------------------------------------
    -- Class Reunion
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CLASS_REUNION) == QUEST_ACCEPTED and player:getVar("ClassReunionProgress") >= 3 and player:getVar("ClassReunion_TalkedToFupepe") ~= 1) then
...
function onEventFinish(player,csid,option)
    if (csid == 442 and option == 1) then -- Quest Start
        player:addQuest(WINDURST,dsp.quests.ids.windurst.MAKING_THE_GRADE);
    ...
    elseif (csid == 458) then -- Quest Finish
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.MAKING_THE_GRADE);
