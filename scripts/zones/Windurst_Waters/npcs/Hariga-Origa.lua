...
function onEventFinish(player,csid,option)
    ...
    if (csid == 0x017d and option == 0) then
        ...
        player:messageSpecial(TextIDs.KEYITEM_OBTAINED,NOTES_FROM_HARIGAORIGA);
    elseif (csid == 0x0181) then
        ...
        if (player:hasKeyItem(MAP_OF_THE_HORUTOTO_RUINS) == false) then
            ...
            player:messageSpecial(TextIDs.KEYITEM_OBTAINED,MAP_OF_THE_HORUTOTO_RUINS);
        end
    ...
    elseif (csid == 0x01a1) then
        ...
        player:messageSpecial(TextIDs\.GIL_OBTAINED,GIL_RATE*3000);
        if (player:hasKeyItem(MAP_OF_FEIYIN) == false) then
            player:addKeyItem(MAP_OF_FEIYIN);
            player:messageSpecial(TextIDs.KEYITEM_OBTAINED,MAP_OF_FEIYIN);
        end
        player:addFame(WINDURST,120);
