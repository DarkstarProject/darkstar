...
function onTrade(player,npc,trade)
    ...
    smudgeStatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_SMUDGE_ON_ONE_S_RECORD);
...
function onTrigger(player,npc)
    ...
    GlyphHanger = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.GLYPH_HANGER);
    chasingStatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CHASING_TALES);
    smudgeStatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.A_SMUDGE_ON_ONE_S_RECORD);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 381 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.GLYPH_HANGER);
    ...
    elseif (csid == 385) then
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.GLYPH_HANGER);
    elseif (csid == 413 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.A_SMUDGE_ON_ONE_S_RECORD);
    elseif (csid == 417) then
        ...
        player:addFame(WINDURST,120);
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.A_SMUDGE_ON_ONE_S_RECORD);
    end
