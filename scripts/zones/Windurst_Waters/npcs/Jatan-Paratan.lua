...
function onTrigger(player,npc)
            ...
            --        player:delQuest(WINDURST,dsp.quests.ids.windurst.WONDERING_MINSTREL);
...
function onEventFinish(player,csid,option)
    if (csid == 634) then    -- WONDERING_MINSTREL: Quest Start
        player:addQuest(WINDURST,dsp.quests.ids.windurst.WONDERING_MINSTREL);
    elseif (csid == 638) then  -- WONDERING_MINSTREL: Quest Finish
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.WONDERING_MINSTREL)
