...
function onTrigger(player,npc)
    ...
    local hatstatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.HAT_IN_HAND);
    local KenapaFood = player:getVar("Kenapa_Food_var"); -- Variable to track progress of Kenapa-Keppa in Food for Thought
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 339) then
        if (option == 0) then
            player:addQuest(WINDURST,dsp.quests.ids.windurst.OVERNIGHT_DELIVERY);
    ...
    elseif (csid == 346) then
        player:delQuest(WINDURST,dsp.quests.ids.windurst.OVERNIGHT_DELIVERY);
    ...
    elseif (csid == 348) then
        if (player:getFreeSlotsCount() > 0) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.OVERNIGHT_DELIVERY);
            player:addFame(WINDURST,100);
            player:needToZone(true);
            player:setVar("Kenapa_Overnight_var",0);
            player:setVar("Kenapa_Overnight_Hour_var",0);
