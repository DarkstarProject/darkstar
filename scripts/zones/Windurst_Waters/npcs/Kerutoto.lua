...
function onTrade(player,npc,trade)
    ...
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.BLUE_RIBBON_BLUES) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    local waking_dreams = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WAKING_DREAMS)
    ...
    elseif (player:hasKeyItem(dsp.ki.VIAL_OF_DREAM_INCENSE)==false and ((player:hasCompletedMission(COP,DARKNESS_NAMED) and  waking_dreams == QUEST_AVAILABLE ) or(waking_dreams  == QUEST_COMPLETED and realday ~= player:getVar("Darkness_Named_date")))) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.WAKING_DREAMS);
    ...
    elseif (BlueRibbonBlues == QUEST_AVAILABLE and player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WATER_WAY_TO_GO) == QUEST_COMPLETED and player:getFameLevel(WINDURST) >= 5) then
...
function onEventFinish(player,csid,option)
    ...
    elseif ((csid == 313 and option == 0) or (csid == 314 and option == 0)) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.FOOD_FOR_THOUGHT);
    ...
    elseif (csid == 357) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.BLUE_RIBBON_BLUES);
    ...
    elseif (csid == 362) then
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.BLUE_RIBBON_BLUES);
    ...
    elseif (csid == 920) then    --diablos reward
        ...
        elseif (option == 5) then
            ...
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.WAKING_DREAMS);
