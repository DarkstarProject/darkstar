...
function onTrigger(player,npc)
    ...
    local ecoWarWindurst = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.ECO_WARRIOR_WIN);
...
function onEventFinish(player,csid,option)
    if (csid == 818 and option == 1) then
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.ECO_WARRIOR_WIN) == QUEST_AVAILABLE) then
            player:addQuest(WINDURST,dsp.quests.ids.windurst.ECO_WARRIOR_WIN);
        end
        player:setVar("ECO_WARRIOR_ACTIVE",player:getZoneID());
        player:setVar("ECO-WAR_ConquestWeek",0);
    elseif (csid == 822) then
        if (player:getFreeSlotsCount() >= 1) then
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.ECO_WARRIOR_WIN);
