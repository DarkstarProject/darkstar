...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.HOIST_THE_JELLY_ROGER) == QUEST_ACCEPTED) then
...
function onEventFinish(player,csid,option)
    if (csid == 10000) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.HOIST_THE_JELLY_ROGER);
    elseif (csid == 10001) then
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.HOIST_THE_JELLY_ROGER);
