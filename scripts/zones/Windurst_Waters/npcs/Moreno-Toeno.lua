...
function onTrade(player,npc,trade)
    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TEACHER_S_PET) >= 1 and trade:hasItemQty(847,1) == true and trade:hasItemQty(4368,1) == true and trade:getGil() == 0 and trade:getItemCount() == 2) then
...
function onTrigger(player,npc)
    ...
    local teacherstatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TEACHER_S_PET);
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 438 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.TEACHER_S_PET);
    ...
    elseif (csid == 440) then
        ...
        if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TEACHER_S_PET) == QUEST_ACCEPTED) then
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.TEACHER_S_PET);
