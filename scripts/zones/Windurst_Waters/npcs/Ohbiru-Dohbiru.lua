...
function onTrade(player,npc,trade)
    local turmoil = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TORAIMARAI_TURMOIL);
    local count = trade:getItemCount();


    if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WATER_WAY_TO_GO) == QUEST_ACCEPTED) then
        if (trade:hasItemQty(4351,1) and count == 1) then
            player:startEvent(355,900);
        end
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.FOOD_FOR_THOUGHT) == QUEST_ACCEPTED) then
...
function onTrigger(player,npc)
    ...
    local turmoil = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TORAIMARAI_TURMOIL);
    local FoodForThought = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.FOOD_FOR_THOUGHT);
    local needToZone = player:needToZone();
    local OhbiruFood = player:getVar("Ohbiru_Food_var"); -- Variable to track progress of Ohbiru-Dohbiru in Food for Thought
    local waterWayToGo = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WATER_WAY_TO_GO);
    local overnightDelivery = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.OVERNIGHT_DELIVERY);
    local SayFlowers = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.SAY_IT_WITH_FLOWERS);
    local FlowerProgress = player:getVar("FLOWER_PROGRESS");
    local blueRibbonBlues = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.BLUE_RIBBON_BLUES)
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 322 or csid == 325 or csid == 326) then
        ...
        if (player:getVar("Kerutoto_Food_var") == 2 and player:getVar("Kenapa_Food_var") == 4) then -- If this is the last NPC to be fed
            player:completeQuest(WINDURST,dsp.quests.ids.windurst.FOOD_FOR_THOUGHT);
    ...
    elseif (csid == 785 and option == 1) then -- Adds Toraimarai turmoil
        player:addQuest(WINDURST,dsp.quests.ids.windurst.TORAIMARAI_TURMOIL);
    ...
    elseif (csid == 791 and turmoil == QUEST_ACCEPTED) then -- Completes Toraimarai turmoil - first time
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.TORAIMARAI_TURMOIL);
        player:addFame(WINDURST,100);
    ...
    elseif (csid == 352 and option == 0 or csid == 354) then
        if (player:getFreeSlotsCount() >= 1) then
            if (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.WATER_WAY_TO_GO) == QUEST_AVAILABLE) then
                player:addQuest(WINDURST,dsp.quests.ids.windurst.WATER_WAY_TO_GO);
            end
            player:addItem(504);
    ...
    elseif (csid == 355) then
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.WATER_WAY_TO_GO);
