...
function onTrigger(player,npc)
    ...
    if (player:getNation() == guardnation and player:getVar("supplyQuest_started") > 0 and supplyRunFresh(player) == 0) then
        player:showText(npc,TextIDs.CONQUEST + 40); -- "We will dispose of those unusable supplies."
        local region = player:getVar("supplyQuest_region");
        player:delKeyItem(getSupplyKey(region));
        player:messageSpecial(TextIDs.KEYITEM_OBTAINED + 1,getSupplyKey(region));
...
function onEventFinish(player,csid,option)
                ...
                if (player:getFreeSlotsCount() >= 1) then
                    ...
                    if (option >= 32933 and option <= 32935) then
                        if (checkConquestRing(player) > 0) then
                            player:messageSpecial(TextIDs.CONQUEST+60,0,0,inventory[Item+2]);
                            break;
                        else
                    ...
                    if (player:hasItem(inventory[Item + 2]) == false) then
                        ...
                        player:messageSpecial(TextIDs\.ITEM_OBTAINED,inventory[Item + 2]);
                    else
                        player:messageSpecial(TextIDs\.ITEM_CANNOT_BE_OBTAINED,inventory[Item + 2]);
                    end;
                else
                    player:messageSpecial(TextIDs\.ITEM_CANNOT_BE_OBTAINED,inventory[Item + 2]);
                end;
    ...
    elseif (option >= 65541 and option <= 65565) then -- player chose supply quest.
        ...
        player:messageSpecial(TextIDs.KEYITEM_OBTAINED,getSupplyKey(region));
