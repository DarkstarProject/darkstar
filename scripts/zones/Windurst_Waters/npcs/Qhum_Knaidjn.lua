...
function onTrade(player,npc,trade)
    unionRepresentativeTrade(player, npc, trade, 0x2729, 8, TextIDs);
end;
...
function onEventFinish(player,csid,option,target)
    ...
    if (csid == 0x2728) then
        unionRepresentativeTriggerFinish(player, option, target, 8, "guild_cooking", keyitems, items, TextIDs);
    elseif (csid == 0x2729) then
        player:messageSpecial(TextIDs.GP_OBTAINED, option);
    end
end;
