...
function onTrigger(player,npc)
    ...
    IAS = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.IN_A_STEW);
    IASvar = player:getVar("IASvar");
...
function onEventFinish(player,csid,option)
    ...
    if (csid == 258 and option == 1) then  -- A Crisis in the Making + ITEM: Quest Offer - ACCEPTED
        player:addQuest(WINDURST,dsp.quests.ids.windurst.A_CRISIS_IN_THE_MAKING);
    ...
    elseif (csid == 267) then -- A Crisis in the Making: Quest Finish
        ...
        player:completeQuest(WINDURST,dsp.quests.ids.windurst.A_CRISIS_IN_THE_MAKING);
        player:needToZone(true);
