...
function onTrigger(player,npc)
    ...
    local bookwormStatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.EARLY_BIRD_CATCHES_THE_BOOKWORM);
    local glyphStatus = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.GLYPH_HANGER);
    ...
    elseif (player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.HAT_IN_HAND) == QUEST_ACCEPTED or player:getVar("QuestHatInHand_var2") == 1) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 387 and option == 0) then -- Early Bird Catches the Bookworm
        player:addQuest(WINDURST,dsp.quests.ids.windurst.EARLY_BIRD_CATCHES_THE_BOOKWORM);
    elseif (csid == 403 and option == 0) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.CHASING_TALES);
