...
function onTrigger(player,npc)
    ...
    local TheFightingFourth = player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.THE_FIGHTING_FOURTH)
    local SnakeOnThePlains = player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.SNAKE_ON_THE_PLAINS)
    local SteamedRams = player:getQuestStatus(CRYSTAL_WAR, dsp.quests.ids.crystalWar.STEAMED_RAMS)
...
function onEventFinish(player,csid,option)
    if csid == 103 and option == 0 then
        player:addQuest(CRYSTAL_WAR, dsp.quests.ids.crystalWar.SNAKE_ON_THE_PLAINS)
    ...
    elseif csid == 104 and option == 1 then
        player:delQuest(CRYSTAL_WAR, dsp.quests.ids.crystalWar.SNAKE_ON_THE_PLAINS)
    ...
    elseif csid == 106 and option == 0 then
        ...
        if player:getVar("Campaign_Nation") == 0 then
            if player:getFreeSlotsCount() >= 1 then
                ...
                player:completeQuest(CRYSTAL_WAR, dsp.quests.ids.crystalWar.SNAKE_ON_THE_PLAINS)
                player:setVar("SEALED_DOORS", 0)
