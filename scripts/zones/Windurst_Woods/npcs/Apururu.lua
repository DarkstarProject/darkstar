...
function onTrade(player,npc,trade)
    ...
    -- CAN CARDIANS CRY?
    elseif player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.CAN_CARDIANS_CRY) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, 551) then
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 319 then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.CAN_CARDIANS_CRY)
    elseif csid == 325 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.CAN_CARDIANS_CRY, {gil=5000}) then
        player:confirmTrade()
    end
end