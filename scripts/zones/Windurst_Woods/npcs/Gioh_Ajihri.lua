...
function onTrigger(player,npc)
    local twinstoneBonding = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TWINSTONE_BONDING)
...
function onEventFinish(player,csid,option)
    if csid == 487 then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.TWINSTONE_BONDING)
    ...
    elseif csid == 490 then
        ...
        if player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.TWINSTONE_BONDING) == QUEST_ACCEPTED then
            npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.TWINSTONE_BONDING, {item=17154, fame=80, title=dsp.title.BOND_FIXER})
        else
