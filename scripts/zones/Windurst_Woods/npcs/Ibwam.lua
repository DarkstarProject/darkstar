...
function onTrade(player,npc,trade)
    if npcUtil.tradeHas(trade, {{"gil", 300}}) and player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.LURE_OF_THE_WILDCAT_WINDURST) == QUEST_COMPLETED and player:getCurrentMission(TOAU) > IMMORTAL_SENTRIES then
...
function onTrigger(player,npc)
    local lureWindurst = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.LURE_OF_THE_WILDCAT_WINDURST)
...
function onEventFinish(player,csid,option)
    if csid == 736 then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.LURE_OF_THE_WILDCAT_WINDURST)
        player:setVar("WildcatWindurst", 0)
        npcUtil.giveKeyItem(player, dsp.ki.GREEN_SENTINEL_BADGE)
    elseif csid == 739 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.LURE_OF_THE_WILDCAT_WINDURST, {fame=150, keyItem=dsp.ki.GREEN_INVITATION_CARD, var="WildcatWindurst"}) then
