...
function onTrade(player,npc,trade)
    if player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CREEPY_CRAWLIES) ~= QUEST_AVAILABLE then
...
function onEventFinish(player,csid,option)
    if csid == 333 and option == 1 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.CREEPY_CRAWLIES)
    elseif csid == 335 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.CREEPY_CRAWLIES, {gil=600, fame=0, title=dsp.title.CRAWLER_CULLER}) then
        player:confirmTrade()
    end
end