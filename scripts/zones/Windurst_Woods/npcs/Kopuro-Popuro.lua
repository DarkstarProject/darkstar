...
function onTrade(player,npc,trade)
    local allNewC2000 = player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.THE_ALL_NEW_C_2000)
    local legendaryPlanB = player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.LEGENDARY_PLAN_B)
...
function onEventFinish(player,csid,option)
    ...
    if csid == 285 and option ~= 2 then  -- option 2 is declining the quest for the second question
        player:addQuest(WINDURST, dsp.quests.ids.windurst.THE_ALL_NEW_C_2000)
    elseif csid == 292 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.THE_ALL_NEW_C_2000, {fame=80, title=dsp.title.CARDIAN_TUTOR, gil=200}) then
        player:confirmTrade()

    -- LEGENDARY PLAN B
    elseif csid == 308 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.LEGENDARY_PLAN_B)
    elseif csid == 314 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.LEGENDARY_PLAN_B, {item=12749, gil=700}) then
        player:confirmTrade()
        player:needToZone(true)

    -- THE ALL NEW C-3000
    elseif csid == 655 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.THE_ALL_NEW_C_3000)
    elseif csid == 657 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.THE_ALL_NEW_C_3000, {fame=10, gil=600}) then
        player:confirmTrade()
    end
end