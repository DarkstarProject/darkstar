...
function onTrigger(player,npc)
    ...
    local C2000 = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_ALL_NEW_C_2000) -- previous quest in line
    local AGreetingCardian = player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.A_GREETING_CARDIAN)
    local LPB = player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.LEGENDARY_PLAN_B)
...
function onEventFinish(player,csid,option)
    -- A Greeting Cardian
    if csid == 296 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.A_GREETING_CARDIAN)
    ...
    elseif csid == 303 then
        if player:getFreeSlotsCount() == 0 then
            ...
            player:completeQuest(WINDURST, dsp.quests.ids.windurst.A_GREETING_CARDIAN)
