...
function onTrigger(player,npc)
    local inAStew = player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.IN_A_STEW)
...
function onEventFinish(player,csid,option)
    ...
    if csid == 224 and option == 1 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.CHOCOBILIOUS)
    elseif csid == 231 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.CHOCOBILIOUS, {fame=220, gil=1500, var="ChocobiliousQuest"}) then
        player:needToZone(true)

    -- IN A STEW
    elseif csid == 235 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.IN_A_STEW)
        player:setVar("IASvar", 1)
    elseif csid == 239 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.IN_A_STEW, {fame=50, gil=900, var="IASvar"}) then
