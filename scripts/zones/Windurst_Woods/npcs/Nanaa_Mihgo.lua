...
function onTrigger(player,npc)
    ...
    local mihgosAmigo = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.MIHGO_S_AMIGO)
    local tenshodoShowdown = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_TENSHODO_SHOWDOWN)
    ...
    elseif player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.LURE_OF_THE_WILDCAT_WINDURST) == QUEST_ACCEPTED and not player:getMaskBit(wildcatWindurst,4) then
    ...
    elseif mihgosAmigo == QUEST_AVAILABLE then
        if player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.CRYING_OVER_ONIONS) == QUEST_AVAILABLE then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 496) then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_TENSHODO_SHOWDOWN)
    ...
    elseif (csid == 504 and option == 1) then  -- start quest "as thick as thieves"
        player:addQuest(WINDURST,dsp.quests.ids.windurst.AS_THICK_AS_THIEVES)
    ...
    elseif (csid == 508 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.AS_THICK_AS_THIEVES, {item=12514, var={"thickAsThievesCS", "thickAsThievesGrapplingCS", "thickAsThievesGamblingCS"}})) then
    ...
    elseif csid == 512 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.HITTING_THE_MARQUISATE)
    ...
    -- ROCK RACKETEER
    elseif csid == 93 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.ROCK_RACKETEER)
    ...
    elseif csid == 80 or csid == 81 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.MIHGO_S_AMIGO)
    elseif csid == 88 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.MIHGO_S_AMIGO, {gil=200, title=dsp.title.CAT_BURGLAR_GROUPIE, fameArea=NORG, fame=60}) then
        player:confirmTrade()
