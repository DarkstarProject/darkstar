...
function onTrigger(player,npc)
    ...
    if (player:getCurrentMission(WINDURST) == LOST_FOR_WORDS and MissionStatus > 0 and MissionStatus < 5) then
        if (MissionStatus == 1) then
            player:startEvent(165,0,dsp.kis.LAPIS_CORAL,dsp.kis.LAPIS_MONOCLE);
        elseif (MissionStatus == 2) then
            player:startEvent(166,0,dsp.kis.LAPIS_CORAL,dsp.kis.LAPIS_MONOCLE);
        ...
        elseif (thickAsThievesCS >= 1 and thickAsThievesCS <= 4 and thickAsThievesGamblingCS <= 7 and thickAsThievesGrapplingCS <= 7) then
            player:startEvent(505,0,dsp.kis.GANG_WHEREABOUTS_NOTE); -- During Quest "Thick as Thieves" (before completing grappling+gambling sidequests)
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 93) then -- start quest "rock racketeer"
        ...
        player:addKeyItem(dsp.kis.SHARP_GRAY_STONE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.SHARP_GRAY_STONE);
    elseif (csid == 98) then
    ...
    elseif (csid == 496) then
        ...
        player:addKeyItem(dsp.kis.LETTER_FROM_THE_TENSHODO);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.LETTER_FROM_THE_TENSHODO);
    elseif (csid == 504 and option == 1) then  -- start quest "as thick as thieves"
        ...
        player:addKeyItem(dsp.kis.GANG_WHEREABOUTS_NOTE);
        player:addKeyItem(dsp.kis.FIRST_FORGED_ENVELOPE);
        player:addKeyItem(dsp.kis.SECOND_FORGED_ENVELOPE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.GANG_WHEREABOUTS_NOTE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.FIRST_FORGED_ENVELOPE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.SECOND_FORGED_ENVELOPE);
    elseif (csid == 508) then    -- complete quest "as thick as thieves"
        if (player:getFreeSlotsCount() == 0) then
            ...
            player:delKeyItem(dsp.kis.GANG_WHEREABOUTS_NOTE);
            player:delKeyItem(dsp.kis.FIRST_SIGNED_FORGED_ENVELOPE);
            player:delKeyItem(dsp.kis.SECOND_SIGNED_FORGED_ENVELOPE);
        end
    elseif (csid == 512) then  -- start quest "hitting The Marquisate "
        ...
        player:addKeyItem(dsp.kis.CAT_BURGLARS_NOTE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.CAT_BURGLARS_NOTE);
    ...
    elseif (csid == 165 and option == 1) then -- Windurst Mission 2-1 continuation
        ...
        player:addKeyItem(dsp.kis.LAPIS_MONOCLE);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.LAPIS_MONOCLE);
        -- Mark the progress
        player:setVar("MissionStatus",2);
    elseif (csid == 169) then -- Windurst Mission 2-1 continuation
        player:setVar("MissionStatus",4);
        player:setVar("MissionStatus_randfoss",0);
        player:delKeyItem(dsp.kis.LAPIS_MONOCLE);
        player:delKeyItem(dsp.kis.LAPIS_CORAL);
        player:addKeyItem(dsp.kis.HIDEOUT_KEY);
        player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.HIDEOUT_KEY);
