...
function onTrigger(player,npc)
    ...
    local theFangedOne = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.THE_FANGED_ONE) -- RNG flag quest
    local theFangedOneCS = player:getVar("TheFangedOne_Event")
    local sinHunting = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.SIN_HUNTING)-- RNG AF1
    local sinHuntingCS = player:getVar("sinHunting")
    local fireAndBrimstone = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.FIRE_AND_BRIMSTONE)-- RNG AF2
    local fireAndBrimstoneCS = player:getVar("fireAndBrimstone")
    local unbridledPassion = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.UNBRIDLED_PASSION)-- RNG AF3
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 351 then
        player:addQuest(WINDURST, dsp.quests.ids.windurst.THE_FANGED_ONE)
        player:setVar("TheFangedOneCS", 1)
    elseif (csid == 357 or csid == 358) and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.THE_FANGED_ONE, {item=13117, title=dsp.title.THE_FANGED_ONE, var={"TheFangedOne_Event", "TheFangedOneCS"}}) then
    ...
    elseif csid == 523 then -- start quest RNG AF1
        player:addQuest(WINDURST, dsp.quests.ids.windurst.SIN_HUNTING)
        npcUtil.giveKeyItem(player, dsp.ki.CHIEFTAINNESS_TWINSTONE_EARRING)
        player:setVar("sinHunting", 1)
    elseif csid == 527 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.SIN_HUNTING, {item=17188, var="sinHunting"}) then -- complete quest RNG AF1
    ...
    elseif csid == 531 then -- start RNG AF2
        player:addQuest(WINDURST, dsp.quests.ids.windurst.FIRE_AND_BRIMSTONE)
        player:setVar("fireAndBrimstone", 1)
    elseif csid == 535 then -- start second part RNG AF2
        player:setVar("fireAndBrimstone", 5)
    elseif csid == 537 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.FIRE_AND_BRIMSTONE, {item=12518, var="fireAndBrimstone"}) then -- complete quest RNG AF2
        player:confirmTrade()

    -- UNBRIDLED PASSION        
    elseif csid == 541 then -- start RNG AF3
        player:addQuest(WINDURST, dsp.quests.ids.windurst.UNBRIDLED_PASSION)
        player:setVar("unbridledPassion", 1)
    elseif csid == 546 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.UNBRIDLED_PASSION, {item=14099, var="unbridledPassion"}) then -- complete quest RNG AF3
