...
function onTrade(player,npc,trade)
    if player:getQuestStatus(WINDURST, dsp.quests.ids.windurst.THE_AMAZIN_SCORPIO) == QUEST_ACCEPTED and npcUtil.tradeHas(trade, 1017) then
...
function onTrigger(player,npc)
    ...
    if player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.LURE_OF_THE_WILDCAT_WINDURST) == QUEST_ACCEPTED and not player:getMaskBit(wildcatWindurst,0) then
...
function onEventFinish(player,csid,option)
    if csid == 481 then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.THE_AMAZIN_SCORPIO)
    elseif csid == 484 and npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.THE_AMAZIN_SCORPIO, {fame=80, title=dsp.title.GREAT_GRAPPLER_SCORPIO, gil=1500}) then
        player:confirmTrade()
