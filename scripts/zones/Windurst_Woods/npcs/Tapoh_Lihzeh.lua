...
function onTrade(player,npc,trade)
    -- CHOCOBILIOUS
    if player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CHOCOBILIOUS) == QUEST_ACCEPTED and player:getVar("ChocobiliousQuest") == 1 and npcUtil.tradeHas(trade, 938) then
        player:startEvent(229, 0, 938)

    -- PAYING LIP SERVICE
    elseif player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.PAYING_LIP_SERVICE) >= QUEST_ACCEPTED then
...
function onTrigger(player,npc)
    local chocobilious = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.CHOCOBILIOUS)
    local chocobiliousCS = player:getVar("ChocobiliousQuest")
    local payingLipService = player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.PAYING_LIP_SERVICE)
...
function onEventFinish(player,csid,option)
    ...
    elseif csid == 477 and option == 1 then
        player:addQuest(WINDURST,dsp.quests.ids.windurst.PAYING_LIP_SERVICE)
    elseif csid == 479 then
        if player:getQuestStatus(WINDURST,dsp.quests.ids.windurst.PAYING_LIP_SERVICE) == QUEST_ACCEPTED then
            npcUtil.completeQuest(player, WINDURST, dsp.quests.ids.windurst.PAYING_LIP_SERVICE, {fame=60, title=dsp.title.KISSER_MAKEUPPER})
        else
