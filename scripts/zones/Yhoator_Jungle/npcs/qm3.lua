...
function onTrigger(player,npc)
    if (player:getQuestStatus(OUTLANDS,TRUE_WILL) == QUEST_ACCEPTED and not player:hasKeyItem(dsp.kis.OLD_TRICK_BOX)) then
        if (player:getVar("trueWillKilledNM") >= 1) then
            if (GetMobByID(KAPPA_AKUSO):isDead() and GetMobByID(KAPPA_BONZE):isDead() and GetMobByID(KAPPA_BIWA):isDead()) then
                player:addKeyItem(dsp.kis.OLD_TRICK_BOX);
                player:messageSpecial(KEYITEM_OBTAINED,dsp.kis.OLD_TRICK_BOX);
