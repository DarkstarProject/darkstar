...
function onTrade(player,npc,trade)
    ...
    if npcUtil.tradeHas(trade, 790) then
        if not player:hasCompletedQuest(OUTLANDS,dsp.quests.ids.outlands.WRATH_OF_THE_OPO_OPOS) and (player:hasCompletedMission(ZILART,HEADSTONE_PILGRIMAGE) or player:hasKeyItem(dsp.ki.FIRE_FRAGMENT)) then
            player:addQuest(OUTLANDS,dsp.quests.ids.outlands.WRATH_OF_THE_OPO_OPOS)
...
function onEventFinish(player,csid,option)
    ...
    -- WRATH OF THE OPO-OPOS
    elseif csid == 202 and npcUtil.completeQuest(player, OUTLANDS, dsp.quests.ids.outlands.WRATH_OF_THE_OPO_OPOS, {item=13143, title=dsp.title.FRIEND_OF_THE_OPOOPOS}) then
        player:confirmTrade()
    end
end