...
// For the characters stored in the size growth,
// For the npc and monsters things like the type of model
...
struct skills_t
{
    ...
    // Rank used only in crafts. 64 needs to be compatible size ID
    uint8 rank[64];
};

struct keyitems_t
{
     uint8 keysList [320]; // Key items table
     uint8 seenList [320]; // Table of the key items that are marked as "seen"
};

struct position_t
{
    uint8 rotation; // Rotation angle of the essence with respect to its position (using the 255 system, the place of 360 °)
    float x;
    float y; // Height of the essence with respect to the location of "sea level"
    float z;
    uint16 moving; // Something like travel distance required for proper rendering on the client number of the essence steps
};
...
struct search_t
{
    uint8 language; // The preferred language of communication
    uint8 messagetype; // Type of comment

    string_t message; // Comment Search
};
...
// Brief comment to the packets below defined as constants
// 1st byte - size package
// Through 4 bytes header begins 0x49, 0x58, 0x46, 0x46 - IXFF
// After the title is presumably the type of message:
// 0x03 - positive
// 0x04 - error (in case of error ranging from 33 bytes uint16 is an error code)
