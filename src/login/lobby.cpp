	...
	if( RFIFOREST(fd) >= 1 )
	{
		...
		switch(code)
		{
			case 0xA1:
			{
				...
				memcpy(ReservePacket+60,"DarkStar", 8); // максимум 15 символов ?
		...
		case 0x26:
			{
				...
				WBUFW(ReservPacket,32) = 0xFFFF;	// BitMask for expansions;
		...
		case 0x24:
			{
				...
				memcpy(ReservePacket+36,"DarkStar",8);
...
int32 lobby_createchar_save(uint32 accid, uint32 charid, char_mini* createchar)
{
	...
	fmtQuery = "INSERT INTO char_stats(charid,mjob) VALUES(%u,%u);";
