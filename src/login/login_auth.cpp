...
int32 login_parse(int32 fd)
{
    ...
    if (session[fd]->rdata.size() == 33)
    {
        ...
        int8 code = ref<uint8>((uint8*)buff, 32);
        ...
        if (check_string(name, 16) && check_string(password, 16))
        {
            ...
            ref<uint8>((uint8*)session[fd]->wdata.data(), 0) = LOGIN_ERROR;
            do_close_login(sd, fd);
            return -1;
        }
        ...
        case LOGIN_ATTEMPT:
        {
            ...
            if (ret != SQL_ERROR  && Sql_NumRows(SqlHandle) != 0)
            {
                ...
                if (status & ACCST_NORMAL)
                {
                    ...
                    ref<uint8>((uint8*)session[fd]->wdata.data(), 0) = LOGIN_SUCCESS;
                    ref<uint32>((uint8*)session[fd]->wdata.data(), 1) = sd->accid;
                    flush_fifo(fd);
                    do_close_tcp(fd);
                }
        ...
        case LOGIN_CREATE:
            ...
            {
                session[fd]->wdata.resize(1);
                ref<uint8>((uint8*)session[fd]->wdata.data(), 0) = LOGIN_ERROR_CREATE;
                do_close_login(sd, fd);
                return -1;
            }
            ...
            if (Sql_NumRows(SqlHandle) == 0)
            {
                ...
                session[fd]->wdata.resize(1);
                ref<uint8>((uint8*)session[fd]->wdata.data(), 0) = LOGIN_SUCCESS_CREATE;
                do_close_login(sd, fd);
            }
            else {
