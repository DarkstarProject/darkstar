...
void message_server_parse(MSGSERVTYPE type, zmq::message_t* extra, zmq::message_t* packet, zmq::message_t* from)
{
    ...
    if (from)
    {
        from_ip.s_addr = ref<uint32>(from->data(), 0);
        from_port = ref<uint16>(from->data(), 4);
    }
    switch (type)
    {
    case MSG_CHAT_TELL:
    ...
    case MSG_LINKSHELL_REMOVE:
    {
        ...
        if (Sql_NumRows(ChatSqlHandle) == 0)
        {
            ...
            ret = Sql_Query(ChatSqlHandle, query, ref<uint32>(extra->data(), 0));
        }
        break;
    }
    case MSG_CHAT_PARTY:
    ...
    case MSG_PT_DISBAND:
    {
        ...
        uint32 partyid = ref<uint32>(extra->data(), 0);
    ...
    case MSG_CHAT_LINKSHELL:
    {
        ...
        ret = Sql_Query(ChatSqlHandle, query, ref<uint32>(extra->data(), 0), ref<uint32>(extra->data(), 0));
        break;
    }
    case MSG_CHAT_YELL:
    {
    ...
    if (ret != SQL_ERROR)
    {
        ...
        while (Sql_NextRow(ChatSqlHandle) == SQL_SUCCESS)
        {
            ...
            {
                ref<uint32>(extra->data(), 0) = Sql_GetUIntData(ChatSqlHandle, 2);
            }
...
void message_server_listen()
{
    while (true)
    {
        ...
        message_server_parse((MSGSERVTYPE)ref<uint8>(type.data(), 0), &extra, &packet, &from);
    }
}
