...
void CBattlefield::StartAllyAssist()
{
    ...
    for (int i = 0; i < m_PlayerList.size(); i++)
    {
        CCharEntity* PCurrentChar = (CCharEntity*)m_PlayerList.at(i);
    ...
    // Attack their target if found.
    if (assistTarget > 0)
    {
        for (int i = 0; i < m_AllyList.size(); i++)
        {
            CMobEntity* PCurrentAlly = (CMobEntity*)m_AllyList.at(i);
    ...
    // We couldn't find an engaged player... so...
    ...
    for (int i = 0; i < m_AllyList.size(); i++)
    {
        ...
        {
            uint32 mobCount = m_EnemyList.size();
            ...
            {
                CMobEntity* PRandomMob = (CMobEntity*)m_EnemyList.at(j);
