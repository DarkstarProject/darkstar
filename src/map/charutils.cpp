...
void DistributeExperiencePoints(CCharEntity* PChar, CMobEntity* PMob)
{
		...
		if (baseexp != 0)
		{
			for (uint8 i = 0; i < PChar->PParty->members.size(); ++i)
			{
				...
				if(PMember->getZone() == PMob->getZone())
				{
					...
					if (PMember->getMod(MOD_DEDICATION) != 0)
					{
						dedication = cap_value(exp * PMember->getMod(MOD_DEDICATION) / 100, 0, PMember->getMod(MOD_DEDICATION_CAP));
						PMember->setModifier(MOD_DEDICATION_CAP, PMember->getMod(MOD_DEDICATION_CAP) - dedication);
						if (PMember->getMod(MOD_DEDICATION_CAP) == 0)
						{
							PMember->StatusEffectContainer->DelStatusEffect(EFFECT_DEDICATION);
						}
						exp += dedication;
					}
	...
	else
	{
		...
		{
			dedication = cap_value(exp * PChar->getMod(MOD_DEDICATION) / 100, 0, PChar->getMod(MOD_DEDICATION_CAP));
...
uint8 checkMultiHits(CCharEntity* PChar, uint16 weaponid){
	...
	case 17440: //kraken club 2-8 (5:15:25:25:15:10:3:2) cdf = 5,20,45,70,85,95,98,100
		distribution = rand()%100;
		if(distribution < 5){ break; }
		else if(distribution < 20){ num++; break; }
		...
		break;
	case 17813: //sobero sukehiro 2-3 cdf=30,80,100
	case 18020: //mercurial kris 2-3 cdf=30,80,100
	case 16555: //ridill 2-3 cdf=30,80,100
	case 16850: //mercurial spear 2-3 cdf=30,80,100
	case 18507: //kinship axe 2-3 cdf=30,80,100
	case 18061: //bahamut's zaghnal 2-3 cdf=30,80,100
		distribution = rand()%100;
		if(distribution < 30){ break; }
		else if(distribution < 80){ num++; break; }
		else{ num+=2; break; }
		break;
	case 18377: //mercurial sword 2-4 cdf=20,50,80,100
		distribution = rand()%100;
		if(distribution < 20){ break; }
		else if(distribution < 50){ num++; break; }
		else if(distribution < 80){ num+=2; break; }
		else{ num+=3; break; }
		break;
	case 19314: //quint spear 2-5 cdf=10,30,60,90,100
	case 17586: //mercurial pole 2-5 cdf=10,30,60,90,100
		distribution = rand()%100;
		if(distribution < 10){ break; }
		else if(distribution < 30){ num++; break; }
		else if(distribution < 60){ num+=2; break; }
		else if(distribution < 90){ num+=3; break; }
		else{ num+=4; break; }
		break;
	case 17969: //fendoir 2 cdf=55,100
	case 17652: //joyeuse 2 cdf=55,100
		distribution = rand()%100;
	...
	{
		if(((CBattleEntity*)PChar)->StatusEffectContainer->HasStatusEffect(EFFECT_HASSO))
		{
			if(rand()%100 < ((float)PChar->getMod(MOD_ZANSHIN))/4)
			{
				num++;
			}
		}
