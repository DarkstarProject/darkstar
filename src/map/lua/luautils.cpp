...
#define lua_prepscript(n,...) int8 File[255]; int32 oldtop = lua_gettop(LuaHandle); \
    ...
    ************************************************************************/
    ...
    int32 init()
    {
        ...
        lua_register(LuaHandle, "AllowRespawn", luautils::AllowRespawn);
    ...
    *                                                                       *
    ************************************************************************/
    int32 AllowRespawn(lua_State* L)
    {
        if (!lua_isnil(L, 1) && lua_isnumber(L, 1))
        {
            uint32 mobid = (uint32)lua_tointeger(L, 1);
            ...
            if (PMob != nullptr)
            {
                ...
                {
                    ShowDebug(CL_RED"AllowRespawn: Boolean parameter not given, mob <%u> SpawnType unchanged.\n" CL_RESET, mobid);
                }
            }
            else
            {
                ShowDebug(CL_RED"AllowRespawn: mob <%u> not found\n" CL_RESET, mobid);
            }
            return 0;
        }
        lua_pushnil(L);
