...
namespace message
{
    ...
    void parse(MSGSERVTYPE type, zmq::message_t* extra, zmq::message_t* packet)
    {
        ...
        case MSG_LOGIN:
        {
            CCharEntity* PChar = zoneutils::GetChar(ref<uint32>(extra->data(), 0));

            if (!PChar)
            {
                Sql_Query(SqlHandle, "DELETE FROM accounts_sessions WHERE charid = %d;", ref<uint32>(extra->data(), 0));
            }
            else
            {
                ...
                if (PChar->PParty)
                {
                    if (PChar->PParty->m_PAlliance != nullptr)
                    {
                        for (uint8 i = 0; i < PChar->PParty->m_PAlliance->partyList.size(); ++i)
                        {
                            ...
                            memcpy(*newPacket, packet->data(), std::min<size_t>(packet->size(), PACKET_SIZE));
                            ((CParty*)PChar->PParty->m_PAlliance->partyList.at(i))->PushPacket(ref<uint32>(extra->data(), 4), 0, newPacket);
                        }
                    }
                    else
                    {
                        CBasicPacket* newPacket = new CBasicPacket();
                        memcpy(*newPacket, packet->data(), std::min<size_t>(packet->size(), PACKET_SIZE));
                        PChar->PParty->PushPacket(ref<uint32>(extra->data(), 4), 0, newPacket);
                    }
                }
            }
            break;
        ...
        {
            uint32 linkshellID = ref<uint32>(extra->data(), 0);
            CLinkshell* PLinkshell = linkshell::GetLinkshell(linkshellID);
            if (PLinkshell)
            {
                CBasicPacket* newPacket = new CBasicPacket();
                memcpy(*newPacket, packet->data(), std::min<size_t>(packet->size(), PACKET_SIZE));
                PLinkshell->PushPacket(ref<uint32>(extra->data(), 4), newPacket);
            }
            break;
        }
        case MSG_CHAT_YELL:
            ...
            zoneutils::ForEachZone([&packet, &extra](CZone* PZone)
            {
                if (PZone->CanUseMisc(MISC_YELL))
                {
                    PZone->ForEachChar([&packet, &extra](CCharEntity* PChar)
                    {
                        ...
                        if (PChar->id != ref<uint32>(extra->data(), 0))
                        {
                            CBasicPacket* newPacket = new CBasicPacket();
                            memcpy(*newPacket, packet->data(), std::min<size_t>(packet->size(), PACKET_SIZE));
        ...
        case MSG_PT_INVITE:
        {
            uint32 id = ref<uint32>(extra->data(), 0);
            // uint16 targid = ref<uint16>(extra->data(), 4);
            uint8 inviteType = ref<uint8>(packet->data(), 0x0B);
            ...
            if (PInvitee)
            {
                ...
                {
                    ref<uint32>(extra->data(), 0) = ref<uint32>(extra->data(), 6);
                ...
                PInvitee->InvitePending.id = ref<uint32>(extra->data(), 6);
                PInvitee->InvitePending.targid = ref<uint16>(extra->data(), 10);
                CBasicPacket* newPacket = new CBasicPacket();
                memcpy(*newPacket, packet->data(), std::min<size_t>(packet->size(), PACKET_SIZE));
        ...
        case MSG_PT_INV_RES:
        {
            uint32 inviterId = ref<uint32>(extra->data(), 0);
            // uint16 inviterTargid = ref<uint16>(extra->data(), 4);
            uint32 inviteeId = ref<uint32>(extra->data(), 6);
            // uint16 inviteeTargid = ref<uint16>(extra->data(), 10);
            uint8 inviteAnswer = ref<uint8>(extra->data(), 12);
        ...
        case MSG_PT_RELOAD:
        {
            if (extra->size() == 8)
            {
                CCharEntity* PChar = zoneutils::GetCharToUpdate(ref<uint32>(extra->data(), 4), ref<uint32>(extra->data(), 0));
                if (PChar)
                {
        ...
        {
            CCharEntity* PChar = zoneutils::GetChar(ref<uint32>(extra->data(), 0));
            uint32 id = ref<uint32>(extra->data(), 4);
            if (PChar)
            {
                if (PChar->PParty)
                {
        ...
        {
            CLinkshell* PLinkshell = linkshell::GetLinkshell(ref<uint32>(extra->data(), 24));

            if (PLinkshell)
            {
                PLinkshell->ChangeMemberRank((int8*)extra->data() + 4, ref<uint8>(extra->data(), 28));
            }
            break;
        }
        case MSG_LINKSHELL_REMOVE:
        {
            ...
            if (PChar && PChar->PLinkshell1 && PChar->PLinkshell1->getID() == ref<uint32>(extra->data(), 24))
            {
                uint8 kickerRank = ref<uint8>(extra->data(), 28);
            ...
            else if (PChar && PChar->PLinkshell2 && PChar->PLinkshell2->getID() == ref<uint32>(extra->data(), 24))
            {
                uint8 kickerRank = ref<uint8>(extra->data(), 28);
        ...
        case MSG_SEND_TO_ZONE:
        {
            ...
            {
                uint32 requester = ref<uint32>(extra->data(), 4);
                ...
                uint16 zoneId = ref<uint16>(extra->data(), 8);
                float x = ref<float>(extra->data(), 10);
                float y = ref<float>(extra->data(), 14);
                float z = ref<float>(extra->data(), 18);
                uint8 rot = ref<uint8>(extra->data(), 22);
                uint32 moghouseID = ref<uint32>(extra->data(), 23);
    ...
    void listen()
    {
        while (true)
        {
            ...
            parse((MSGSERVTYPE)ref<uint8>(type.data(), 0), &extra, &packet);
        }
    }
    ...
    void send(MSGSERVTYPE type, void* data, size_t datalen, CBasicPacket* packet)
    {
        ...
        ref<uint8>(msg.type->data(), 0) = type;
