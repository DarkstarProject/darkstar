        ...
        case MSG_SEND_TO_ZONE:
        {
            ...
            if (PChar && PChar->loc.zone)
            {
                ...
                if (requester != 0)
                {
                    ...
                    ref<uint32>((uint8*)&buf, 0) = requester;
                    ref<uint16>((uint8*)&buf, 8) = PChar->getZone();
                    ref<float>((uint8*)&buf, 10) = PChar->loc.p.x;
                    ref<float>((uint8*)&buf, 14) = PChar->loc.p.y;
                    ref<float>((uint8*)&buf, 18) = PChar->loc.p.z;
                    ref<uint8>((uint8*)&buf, 22) = PChar->loc.p.rotation;
                    ref<uint32>((uint8*)&buf, 23) = PChar->m_moghouseID;
