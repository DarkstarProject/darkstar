...
int16 CMobSpellContainer::GetAggroSpell()
{
  // high chance to return ga spell
    if (HasGaSpells() && WELL512::irand() % 100 <= m_PMob->getMobMod(MOBMOD_GA_CHANCE)){
    return GetGaSpell();
  }
...
int16 CMobSpellContainer::GetSpell()
{
  ...
  if (HasHealSpells() && m_PMob->GetHPP() <= m_PMob->getMobMod(MOBMOD_HP_HEAL_CHANCE) && WELL512::irand() % 100 < m_PMob->getMobMod(MOBMOD_HEAL_CHANCE)){
    return GetHealSpell();
  }

  // almost always use na if I can
  if (HasNaSpells() && WELL512::irand() % 100 < m_PMob->getMobMod(MOBMOD_NA_CHANCE)){
  ...
  if (HasBuffSpells() && WELL512::irand() % 100 < m_PMob->getMobMod(MOBMOD_BUFF_CHANCE)){
    return GetBuffSpell();
  }
...
int16 CMobSpellContainer::GetGaSpell()
{
  ...
  return m_gaList[WELL512::irand() % m_gaList.size()];
}
...
int16 CMobSpellContainer::GetDamageSpell()
{
  ...
  return m_damageList[WELL512::irand() % m_damageList.size()];
}
...
int16 CMobSpellContainer::GetBuffSpell()
{
  ...
  return m_buffList[WELL512::irand() % m_buffList.size()];
}
...
int16 CMobSpellContainer::GetHealSpell()
{
  ...
  return m_healList[WELL512::irand() % m_healList.size()];
}
