...
CFishingPacket::CFishingPacket(std::uint16_t stamina, std::uint16_t regen, std::uint16_t response, std::uint16_t hitDmg, std::uint16_t arrowDelay, std::uint16_t missRegen, std::uint16_t gameTime, std::uint8_t sense, uint32 special)
{
    this->type = 0x15; //0x115
    this->size = 0x0D;

    ref<std::uint16_t>(0x04) = stamina;    // fish HP, generally in thousands - (100 + x)(floor(fishLvl / 2) + 18)
    ref<std::uint16_t>(0x06) = arrowDelay; // how long you have to hit the arrows, generally low 10s, max 15? Increased by 2 by Penguin Ring
    ref<std::uint16_t>(0x08) = regen;      // how much stamina fish regains/loses per frame.  base 128 -drain +regen - reduced by same factor of 'x' affected by: advanced support, apron/smock, ebisu rod
    ref<std::uint16_t>(0x0A) = response;   // fish movement, how active the fish is moving left to right (multiples of 20) - Increased by 2 by Penguin Ring
