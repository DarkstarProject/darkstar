...
CInventoryItemPacket::CInventoryItemPacket(CItem* PItem, uint8 LocationID, uint8 SlotID) 
{
	...
	if (PItem != nullptr)
	{
		...
		{
			WBUFB(data,(0x11)) = 0x01;    // flag ITEM_CHARGED
            ...
            if (((CItemUsable*)PItem)->getCurrentCharges() > 0)
            {
                ...
                else
                {
                    ...
                    WBUFL(data,(0x15)) = ((CItemUsable*)PItem)->getNextUseTime();             // next use the timer
                    WBUFL(data,(0x19)) = ((CItemUsable*)PItem)->getUseDelay() + CurrentTime;  // Use a timer delay
                }
            }
		}
