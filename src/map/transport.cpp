...
void CTransportHandler::TransportTimer() 
{
    ...
    for(uint32 i = 0; i < TransportList.size(); ++i)
    {
        ...
        if (ShipTimerOffset == 0)
        {
            ...
            ref<uint32>((uint8*)&PTransport->PTransportNPC->name[0],4) = CVanaTime::getInstance()->getVanaTime();
...
void CTransportHandler::startElevator(Elevator_t * elevator)
{
    ...
    ref<uint32>((uint8*)&elevator->Elevator->name[0],4) = CVanaTime::getInstance()->getVanaTime();
