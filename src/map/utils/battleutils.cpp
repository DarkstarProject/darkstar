...
#define PAIR(x, y) ((x << 8) + y)
    ...
    ************************************************************************/
    ...
    uint32 CalculateSpellCastTime(CBattleEntity* PEntity, CSpell* PSpell)
    {
        ...
        if (PSpell->getSkillType() == SKILLTYPE::SKILL_ELE) // Elemental Celerity reductions
        {
    ...
    int16 CalculateSpellTP(CBattleEntity* PEntity, CSpell* PSpell)
    {
        ...
        if (PSpell->getSkillType() == SKILLTYPE::SKILL_ELE || PSpell->getSkillType() == SKILLTYPE::SKILL_DRK)
        {
            if (CCharEntity* PChar = dynamic_cast<CCharEntity*>(PEntity))
            {
                ...
                {
                    tp = static_cast<int16>(PSpell->getMPCost() * PChar->getMod(Mod::OCCULT_ACUMEN) / 100.f * (1 + (PChar->getMod(Mod::STORETP) / 100.f)));
                }
            }
        }
