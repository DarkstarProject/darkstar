...
void LoadZoneList()
{
	...
	const int8* query = "SELECT zoneid FROM zone_settings WHERE IF(%d <> 0, %d = zoneip AND %d = zoneport, TRUE);";

	int ret = Sql_Query(SqlHandle, query, map_ip, map_ip, map_port);
